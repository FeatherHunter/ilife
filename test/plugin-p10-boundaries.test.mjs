...
describe('P10 依赖方向', () => {
  it('单品→总管单向（总管零单品依赖）', () => {
    ...
    // #48 样板线：plugin-calorie 总管依赖已转正式版号 ^0.1.0（B① 全部换已发布号）；#50 首对复制 plugin-chef、home 对 plugin-home-ilife、bill 对 plugin-bill-ilife、schedule 对 plugin-schedule-ilife 同改（见 docs/skill-landing-r2.md）。
    const FORMAL48 = new Set(['plugin-calorie', 'plugin-chef', 'plugin-home-ilife', 'plugin-bill-ilife', 'plugin-schedule-ilife']);
    const SKILL_OF = { 'plugin-calorie': 'skill-calorie', 'plugin-chef': 'skill-chef', 'plugin-home-ilife': 'skill-home', 'plugin-bill-ilife': 'skill-bill', 'plugin-schedule-ilife': 'skill-schedule' };
