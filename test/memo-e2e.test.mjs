...
// （两者都住 `CFG`／`memo.yaml`）。测试隔离的唯一口子是**家目录**（`CFG` 就是那条临时家目录）：
// 跑在 node 测试运行器里却回落到真实家目录时，公共层直接抛 `CONFIG_TEST_ISOLATION_MISSING`（响亮失败）。
...
before(() => {
  ...
  // #695 起配置文件的唯一真相是 `<临时家目录>/.ilife/memo.yaml`（环境变量读取已删）：
  ...
  useHome(CFG); // 当刻进程这一格也指过去：`run()` 起的子进程是 `{...process.env}`，继承的就是这份家目录
});
