...
const doc = readFileSync(join(root, 'docs/bill-migration-split.md'), 'utf8');
const fence = doc.split('```bill-keys')[1].split('```')[0].trim().split('\n');
const rows = fence.map((l) => l.split('|').map((s) => s.trim()));

describe('饼干拆分确认 #19', () => {
  it('16 联动 key×shape 全合法（命名空间+6 形状）', () => {
