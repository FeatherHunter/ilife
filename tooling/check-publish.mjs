...
function gateFreshTmp() {
  ...
  try {
    ...
    '  dbs.push(db);',
    "  // #727／#754：技能侧「库在哪」由配置文件定（`<配置目录>/<技能>.yaml`），配置目录＝**家目录**下的 `.ilife`；",
    "  // 隔离改走**家目录注入**（win32 `USERPROFILE`／POSIX `HOME`）——旧布景（`SKILLS_DB_PATH` ＋ `ILIFE_CONFIG_DIR`）已随",
    "  // 「0 环境变量」退役，沿用会让子进程落到**真实家目录**（实测：备忘取数 exit4 假红，还有写真实数据的风险）。",
    "  const home = mkdtempSync(join(tmpdir(), 'ilife-g3-home-'));",
    '  dbs.push(home);',
    "  const cfgDir = join(home, '.ilife');",
    "  mkdirSync(join(cfgDir, 'data'), { recursive: true });",
    "  if (skill === 'skill-memo-ilife') initMemoTestDb(join(cfgDir, 'data', 'memo.db'));",
    "  const r = spawnSync(process.execPath, [p, key, '--params', '{}'], { encoding: 'utf8', env: { ...process.env, USERPROFILE: home, HOME: home } });",
