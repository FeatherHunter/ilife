 ...
 *
 * **定论（源面取哪一态）**：源面取**工作区态**，不取索引态——
 * ① 本工具量的是「窗内工作区有没有被人动过」（`--compare` 比的是 `saveBaseline(当刻)` 与基线 JSON），
 *    工作区正是它要对的对象；
 * ② 索引里「已删、未提交」的件在盘上并不存在，它**不该进源面**（其删除正是 `--compare` 该报的差异），
 *    硬进就是 `ENOENT` 把整个工具打成崩；
 * ③ 因此源面件数在「已删未提交」期间会比索引少，这是**如实读数**，不是漏报。
 * 读者注意：`--compare` 证的是「存基线之后没再被动过」，**不是**「跨窗口两次作业前后一致」——
 * 后者的判据要按各自票面另立（搬迁类＝页产物逐字节）。出处：票 #715 实测，见
 * `docs/skills/skill-calorie/t715-搬家-证据.md` §4。
