...
assert(combos.dependencies?.['@feather_wch/base-link-core'] !== undefined, 'combos 强依赖 link-core');
const present = readFileSync(join(root, 'packages/base-combos/src/present.ts'), 'utf8');
assert(!present.includes('@feather_wch/base-render') && !/from\s+['"].*render/.test(present), 'present 只许字符串级引用，禁 import render');
...
assert(!coreSrc.includes('@feather_wch/'), 'link-core 源码不引用任何 workspace 包');
// 装配归一：注册原语只许住 render
