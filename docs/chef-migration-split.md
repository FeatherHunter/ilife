...
老家只读对照 `D:/2Study/StudyNotes/SKILLS/私家大厨`：py 84（核心 73 + 杂项 11，总行 16574；杂项为 .scratch/v4-impl-map 10 + v4-spec 1，只读不迁），SKILL.md 实测 541 行，17 表（recipes/ingredients/steps/history 主 4 + 营养/分类/计划扩展 13），scenes/ 10 域（recipe/view/search/write/cooking/shopping/history/help/plan/stock/link），scenarios 61 + alias（合计 35 唤醒词，HELP 现找 4 条含内），文件 DB（chef_data.db；老家无 DB 描述按 issue #18，TS 新建 sqlite，见下）。
...
| 口径 | src/policy/category.ts 合一（难度 5 档 + 状态 4 种 + 火候 5 档 + 食材 11 类 + 评分 0-5 含小数）+ wakewords.ts（35 短语→8 key，最长匹配） | SKILL.md 实测 541 行口径段 + references/scenarios（61 场景 + alias） | 口径 |
...
| SKILL | SKILL.md 重写+HELP 互联注入（35 行速查表，构建期静态文本） | SKILL.md 实测 541 行（含 Language/command_cn/scenario/HELP 保留字） | SKILL |
...
口径说明（6 处 vs 8 key）：外部消费侧 6 处（search/view/write/cooking.run/shopping.query/history.record 为外部可调；history.query 的 quality/backup 与 help.lookup 为内部体检/现找能力，无外部 combos 消费），故 8 key 含 2 内部键。combos 落表后续小票：#18 后续（合流后新建 combos 落表票，沿 P8 冻结口径，本修不碰 combos.yaml）。
