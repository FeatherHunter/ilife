...
**控件样式区边界**：`CONTROL_STYLE_SECTIONS` 是**闭集**，每区一个类名命名空间（自动加 `STYLE_PREFIX = 'ilife-'`）。样式唯一真相源在 base-paint；单品包禁自带样式常量（边界规则 6）。技能现有私有 CSS 串（bill／chef／home／schedule 逐字重复约 300 字符）随 #75 删除。**（落点注记：本票不执行，随 #96 门 → #108–#113 真迁移；记账见 §8.11 D2／返修台账 FX-75-10。）**
...
| `actionBar` | `renderActionBar(input: ActionBarInput): string` | 场景按钮（`kind: primary／red`）＋ 复制数据／复制日志 ghost 按钮（独立一行，偶数一行 2 个，`min-height` 40px、12px/600、描边透明度 0.38）；复制文本在**渲染期**序列化后写入 `data-t`，`actionId`（`CopyButtonInput.actionId`，**必填**）写入 `ACTION_ID_ATTR`，零注入面。**三格式形态（#247，2026-09-12 用户裁定「取老仓原样」）**：`CopyButtonInput.formats`（`CopyFormatTexts`，与 `text` 互斥）给了就出「复制数据 ▾ ＋ 纯文本／JSON／CSV 三选一菜单」——按钮只开合菜单、**不写** `data-t` 也**不写** `ACTION_ID_ATTR`（挂 `data-fmt-open="1"` 作开合标记），菜单三项各带 `data-fmt="<键>"`（键取 `COPY_FORMATS`）＋ 自己的 `data-t`（该格式已序列化文本）＋ `data-action-id` **留空**（靠 `data-t` 分辨，与 HELP 速查台逐行写同一冻结 id 同口径） | 同 `toast` 的入参校验；`copyData`／`copyLog` 缺 `actionId` 或 id 重复 → 抛 `ControlsError` code `bad-input`；`formats` 与 `text` 同给、缺某个格式、`hints` 非三串 → 同码 |
...
- **三格式菜单的例外（#247，2026-09-12 用户裁定「恢复老仓原样」）**：`CopyButtonInput.formats` 的菜单形态**不新增 actionId**、也不占 `data-action-id`：
  - 开合器按钮挂 `data-fmt-open="1"`（**开合标记**，不是 id）：点击的效果是开合菜单，不是复制，故它既不是复制目标、也不进 `listActionIds()`——`bindCopyAction` 天然不碰它，**无需为此改任何冻结签名**（模块侧只读 `formats` 出标记，行为全在页面运行时的委派里）。
  - 菜单三项各带 `data-fmt="<键>"`（键取 `COPY_FORMATS`）＋ 自己的 `data-t`（该格式已序列化文本）；`actionId` **留空**——三颗逐字不同的 id 会与上面「同次渲染内唯一」的口径冲突，而页面运行时的委派读的是**被点击元素**自己的 `data-t`（不做 id → 元素反查），故三项共用同一份（或不给）id 不产生歧义；调用方若经 `bindCopyAction` 接线，只会订阅到开合器之外的复制按钮（如复制日志），菜单项由其**自己**的页面运行时处理。
  - 唯一性的可断言形态因此改为：**菜单项不计入**「页内 `data-action-id` 集合」，`data-fmt` 集合恒为 `COPY_FORMATS` 逐字同序。证据：`packages/base-render/test/copy-format-menu-247.test.mjs` S1/S2/S3。
- **`format` 与 `formats` 的分工（#247，与 76-6 不冲突）**：`CopyButtonInput.format` 仍**不参与渲染**（序列化归 #77，`text` 恒为已序列化字符串）；`formats` 是**已序列化结果**的三种形态载体（三份 `text`，不读 `format` 语义），渲染端只做「各写各的 `data-t`」。
...
| C-5 不实现 `formatMenu`／`download` | **已被 #247 部分取代（记账）**：`formatMenu` 的落点改为 `CopyButtonInput.formats`（三格式三选一菜单，2026-09-12 用户裁定「取老仓原样」），**不是**旧契约里的 `opts.formatMenu` 布尔开关；`download` 仍不实现 | `src/spec/controls.ts` `CopyFormatTexts`；契约 §3.3「三格式菜单的例外」；冻结面**未增删任何条目**（无新出口、无 getter） |
...
| **D3** `extraCss` 是否运行时校验 | **D3 修订（编排者裁定，覆盖原「不校验」）**：契约 doc:299-300 三条「不得」（不得改写基座值／不得新增 token 名／不得引入 Q14 禁入项）＋ doc:292 深色区禁令**原无任何落点** → 补**最小强制**：`extraCss` 命中 ① `:root` 选择器、② `STYLE_FORBIDDEN_TOKENS`（`--r-xl`／`--pink`）、③ 深色区选择器（`[data-theme`／`prefers-color-scheme: dark`）**任一**即抛错。错误形态沿用仓内既有口径：**不导出**的 Error（`name='StyleSheetError'`）＋ `code`（`extra-css-root`／`extra-css-forbidden-token`／`extra-css-dark-scheme`），参照 #74 `TemplateError`／#76 `bad-input` 的「不导出、按 name／code 判定」先例；**零新增对外导出**（冻结面 130 条不变）。合法的技能作用域覆盖块（`.ilife-<skill>{--blue:…}`）照常通过；**未知 token 名不强制**（契约未冻结 token 名闭集的判定方式 → 保持调用方责任）。**如需回退为纯调用方责任：删 `src/style.ts` 的 `assertExtraCss()` 单点即可**（T24 会同步变红）。理由：契约三条「不得」若无落点即无法被验收。 | `src/style.ts` `assertExtraCss()`／`buildStyleSheet`；`style.test.mjs` T14／T15／T16／**T24** |
...
**未定口径（登记，不冻结也不排除）**：① ~~`extraCss` 违规的强制失败路径~~（**已定：D3 修订 = 三禁强制**，见上表 D3 行）；② `charts` 区是否应进共享样式表（契约 §3.2 闭集含 `charts`，§3.5 声明「各有唯一产出者」，二者张力已在记账 3 写明；若后续裁定不入，删 `SECTION_BUILDERS.charts` 单点即可）；③ 零渐变的唯一例外（`chartsCss` 的虚线图例 `repeating-linear-gradient`，属 #78 冻结产出、本票必须复用）；④ `extraCss` 的**未知 token 名**是否也应拦（现**不强制**，见 D3 修订）；⑤ 三禁守卫是**文本级**判定（`/:root/i`、`STYLE_FORBIDDEN_TOKENS`、`/[data-theme/i`／`prefers-color-scheme: dark`），**不解析 CSS AST**——**两面都记**：**误拦面**（第二轮 W6 已修：注释里的 `:root`／`--pink` 不参与判定；`--pink` 改 token 边界匹配，`--pinkish`／`--r-xlarge` 不再误拦）；**漏拦面**（已知未覆盖的等价绕过写法，如 `html{--blue:…}` 改写根元素、自定义属性名大小写变体）仍属**调用方责任**；如需覆盖，须先裁定扩大三禁范围（不在本票授权内）。

#### 8.11.1 #75 返修台账（A1 PASS 87／A2 FAIL 77 → 编排者裁定返修）

对抗式审查（A1／A2）结论：A1 无 S1（87/100）、A2 有 S1（77/100）→ 编排者裁定**返修**。逐条处置（**禁止默默略过**）：

| 编号 | 缺陷（审查实测锚点） | 处置 | 落点／证据 |
|---|---|---|---|
| FX-75-1（**S1**） | 运行时 toast 布局回归：`style.ts:112-129` 的 `.ilife-toast{display:flex;align-items:flex-start;gap:12px}` 只适配**静态产出器**（`controls.ts:216-221` 有 `.ilife-toast-body` 包裹），helpers 运行时 DOM（`controls.ts:577-596`）**无**包裹 → 标题与详情同排（A2 浏览器实测 `rt_title_top=14／rt_detail_top=14`，toast 高 53px；静态同组件 102px） | **修**：`.ilife-toast{flex-wrap:wrap}` ＋ `.ilife-toast-title-detail{flex:1 1 100%}`；另把 `.ilife-toast-body` 的 `flex:1 1 auto` → `flex:1 1 0%` 防静态侧 wrap 折行 | `src/style.ts` toast 区；`style.test.mjs` **T23**；浏览器实测 `H-12c`（`rt_title_top=621／rt_detail_top=653／rt_toast_h=113px`）＋`B-12h`（静态 `404/404/404` 单行） |
| FX-75-2（S2） | 圆角越集 ＋ 证据断言被私自放宽：`.ilife-toast-count{border-radius:6px}` 越出 H-10 允许集，而 `t75-visual-evidence.mjs` 的允许表被加了 `'2px','6px'` → 该条恒 PASS | **修**：`6px` → `8px`；**恢复严格集** `{8,14,20,999,50%}`，仅 **charts 段**单独豁免 `2px`（#78 图例色块） | `src/style.ts` toast 区；证据脚本 `H-10a`／`H-10a2`／`H-10c`；变异 **M10**（改回 6px → 视觉脚本红） |
| FX-75-3（S2） | T8／T10 对「基座规则整条删除」不敏感（A2 持锁实测：删 `.ilife-toast {`／`.ilife-copy-btn {` 整条 → 23 用例仍全绿）；根因 `style.test.mjs:209` 负向前瞻＋`:234` `css.includes('.')` 子串匹配被 `.ilife-toast-stack`／`.ilife-copy-btn-primary` 误满足 | **修**：新增 `ruleBlocks()` 规则块解析器 ＋ `SECTION_BASE_RULE`（每区基座规则 ＋ **声明数下限**）；T8／T10 改**规则块选择器级**断言 | `test/style.test.mjs` T8／T10；变异 **M6**（删 toast 基座块）／**M7**（删 copy-btn 基座块）／**M8**（掏空至 1 条声明）→ 均红 |
| FX-75-4（S2） | `statusBadge` 逐值偏离且未记账：实测 `padding:1px 8px／font-size:11px／gap:normal`，施工单 B §1.4 与旧 `.hm-status` 为 `6px 14px／13px／gap:6px` | **修**：对齐规格值（`gap:6px`／`padding:6px 14px`／`font-size:13px`／`width:fit-content`） | `src/style.ts` statusBadge 区；浏览器 `B-12d`（四态底色可区分）＋ CSS 逐值断言（T8 基座声明数 11） |
| FX-75-5（S2） | `errorReceipt` 按钮布局不等价：`style.ts:433-437` 单 flex 行（A2 实测 retry 独占 1352px、两个 ghost 仅 78px；旧层 `base.css:256-257` 是两行 grid、ghost 各半宽） | **修**：改 `grid-template-columns: repeat(ACTION_BAR_DEFAULTS.evenRowPairs, minmax(0,1fr))` ＋ `.ilife-error-actions > .ilife-copy-btn-wide{grid-column:1/-1}`，对齐旧层两行语义 | `src/style.ts` errorReceipt 区；浏览器 `B-12i`（`grid/2`）／`B-12j`（`retry_w=1337px＝容器宽`）／`B-12k`（`ghost_w=665px ≈ (1337−8)/2`）；变异 **M12** |
| FX-75-6（S3） | H-04 取证漏段：`nonCharts = slice(0, indexOf('/* charts */'))` 只覆盖 charts **之前** → helpShell（在 charts 之后）未纳入渐变检查 | **修**：`nonCharts` = 「除 charts 段外的**全部**产出文本」（按两区注释头切段，锚点缺失即显式失败） | 证据脚本 `H-04`／`H-04b`（自证切片覆盖 helpShell 段） |
| FX-75-7（S3） | `DSH_BROWSER` 无效值静默回落：`.find(existsSync)` 使显式设错路径仍用硬编码 Chrome（A2 实测 exit 0） | **修**：`DSH_BROWSER` **被显式设置**但不存在 → `console.error` ＋ `process.exit(1)` | 证据脚本浏览器选择块；变异探针 **P1**（无变异，仅设错 env → exit 1） |
| FX-75-8（S2） | 视觉证据无独立锚点：A1 变异 `dist/spec/style.js` 的 `--blue` → `#123456` 时 `style.test.mjs` 变红但 **visual 脚本 exit 0**（`H-01a` 拿 `CSS_VAR_TOKENS['--blue']` 与自己比） | **修**：视觉脚本对契约 doc:276-288 的 `:root` CSS 块做**外部 oracle 逐字断言**（不引用被测量常量） | 证据脚本 `H-01c`／`H-01d`／`H-01e`；变异 **M11**（改 `--blue` → 视觉脚本红） |
| FX-75-9（S2） | `extraCss` 无强制：`buildStyleSheet({extraCss:':root{--blue:#ff0000}'})` 不抛错，违反 doc:299-300 三条「不得」 | **修**：D3 修订 → 三禁强制（见上表 D3 行） | `src/style.ts` `assertExtraCss()`；`style.test.mjs` **T24**；变异 **M13**（删守卫调用 → T24 红） |
| FX-75-10（S3） | 契约 doc:295「随 #75 删除」未改口 | **修**：doc:295 补**落点注记**（本票不执行 → #96 门 → #108–#113 真迁移），台账 D2 行同步 | 契约 `:295`（注记）＋本节 D2 行；改后 `contract-signatures.test.mjs` 仍全绿（标记区表格／章节标题／token 逐值断言均未触碰） |

#### 8.11.2 #75 第二轮返修台账（R1 FAIL 82／R2 PASS 84 → 编排者裁定再返修）

第二轮对抗式审查（R1 FAIL 82／R2 PASS 84，均分 83 < 85）→ **再返修**。R1 在返修中**发现新引入的用户可见缺陷**（FX-75-11），本轮按**根因修**处置。逐条（**禁止默默略过**）：

| 编号 | 缺陷（审查实测锚点） | 处置 | 落点／证据 |
|---|---|---|---|
| FX-75-11（**S2**·新引入） | 运行时 toast **关闭按钮被挤到第 3 行**：R1 实测（headless 1440×900，逐字复刻 helpers DOM）`标题 top=677／详情 top=709／关闭 top=737`、关闭 `left=580` 独占第三行、容器高 **113px**（去掉两条权宜声明后为 `737/737/737` 同行、高 53px）。根因：helpers 运行时 DOM（`src/controls.ts` 的 `feedback()`）**缺** `.ilife-toast-body`／`.ilife-toast-title-row` 包裹，上一轮只加了 CSS 权宜补丁（`.ilife-toast{flex-wrap:wrap}` ＋ `.ilife-toast-title-detail{flex:1 1 100%}`） | **修（根因，授权改 `controls.ts`）**：helpers 运行时 DOM 与静态产出器／旧层**结构对齐**——`icon? + .toast-body(> .toast-title-row(> title) ＋ 可选 .toast-title-detail) + .toast-close`（旧层 `.hm-toast-icon + .hm-toast-body(> .hm-toast-title-row + .hm-toast-detail) + .hm-toast-close`，`.scratch/t76/old-controls.md` §1.1）；类名常量 `TOAST_BODY_CLASS`／`TOAST_TITLE_ROW_CLASS`／`TOAST_DETAIL_CLASS` 提到模块级、**静态产出器与 helpers 共用**；**删除**全部权宜补丁（`flex-wrap`／`flex:1 1 100%`／`body{flex:1 1 0%}` → `flex:1 1 auto`）；**删除** `style.test.mjs` 那条 `!helpers.includes('ilife-toast-body')` 断言（它把补丁锁死、禁止结构修复）。**未**破坏 #76 冻结产出内容契约：`SHARED_HELPERS_JS_RULE`（§3.3／FX-18）只约束自包含／幂等／DOM 允许／禁全局赋值／禁 `node:`，**不约束 DOM 结构**；`contract-signatures.test.mjs` 47/47 仍全绿 | `src/controls.ts`（`feedback()` ＋ 三个类名常量）；`src/style.ts` toast 区；`style.test.mjs` **T23**；浏览器实测 **`H-12c3`（`rt_close_top === rt_title_top`）**／`H-12c`／`H-12d`／`H-12e`／`H-12g`；变异 **M18**（破坏 body 类名 → `H-12d` 红） |
| FX-75-12（S2） | `statusBadge` 颜色**溯源不实**：`style.ts` 注释称「沿用旧 `.hm-status.{ok,warn,danger,empty}`」，实测 3/4 组字色不同（ok `#1f8c3d→#1f8f3d`、warn `#a25b00→#b25000`、danger `#a83228→#c0392b`），且 `#1f8f3d`／`#b25000` **全仓无出处**；底色由实色改 12% alpha | **修（parity 为准）**：改回旧逐值**实色**——ok `#e6f7ec`／`#1f8c3d`、warn `#fff5e0`／`#a25b00`、danger `#fff0ee`／`#a83228`、empty `#f0f0f3`／`var(--fg2)`（解析值 `#6e6e73`＝旧 `base.css:228`；旧值 oracle：`公共组件/assets/base.css:225-228`，旧值清单亦见施工单 B `:253`）；注释改为与实现一致 | `src/style.ts` statusBadge 区；`style.test.mjs` **T25**（外部 oracle 表）；浏览器 `B-12d`／`B-12d2`／`B-12d3`／`B-12d4`；变异 **M17** |
| FX-75-13（S2） | H-16 `copied` **变绿缺失且零记账**：产出 CSS／dist 中 `copied` 命中 **0**；`docs/visual-spec-help.md:193,195,197` 要求「按钮变绿 `copied` ＋ 450ms 弹簧 ＋ toast 双反馈」，施工单 B §4.1:280 落点写明「#75（CSS 部分）＋ `buildSharedHelpersJs`」 | **修（本票只补 CSS 侧）**：新增 `.ilife-copy-btn.copied{border-color:var(--ok);background:var(--ok);color:var(--card)}`；弹簧（`transition: transform .45s cubic-bezier(.34,1.56,.64,1)`）已在基座，`H-16a` 实测 `0.45s`。**移交登记**：运行时给按钮加 `copied` 类由 helpers 负责（**本票不改** `buildSharedHelpersJs` 的复制反馈行为）→ 落点 **#121《复制按钮 copied 态：运行时加类》（已建，阻塞 #88）** | `src/style.ts` copyButton 区；`style.test.mjs` **T27**；浏览器 **`B-12m`**（`copied` 态 computed = `rgb(52, 199, 89)`）／`H-16a`；变异 **M15** |
| FX-75-14（S2） | toast **入场动效完全缺失**：旧 `.hm-toast{transform:scale(.9);opacity:0}` ＋ `.hm-toast.show{…}`（`base.js:75`）；新产出无任何 `@keyframes`／transition。而新控制器**不加 `.show`**（`ToastController.show` 是方法名）→ 旧机制无触发点 | **修（CSS-only）**：`@keyframes <prefix>toast-in{from{opacity:0;transform:scale(.9) translateY(4px)} to{opacity:1;transform:none}}` ＋ `.ilife-toast{animation:… .22s cubic-bezier(.34,1.56,.64,1) both}`，**不依赖 JS 加类**（helpers 产出内 `classList`／`"show"` 命中 0，T28 断言）；`@media (prefers-reduced-motion: reduce)` 下 `animation: none` **归零** | `src/style.ts` toast 区；`style.test.mjs` **T28**；浏览器 **`H-21a`／`H-21b`／`H-21c`／`H-21d`／`H-21e`／`H-21f`**；变异 **M14**（删 animation → `H-21a` 红）／**M19**（删 reduced-motion 归零 → `H-21c` 红） |
| FX-75-15（S3） | `.ilife-error-actions` **缺旧 `.hm-actions` 的 `max-width:520px;margin:0 auto`**（`base.css:87-89`，v1.7 专修「被全宽容器拉伸」）：实测 retry `1352`／ghost `672`，旧应为 `520`／`256`；而 `.ilife-action-bar` 有 520px。旧 `.hm-error .hm-actions + .hm-actions{margin-top:14px}` 的新实现是 `gap:8px` | **修**：补 `max-width:520px` ＋ `margin-left/right:auto`；`gap:8px` → `row-gap:14px;column-gap:8px`（逐值对齐旧层两行间距 14px）；FX-75-5 的引证补 `base.css:87-89` | `src/style.ts` errorReceipt 区；`style.test.mjs` **T26**；浏览器 **`B-12j`**（`max-width=520px`／容器 520／retry 520／ghost 256／row-gap 14px／居中偏移 ≤2px）；变异 **M16** |
| FX-75-16（S3） | `assertExtraCss` **假阳性**：`/:root(?![\w-])/i` 会拦下**注释里**出现 `:root` 的合法覆盖块；`includes(forbidden)` 会拦下 `--pinkish` 这类名字（R1 实测均抛） | **修**：判定前先 `stripCssComments()`（注释不参与判定，也**不能**藏住真实声明）；禁入 token 改**边界匹配** `(^|[^A-Za-z0-9_-])--pink(?![A-Za-z0-9_-])`；`--r-xlarge` 同口径 | `src/style.ts` `stripCssComments()`／`hasBoundedToken()`；`style.test.mjs` **T24** ④⑤（注释 `:root` 通过／`--pinkish` 通过／`--r-xlarge` 通过／`/*c*/:root{}` 仍拦／注释＋真实 `--pink` 仍拦／`--pink-2` 与 `--pink` 同块仍拦）；本节未定口径⑤同步记「误拦＋漏拦」两面 |
| FX-75-17（S3） | 变异脚本**判据弱**：`t75-mutation-evidence.mjs` 用 `out.includes(name)` 判命中，node 报告里 `✔ <同名>` 也会命中（判据退化为「文件里存在该名字」） | **修**：只从**失败行**取名字——`node --test` 只看 `✖`（含 TAP `not ok`）行；视觉脚本只看 `**FAIL**` 表行且名字取自该行第 1 列；无变异探针 P1 走 `expectMode:'message'` | `docs/research/t75-mutation-evidence.mjs` `matchedExpect()` |
| FX-75-18（S3） | 窄屏判据**未真跑 375px**：headless `--window-size=375,812` 实测 `innerWidth=526`（1440→1414）→ `≤400px` 断点从未覆盖；`H-12e` 阈值过弱（≥40）；`B-12j`／`B-12k` 判别力不足 | **修**：窄屏改**iframe 宽 375**（同一张控制页 `srcdoc` 承载，`H-12f` 自证内层 `innerWidth===375`）；`H-12e` 升级为**结构判据**（`toast_h − body_h ≤ 28px` ⇒ 关闭按钮不另占行）＋ 区间 `[50,90]px`；`B-12j` 与 `B-12k` **合并**为一条逐值判据（520／520／256 ＋ row-gap 14px ＋ 居中偏移） | `docs/research/t75-visual-evidence.mjs` `narrowPage`／`H-12f`／`H-12g`／`H-12h`／`H-12e`／`B-12j` |

**门禁实测（#75 返修后 · 2026-09-09）**：`pnpm build` 退出 0／`pnpm boundaries` 退出 0（PASS）／`pnpm snapshot:check` 退出 0（`0.1.0@2fc0b42170d9604a` 未变）／`pnpm publish:pre` 退出 0（PASS）／`contract-signatures.test.mjs` **47**＋`style.test.mjs` **25**＋`charts.test.mjs` **87** = **159/159 fail 0**／`pnpm test` **新增 leaf 失败 = 0**（tests 871／suites 115／pass 846／fail 25；判据 = 失败用例名集合逐名比对：baseline 21 条 leaf 名，NEW 的 7 条为父级 suite 行（baseline 口径本就不收）、1 条为**并发在飞票 #87**（`#87 ① 字段清洗逐字复刻`，不计入 #75）、GONE 0）／发布面真打 tarball **22/22**（css=17086 B，含发布产物上的 extraCss 三禁）／变异自证 **15/15**（还原后重跑 fail=0）／浏览器 computed **56/56**（含运行时 toast 分层 ①、errorReceipt 两行 grid ⑤、严格圆角集 ②、外部 oracle ⑧）／`SPEC_FROZEN_SURFACE` **130 条**（implemented **130**／pending **0**；runtime 88／type 42，**零新增导出**）。

**门禁实测（#75 第二轮返修后 · 2026-09-09）**：`pnpm build` 退出 0／`node tooling/check-boundaries.mjs` 退出 0（PASS）／`node tooling/write-snapshot.mjs --check` 退出 0／`pnpm publish:pre` 退出 0（PASS）／`contract-signatures.test.mjs` **47**＋`style.test.mjs` **29**（T23 重写 ＋ 新增 T25–T28）＋`charts.test.mjs` **87**＋`controls.test.mjs` **64** = **227/227 fail 0**／发布面真打 tarball **25/25**（css=**17452 B**，新增发布产物上的 W2／W3／W4 三条事实）／变异自证 **21/21**（M2b 改「错误文案」判据、M9 改为「重新加回 `flex-wrap` 权宜补丁」、新增 M14–M19；**命中只从失败行取**，还原后自证 sha256 相同且重跑 fail=0）／浏览器 computed **71/71**（W1 `rt_close_top === rt_title_top`＝675／675、`rt_toast_h=58px`（`rt_body_h=32px`，差值 26 ≤ 30）、窄屏 **iframe 375** `rt_close_top=743 === rt_title_top=743`／`rt_right=331 ≤ 375`、W5 `520/520/256`＋`row-gap:14px`＋居中偏移 0px、W2 四态逐值、W3 `copied`＝`rgb(52,199,89)`、W4 `animationName=ilife-toast-in`／reduced-motion `none`／无动画 opacity 1）／`SPEC_FROZEN_SURFACE` **130 条**（implemented **130**／pending **0**；runtime 88／type 42，**零新增导出**）。

**门禁实测（#75 落地后 · 返修前基线）**：`pnpm build` 退出 0／`pnpm boundaries` 退出 0（PASS）／`pnpm snapshot:check` 退出 0／`pnpm publish:pre` 退出 0（PASS）／签名测试 `contract-signatures.test.mjs` **47 用例全绿**／`charts.test.mjs` **87 用例全绿**（`chartsCss` 加 `export` 未破坏 #78）／`style.test.mjs` **23 用例全绿**／`pnpm test` **新增失败 = 0**（判据 = 失败用例名集合逐名比对：tests 819→842／pass 795→818／fail **24→24**／`✖` 名字集 28→28，NEW 与 GONE 均为空集）／发布面真打 tarball **19/19**／变异自证 **6/6**／浏览器 computed **42/42**／`SPEC_FROZEN_SURFACE` **130 条**（implemented **130**／pending **0**；runtime 88／type 42）。

**t-chartfix 修正记账（2026-09-09，编排者裁定）**：桌面端 `.ilife-charts-bar .ilife-charts-xlabel` 字号 `10.5px`→`9.5px`（与移动端同选择器同值，恢复轴标签 9.5＜数值 10 的层级；契约正文零命中 `10.5px`，系测试快照值修正，**非冻结面变更**，`SPEC_FROZEN_SURFACE` 仍 **130 条**／implemented **130**／pending **0**）。落点：`src/charts.ts`（CSS 字面量一处）＋ `test/charts.test.mjs`（`H.图表文本字号` 同步＋层级注释）。
