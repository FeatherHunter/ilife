...
| FX-75-12（S2） | `statusBadge` 颜色**溯源不实**：`style.ts` 注释称「沿用旧 `.hm-status.{ok,warn,danger,empty}`」，实测 3/4 组字色不同（ok `#1f8c3d→#1f8f3d`、warn `#a25b00→#b25000`、danger `#a83228→#c0392b`），且 `#1f8f3d`／`#b25000` **全仓无出处**；底色由实色改 12% alpha | **修（parity 为准）**：改回旧逐值**实色**——ok `#e6f7ec`／`#1f8c3d`、warn `#fff5e0`／`#a25b00`、danger `#fff0ee`／`#a83228`、empty `#f0f0f3`／`var(--fg3)`（旧值 oracle：`公共组件/assets/base.css:225-228`，旧值清单亦见施工单 B `:253`）；注释改为与实现一致 | `src/style.ts` statusBadge 区；`style.test.mjs` **T25**（外部 oracle 表）；浏览器 `B-12d`／`B-12d2`／`B-12d3`／`B-12d4`；变异 **M17** |
| FX-75-13（S2） | H-16 `copied` **变绿缺失且零记账**：产出 CSS／dist 中 `copied` 命中 **0**；`docs/visual-spec-help.md:193,195,197` 要求「按钮变绿 `copied` ＋ 450ms 弹簧 ＋ toast 双反馈」，施工单 B §4.1:280 落点写明「#75（CSS 部分）＋ `buildSharedHelpersJs`」 | **修（本票只补 CSS 侧）**：新增 `.ilife-copy-btn.copied{border-color:var(--ok);background:var(--ok);color:var(--card)}`；弹簧（`transition: transform .45s cubic-bezier(.34,1.56,.64,1)`）已在基座，`H-16a` 实测 `0.45s`。**移交登记**：运行时给按钮加 `copied` 类由 helpers 负责（**本票不改** `buildSharedHelpersJs` 的复制反馈行为）→ 落点 **#88／#91 或另开票** | `src/style.ts` copyButton 区；`style.test.mjs` **T27**；浏览器 **`B-12m`**（`copied` 态 computed = `rgb(52, 199, 89)`）／`H-16a`；变异 **M15** |
