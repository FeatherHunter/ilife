...
> 状态：**四条决策已裁决**（见 `t195-decisions-record.md`）；票 #195 已据此关票，票 5 #188 与票 6 #189 开工。**另有两条后续更正**（见 `t187-decision.md` 第十一条与票面第五节第 11 条）：徽章类型词以模板 10 词表为准；combo 要登记。
> 事实包见 `docs/skills/skill-home/t195-facts/` 与 `t187-facts.md`／`t188-facts-*.md`，审查记录见 `review-t195-A.md`／`review-t195-B.md`。

## 零、决策摘要（细节与理由见 `t195-decisions-record.md`＋`t187-decision.md`）

| 序 | 决策 | 结论 | 依据 |
|---|---|---|---|
| 1 | 内容资产形态 | 机器生成的 typed `.ts` ＋ 生成器（含 `--check`）＋ 摘要锁；生成物禁手改；**锁必须接进包内门**；生成器输入源必须在仓内 | 用户裁决「照卡路里、饼干记账的做法」，两家一致 |
| 2 | 一级分组英文标识 | 老骨架 9 个域 key，`link` 只留登记位不入组 | 编排方第一性原理裁定：词表必须能完整切分 9 个分组 |
| 3 | 技能级 HELP 件落点 | 留在 `src/help/`（不搬） | 用户裁决 A |
| 4 | 速查与 HELP 的事实源 | **各留一份**，加一条双向对账锁 | 用户裁决「照卡路里、饼干记账的做法」，两家一致 |
| 5 | 命名落盘管线归属 | 居家自持只留**落点值**，命名通式／独占写／递补／回执全走共用件 `base-paint/save-html` | 票 #187 裁决（`#237` 已收拢，账单／卡路里／大厨／作息／备忘五家都在用） |
| 6 | 缺省出口口径 | **缺省＝HELP 文件**（回执追加 `delivery`）；速查支走 `mode:"lookup"` 且与 HELP 分名 | 票 #187 裁决（地图 Destination 已定；照 #139／#144 判法） |

## 一、总览：目录树与票面四问作答

依据：只读 part1＝`t195-part1-new-files.md`、part2＝`t195-part2-existing-and-gates.md`；本件只给形状与判据，不写生产代码。

件数：新增 **7 件**＝`src/render/helpPaths.ts`、`src/render/helpFile.ts`、`src/output.ts`、`src/help/<名>.ts`、内容资产生成物、`scripts/gen-help-assets.mjs`、`test/help-assets.test.mjs`（测试件里另两件按改动办，计入下句「改动」账）／改动 **8 件**（`package.json`／`SKILL.md`／`build-help.mjs`／`help.html`／`src/index.ts`／`src/render/index.ts`／`src/help/index.ts`／`cmd_read.ts`）＋仓根 `tooling/check-boundaries.mjs` 1 件。
...
src/help/ ← 既有目录，本票落位所在：index.ts [改动·加一行转发]／lookup.ts [不动·保持 HelpHit、lookupHelp 形状]（part2 §1.5）／help<名>.ts [新增·落文件＋回执绝对路径，名待定]（part2 §6.3）
src/render/ ← 既有目录，已有 6 件：index.ts／errors.ts／envelope.ts／html.ts／templates.ts／views.ts（见 §2）：helpPaths.ts [新增·命名通式，零 IO]／helpFile.ts [新增·全量 HELP JSON → 全页 HTML]（part1 §1）／index.ts [改动·原报新增，既有 barrel 上加行，§2 裁定 1]
src/output.ts            [新增] HTML 唯一落盘点 deliverHtml（part1 §1）
src/cli/cmd_read.ts      [改动] 只挪 home.help.lookup 一支（part2 §1.1）
内容资产生成物（居家路径待定）[新增] 生成物、禁手改词（part1 §1、§4.2）
...
## 2. 撞名／撞目录核对（实地列盘上路径；**2 处真冲突**）
1. **`src/render/index.ts` 真冲突**：part1 §1 列「新增」，盘上已存在且是完整 barrel（7 条 export 语句，实测名单：`HomeRenderError`／envelope **4 名**＝`HOME_KEY_SHAPES`／`homeShapeFor`／`buildHomeEnvelope`／`parseHomeEnvelope`／views 14 函数＋类型 `ItemCard`／`HelpItem`／html 10 名／templates 3 名 `HOME_TEMPLATES`／`templateFor`／`loadTemplate`＋类型 `HomeTemplate`；逐条读 `src/render/index.ts`，7 条语句说的是 6 个来源件、不是 7 个数）。裁定：**按改动办**，在既有 barrel 上加行、不换路径——新建同名＝覆盖既有出口，会铲掉 envelope／templates／views 对外名，破 `render.test.mjs` 与包外转发。
...
4. 无冲突：测试三件与既有五件不重名；`src/output.ts` 盘上无；`src/help/`、`src/render/` 目录已存在→除内容资产生成物路径待定（§1:17）外，其余新增件都不建新目录，全在既有目录内加文件。

## 3. 票面四问逐条作答
**问 1 · 新增哪些目录／文件、各给什么、有无共用件**
- 目录：除内容资产生成物的居家路径待定（§1:17、§5 第 2 条）外，其余新增件都在既有目录内加文件、不建新目录。对外：`helpPaths.ts` **2 个常量**（part1 §1、§2 第 1 行）；`helpFile.ts` **21 个**＝6 常量＋8 接口＋7 函数（part1 §2 第 3 行）；`src/render/index.ts` 在既有 barrel 上加行（§2 裁定 1）；`output.ts` **3 个**＝`deliverHtml`＋两再导出类型；`src/help/<名>.ts` **1 个**落文件函数（part2 §6.3）。
- **同能力内部复用两处，不是规则里的共用件**：`helpPaths.ts` 被同包 `helpFile.ts` 与 `output.ts` 用（命名通式唯一处）；`output.ts` 被 `home.help.lookup` 的「速查」与「落 help 文件」两路用（签名逐字照 bill，part1 §2 第 2 行）。判据：`docs/agents/structure.md:67` 的共用件要「写得出**哪两个能力**在用」、且住与能力目录并列的共用位（`t195-facts/06-precedents.md:74/76`）；上述两处都在**同一能力内部**两路复用，既不满足「两个能力」、也不住共用位，两个触发条件（新建目录层级／碰三个以上能力）均不成立（`t195-facts/06-precedents.md:76`）→ **本票不新建共用件，无需共用件点头句**。
...
- 归票：`helpPaths.ts`→票 7 #190；`helpFile.ts`→票 6 #189；`output.ts`→票 7 #190；生成器＋内容资产＋资产锁→票 5 #188；`src/render/index.ts`→票 6 #189（part1 §1 逐行给）。**测试三件归属已写死**（照 part1 §1 第 15–17 行）：`test/help-delivery-*.test.mjs`→**票 7 #190**；`test/help-exit-*.test.mjs`→**票 7 #190**；`test/help-assets.test.mjs`→**票 5 #188**；**仅帮助件交叉待定**：`src/help/<名>.ts` 归票 6 还是票 7（part1 §5 末条）。
...
## 4. 必报五步进度
- 一 · 影响清单：**已完成**（part2 §1，本件 §1 收成树）。二 · 结构设计：**已产出，待用户点头**——本件只到「报清形状＋待用户点头」（`docs/agents/structure.md:78`「用户看过再动」）；**尚不算闭环**：内容资产生成物的居家路径未定（§1、§5 第 2 条），树里仍挂未定节点，闭环须等该路径定下来。
...
## 5. 本件定不下来的（交下一棒汇总成待用户点头的问题）
1. 新增件名单**最终修订**：§2 两条只裁到「按改动办／条件不成立」，划掉新增须用户点头。
2. 内容资产生成物的**居家路径与目录名**（不能照搬 `src/triggers/`，part1 §5 第 1 条）。
3. `LOOKUP_FILE_STEM` 取值、要不要分「速查」支（part1 §5 第 2 条）——属票 4。
4. HELP 五字段取值与首次使用横幅显隐口径（part1 §5 第 3 条）——无仓内依据。
5. `src/help/<名>.ts` 的**新文件名**（part2 §6.3）——本件只给到「住 `src/help/`」。
6. 徽章类型词是否都落在共享模板认的 5 个之内（part1 §5）；用表外词＝改共享模板，需另立票。
7. `references/scenarios.yaml` 是否已是居家现成事实源（part1 §5）——未核实，写生成器前必须确认。
...
| --- | --- | --- | --- |
| `src/render/helpPaths.ts` | 命名与落点通式，零 IO 纯常量、只出初候选 | `HELP_HTML_DIR_NAME`＝`'home_manager_html'`；`LOOKUP_FILE_STEM`（取值待票 4 定） | 票 7 #190 |
| `src/render/helpFile.ts` | 内容资产＋派生 → 全量 HELP JSON → 全页 HTML，零 IO、零落盘 | 照 bill 的 21 个导出名单（6 常量＋8 接口＋7 函数），值换居家：`HELP_FILE_STEM`／`HELP_FILE_SKILL_NAME`／`HELP_FILE_TITLE`／`HELP_FILE_VERSION`／`HELP_INIT_SCENE_ID`／`HELP_CONTACT`、`HelpFileData`／`HelpFileOptions`／`HelpMetaBlock`／`HelpInitBanner`／`HelpIndex`／`HelpIndexItem`／`HelpContact*`、`formatHelpMinute`／`deriveSummaryLine`／`buildMetaBlocks`／`buildInitBanner`／`buildHelpFileData`／`renderHelpFileHtml`／`buildHelpIndex` | 票 6 #189 |
| `src/render/index.ts` | 汇总出口 | 值／类型／`helpPaths` 两常量的再导出名单，留给票 6 定 | 票 6 #189 |
| `src/output.ts` | HTML 产物唯一落盘点（独占创建＋递补＋file 态交付） | `deliverHtml`｜`HtmlDelivery`｜`HtmlLanding`（后两者自 `base-paint/save-html` 再导出） | 票 7 #190 |
| 内容资产生成物（居家路径待定，**不搬** `src/triggers/`） | 生成物：全量 HELP 资产（域／组／场景／唤醒词／场景索引） | 导出名单留给你票 5 定（bill 那份给 `WAKE_GROUPS`／`SCENE_BY_ID`／`HELP_WAKE_WORDS`／`WAKE_ASSETS`） | 票 5 #188 |
| `scripts/gen-help-assets.mjs` | 由事实源生成上一件，含 `--check` 分支 | 无（脚本，不对外） | 票 5 #188 |
| `test/help-delivery-*.test.mjs` | 模块级锁：命名通式／初候选不建目录／同秒三写不被覆盖 | 无 | 票 7 #190 |
...
- 不是新增、但要同批改的既有件（同报）：`packages/skill-home/package.json`（依赖闭包加 `base-paint`）、`packages/skill-home/SKILL.md`（补「HELP 交付」节）、`packages/skill-home/src/cli/cmd_read.ts`（HELP 在开库之前分派）、`tooling/check-boundaries.mjs`（把 `skill-home` 移出 `SKILLS_BASE_FROZEN`）。

## 2. 照抄对应表（bill 件含真实导出名 → 居家对应件）

| bill 件（导出名） | 居家对应件 | 保留什么 | 必须改什么 |
| --- | --- | --- | --- |
| `packages/skill-bill/src/render/helpPaths.ts`（`HELP_HTML_DIR_NAME`／`LOOKUP_FILE_STEM`） | `src/render/helpPaths.ts` | 零 import、零函数、零类型的纯常量形状；时间戳通式口径「本地时区／零填充秒／同秒递补 `_2` 起」 | 两常量取值换居家口径（目录名／文件名主体）；速查支要不要分名看票 4 |
| `packages/skill-bill/src/output.ts`（`deliverHtml`／`HtmlDelivery`／`HtmlLanding`） | `src/output.ts` | 签名与形状逐字：`explicit` 优先且覆盖写、`target` 缺位即抛、递补只认 `EEXIST`、回执 `{ mode:'file', path, bytes }` 且 `path` 恒绝对 | 注释里的技能名与报错前缀（`[skill-bill]` → 居家）；`HtmlLanding`／`HtmlReceipt` 仍从 `base-paint/save-html` 取 |
| `packages/skill-bill/src/render/helpFile.ts`（21 导出：6 常量＋8 接口＋7 函数） | `src/render/helpFile.ts` | 全量 HELP JSON 形状（`renderHelpFileHtml` 直调 `renderHelpShellHtml`）；三块可选内容字段一律带着、显隐走 `hidden`；域数／场景数／版本计数一律派生；`now` 显式传入 | 六个常量取值（文件名主体住本件**不**住 `helpPaths.ts`）；`groups` 换成居家内容资产；`init_banner` 的 `prompt` 来源换居家场景 |
| `packages/skill-bill/src/render/index.ts` | `src/render/index.ts` | 汇总出口这一层的做法（值／类型／常量再导出） | 名单随居家件，具体留给票 6 定 |
...
- `gen-wake-assets.mjs` 的读取段：事实源从老实物 HTML 的 payload 换成 `references/scenarios.yaml`，读取段要重写。
...
### 4.2 内容资产生成物：bill 侧 `packages/skill-bill/src/triggers/wake-assets.ts`（986 行）→ 居家第 5 件同性质
