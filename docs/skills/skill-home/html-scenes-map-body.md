...
**frontier（0 张）**：票 20（#817 收口：端到端＋双端墙＋链路总览＋逐页视觉复核＋综合分 ≥90＋维护者终审）——它被 11 张域票阻塞，域票已全关；**2026-09-22 起另被图外票 #889（照片落点落地）阻塞**：那张票要改 `items` 域的 `photos`／`photo_wall` 两族页面，不先落地，收口墙与逐页复核会签在即将变动的页面上。故当刻**无可取票**；#889 关闭后 frontier 恢复为票 20（原生依赖边已建，见 tracker）。
