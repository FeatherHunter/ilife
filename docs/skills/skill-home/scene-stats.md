...
**定稿批与上一批的等价性核对**（对抗式自查：整治「改了源码没重出产物」这类漂移）：
`.scratch/811/` 的四份产物以**定稿源码**重出为 `20260923_083322` 批，与改前那批逐份比对——
`body` 起**长度完全相同**，差异只有两处、都不在本票写集里：
① head 里那份共享样式表多了一条 `.fam-head` 规则（另一席在途改 `src/render/**` 与共享层，未提交）；
② 高频排行的 `access_count`（160→163、104→107）——`.scratch/home-seed` 是共享的种子库，
期间有别的席跑过 `home.item.detail`（唯一会 `access_count+1` 的命令），种子数据动了。
⇒ 本票的页装配与载荷在改动前后**等价**（同一份数据进去出同一份页）。

四门判据在**定稿批**上复跑：分隔符 `RESULT: 4/4 PASS`、结构块 `4/4 PASS`（24/26/29/23）、
双端 `RESULT: 4/4 PASS`（溢出 0／最窄 44px／藏匿 0）、墙自检 `4 格；链接 13 条；缺失 0 -> 可发`。
