#!/usr/bin/env node
/** 居家管家场景页视觉验收墙生成器（入仓件 · 无依赖 · 票 7）。
 *
 * 口径出处：`docs/agents/视觉验收墙.md`（§3 收口七步／§4 完成判据／§5 证据件落哪／
 * §6.2 骨架／§6.3 四条形制／§7 坑）。照卡路里 `docs/skills/skill-calorie/scene02-验收墙/gen-wall.mjs`
 * 逐字抄骨架（缩放算式、列数规则、`dropped`／`dead` 一起判、不加 `loading="lazy"`、
 * 清单不带 BOM），按居家重写三处：①标题与分组不写死任何域（域与清单都来自参数）；
 * ②补字节校验（清单行给 `bytes` 即比对盘上大小，不等即缺件，抄 `t532-清单.mjs:123-126` 的主意）；
 * ③补禁 `loading="lazy"` 的正则断言（抄 `t532-wall.mjs:133-136` 的主意）；
 * ④留出别名位（清单行给 `aliases` 数组即在格内作宿主场景的别名附注，不占新格，
 * 口径见票 22「唤醒词层规格」④：变体与无场景词不占新格）。
 *
 * 用法（四模式）：
 *   node gen-scene-wall.mjs <产物目录> <输出名> [宽] [高]   出墙：出一张墙 ＋ 重出索引 ＋ 自检
 *     node gen-scene-wall.mjs . 手机墙-390.html 390 820      # 手机墙：390 宽 × 3 列
 *     node gen-scene-wall.mjs . 桌面墙-1280.html 1280 860    # 桌面墙：1280 宽 × 1 列
 *   缺省＝`.`／`手机墙-390.html`／`390`／`820`。列数＝宽 ≤500 取 3、否则取 1（仓规 §6.3-2）。
 *   宽 >500 时**整体缩显示**：`SCALE = min(0.5, 600 / 宽)`，外面套 `overflow:hidden` 的定尺寸框、
 *   里面 iframe 用 `transform:scale()` —— **缩的是显示不是视口**，格子里的媒体查询仍按 `宽` 生效
 *   （仓规 §6.3-3）。
 *   node gen-scene-wall.mjs --stage <源目录> <产物目录>    造册：按清单 `file` 把产物复制进同目录，
 *     再出双墙 ＋ 索引 ＋ 自检（双端成对：手机 390 ＋ 桌面 1280）
 *   node gen-scene-wall.mjs --index <产物目录>             出索引：只重出总索引页
 *   node gen-scene-wall.mjs --check <产物目录> [墙文件名]   只跑自检（**反例测试就是这条**，
 *     改清单即红；给第二个参数即连那张墙一起查）
 *
 * 命名规则（全仓只此一处算）：发布名 ＝ 清单 `manifest.json` 的 `file` 字段；复制、墙、索引、
 * 自检都只读它，**别处不许再拼**（仓规 §6.1「同一个名字只在一处算出来」——两侧不一致＝全墙集体死链）。
 * 本生成器不自己算名字、也不裁命名规则（名字的唯一算法在票 2 契约里、由票 4 在产出侧实施），
 * 所以本票**不被命名裁决阻塞**。墙页、索引页与产物**必须同目录**（iframe 走相对路径）。
 *
 * 自检正反两面（仓规 §4）：
 *   正例 `node gen-scene-wall.mjs --check <产物目录> <墙文件>` → 打印「N 格；链接 M 条；缺失 0 -> 可发」**exit 0**；
 *   反例 把清单某行 `file` 改成盘上不存在的名字再跑 → **exit 1 且点名那一份**；改回再跑回 exit 0。
 *
 * 另外三处照做：**不加 `loading="lazy"`**（仓规 §7 第一坑：加了下半页格子永远空白；本件对出好的墙页做正则断言，
 * 命中即红）；清单**不带 BOM**（读到时剥并告警，请存不带签名的 UTF-8）。
 *
 * 清单契约（`manifest.json`，与链路总览页生成器共用同一份）：
 * 册子级：`batch`（批名，上屏用）／`naming`（命名说明）／`notShipped`（有意不出产物及其原因，
 * 数组 `{what, why}`）／`readings`（本批机器读数，原样上索引，生成器不自己算）／`rows`（必填，非空数组）。
 * 行级必填：`seq`／`wake`（唤醒词）／`file`（文件名，只从这里读）／`domain` 或 `kind`（域，二者至少其一；
 * 都给即都印）。行级可选：`title`（场景标题）／`prompt`（给链路页用）／`command`（命令，给链路页用；
 * `key`／`cli`／`cli_ran` 作同义备选）／`kind`（页面类型）／`family`（页面族，索引按它分组，
 * 缺则按域分组）／`check`（这一格该确认什么）／`aliases`（别名附注数组，不占新格）／
 * `bytes`（字节校验期望值，给即比对）／`src`（造册时源文件名，缺则与 `file` 同名）。
 * 清单字段若与 #183 已定的落点值冲突，当场补票，不在本脚本里私改（本件现状：无冲突——#183 定的是
 * HELP 文件的落点，场景页命名待票 2 契约，见说明文档）。
 */
import { copyFileSync, existsSync, mkdirSync, readFileSync, statSync, writeFileSync } from 'node:fs';
import { join, resolve } from 'node:path';

const MANIFEST = 'manifest.json';
const INDEX = '总索引.html';
const WALL_MOBILE = '手机墙-390.html';
const WALL_DESKTOP = '桌面墙-1280.html';

const esc = (s) => String(s).replace(/[&<>"]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c]));
const die = (code, msg) => { console.error(msg); process.exit(code); };

/** 清单是唯一权威：`rows[].file` 就是最终发布名。域与清单都来自参数，本件不写死任何域。 */
function loadManifest(dir) {
  const p = join(dir, MANIFEST);
  if (!existsSync(p)) die(2, `没有清单：${p}（清单须入仓，别让墙猜文件名）`);
  let raw = readFileSync(p, 'utf8');
  if (raw.charCodeAt(0) === 0xfeff) {
    console.error('注意：清单带 BOM（仓规 §7：读出来会 Unexpected token）——本次已剥，请存不带签名的 UTF-8');
    raw = raw.replace(/^\uFEFF/, '');
  }
  const mf = JSON.parse(raw);
  const rows = mf.rows;
  if (!Array.isArray(rows) || rows.length === 0) die(2, `${p} 的 rows 为空`);
  for (const r of rows) {
    if (r.seq === undefined || !r.wake || !r.file || (!r.domain && !r.kind)) {
      die(2, `清单第 ${r.seq ?? '?'} 行缺字段（至少 seq／wake／file／domain 或 kind）：${JSON.stringify(r)}`);
    }
    if (r.aliases !== undefined && !Array.isArray(r.aliases)) {
      die(2, `清单第 ${r.seq} 行 aliases 不是数组（别名不占新格，只作附注）：${JSON.stringify(r.aliases)}`);
    }
  }
  return {
    mf,
    rows,
    batch: typeof mf.batch === 'string' && mf.batch ? mf.batch : '居家管家',
    notShipped: Array.isArray(mf.notShipped) ? mf.notShipped : [],
    readings: mf.readings && typeof mf.readings === 'object' ? mf.readings : {},
  };
}

/** ① 造册：按清单 `file`（＝发布名，全仓只此一处算）把产物复制进**同一目录**（不是移动）。 */
function stage(srcDir, dstDir) {
  const { rows } = loadManifest(dstDir);
  mkdirSync(dstDir, { recursive: true });
  const miss = [];
  for (const r of rows) {
    const from = join(srcDir, r.src || r.file);
    if (!existsSync(from)) { miss.push(`${r.seq} ${r.wake} -> ${r.src || r.file}`); continue; }
    copyFileSync(from, join(dstDir, r.file));
  }
  if (miss.length) die(1, `源目录缺件，先重出产物再铺墙：\n  ${miss.join('\n  ')}`);
  console.log(`复制进仓：${rows.length} 件 -> ${dstDir}（源 ${srcDir}，源目录原样保留）`);
  return rows;
}

const STYLE = `*{margin:0;padding:0;box-sizing:border-box}
body{font-family:-apple-system,BlinkMacSystemFont,"PingFang SC","Microsoft YaHei",sans-serif;background:#f5f5f7;color:#1d1d1f}
.wrap{padding:24px 20px 60px}
h1{font-size:22px;font-weight:600;margin-bottom:6px}
.sub{color:#6e6e73;font-size:13.5px;margin-bottom:20px;line-height:1.7}
.sub b{color:#1d1d1f}
.grid{display:grid;grid-template-columns:repeat(COLS,BOXWpx);gap:18px;align-items:start;justify-content:start}
figure{background:#fff;border:1px solid #d2d2d7;border-radius:12px;overflow:hidden;width:BOXWpx}
figcaption{display:flex;justify-content:space-between;align-items:baseline;gap:8px;padding:8px 10px;font-size:12.5px;font-weight:600;border-bottom:1px solid #e8e8ed}
figcaption a{color:#007aff;text-decoration:none}
figcaption span{color:#86868b;font-weight:400;font-size:11.5px;white-space:nowrap}
.check{padding:7px 10px;font-size:11.5px;line-height:1.6;color:#3a3a3c;border-bottom:1px solid #e8e8ed;background:#fafafa}
.check b{color:#1d1d1f}
.shrink{overflow:hidden;position:relative}
iframe{display:block;border:0;background:#fff}`;

/** 每格＝真产物在**该宽度**下的真渲染（仓规 §6.3-1：不塞缩略图；§6.3-3：宽产物整体缩显示）。 */
function cell(r) {
  const domainKind = [r.domain, r.kind].filter(Boolean).join(' · ');
  const aliasLine = Array.isArray(r.aliases) && r.aliases.length
    ? `<br><b>别名：</b>${esc(r.aliases.join('、'))}（不占新格）`
    : '';
  return { domainKind, aliasLine };
}

function cellHtml(r, w, h, scale) {
  const boxW = Math.round(w * scale), boxH = Math.round(h * scale);
  const frame = scale === 1
    ? `    <iframe src="${esc(r.file)}" width="${w}" height="${h}" title="${esc(r.wake)}"></iframe>`
    : `    <div class="shrink" style="width:${boxW}px;height:${boxH}px">\n      <iframe src="${esc(r.file)}" width="${w}" height="${h}" style="transform:scale(${scale.toFixed(3)});transform-origin:0 0" title="${esc(r.wake)}"></iframe>\n    </div>`;
  const { domainKind, aliasLine } = cell(r);
  return `  <figure>
    <figcaption><a href="${esc(r.file)}" target="_blank" rel="noopener">${esc(String(r.seq) + ' ' + (r.title || r.wake))}</a>
      <span>${esc(domainKind)}</span></figcaption>
    <div class="check"><b>念这句：</b>${esc(r.wake)}${aliasLine}<br><b>这一格该确认什么：</b>${esc(r.check || '（清单未给 check）')}</div>
${frame}
  </figure>`;
}

/** ② 出墙：每格一件真产物，格子宽＝要量的那个视口，高一致。 */
function buildWall(dir, rows, out, w, h, batch) {
  const cols = w <= 500 ? 3 : 1;
  const scale = w <= 500 ? 1 : Math.min(0.5, 600 / w);
  const boxW = Math.round(w * scale);
  const cells = rows.map((r) => cellHtml(r, w, h, scale)).join('\n');
  const html = `<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(batch)}验收墙 ${esc(out)}（${rows.length} 格 × ${w} 宽）</title>
<style>
${STYLE.replace(/COLS/g, String(cols)).replace(/BOXW/g, String(boxW))}
</style></head><body><div class="wrap">
<h1>${esc(batch)}验收墙 · ${rows.length} 格 × ${w} 宽（格高 ${h}）</h1>
<div class="sub">每格是一份产物在 <b>${w} 宽</b>下的<b>真实渲染</b>（可交互、媒体查询按该宽生效）。点标题在新标签打开整页。
${scale === 1 ? '格子宽＝视口宽，1:1。' : `宽产物按 <b>${scale.toFixed(3)}</b> 整体缩显示（缩的是显示不是视口，视口仍是 ${w}）。`}
细看入口：<a href="${INDEX}">总索引</a>。这一页给人看，不是交付产物。</div>
<div class="grid">
${cells}
</div></div></body></html>
`;
  writeFileSync(join(dir, out), html, 'utf8');
  return { name: out, html };
}

/** ③ 出索引：按页面族（缺则按域）分组、每份一张卡，末尾列清「有意不出产物及其原因」（仓规 §3-4）。 */
function buildIndex(dir, rows, notShipped, readings, batch) {
  const groupOf = (r) => r.family || r.domain || r.kind || '未分组';
  const families = [...new Set(rows.map(groupOf))];
  const groups = families.map((f) => {
    const mine = rows.filter((r) => groupOf(r) === f);
    const cards = mine.map((r) => {
      const aliasLine = Array.isArray(r.aliases) && r.aliases.length
        ? `\n        <dt>别名</dt><dd>${esc(r.aliases.join('、'))}（不占新格）</dd>`
        : '';
      return `    <div class="card">
      <div class="card-head"><span class="seq">${r.seq}</span><a href="${esc(r.file)}">${esc(r.title || r.wake)}</a><span class="kind">${esc([r.domain, r.kind].filter(Boolean).join(' · '))}</span></div>
      <dl><dt>念这句</dt><dd>${esc(r.wake)}</dd>
        <dt>命令</dt><dd><code>${esc(r.command || r.key || r.cli || r.cli_ran || '（清单未给）')}</code></dd>
        <dt>文件</dt><dd><code>${esc(r.file)}</code></dd>
        <dt>该确认什么</dt><dd>${esc(r.check || '（清单未给）')}</dd>${aliasLine}</dl>
    </div>`;
    }).join('\n');
    return `  <h2>${esc(f)}（${mine.length} 件）</h2>
  <div class="cards">
${cards}
  </div>`;
  }).join('\n');

  const walls = [WALL_MOBILE, WALL_DESKTOP].filter((w) => existsSync(join(dir, w)));
  const wallLine = walls.length
    ? walls.map((w) => `<a href="${esc(w)}">${esc(w)}</a>`).join(' · ')
    : '（本次未出墙页）';
  const notShippedHtml = notShipped.length
    ? notShipped.map((n) => `      <li><b>${esc(n.what)}</b>：${esc(n.why)}</li>`).join('\n')
    : '      <li><b>清单未给 notShipped</b>：本页这一段不该为空，补进 manifest.json 再重跑。</li>';
  const readKeys = Object.keys(readings || {});
  const readingsHtml = readKeys.length
    ? readKeys.map((k) => `      <li><b>${esc(k)}</b>：${esc(readings[k])}</li>`).join('\n')
    : '      <li><b>清单未给 readings</b>：本页这一段不该为空，补进 manifest.json 再重跑。</li>';

  const html = `<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(batch)}验收总索引（${rows.length} 件）</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:-apple-system,BlinkMacSystemFont,"PingFang SC","Microsoft YaHei",sans-serif;background:#f5f5f7;color:#1d1d1f}
.wrap{padding:28px 22px 60px;max-width:1080px}
h1{font-size:24px;font-weight:600;margin-bottom:6px}
.sub{color:#6e6e73;font-size:13.5px;line-height:1.8;margin-bottom:22px}
.sub b{color:#1d1d1f}
h2{font-size:15px;font-weight:600;margin:26px 0 10px}
.cards{display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:14px}
.card{background:#fff;border:1px solid #d2d2d7;border-radius:12px;padding:13px 15px}
.card-head{display:flex;align-items:baseline;gap:8px;margin-bottom:9px}
.seq{background:#1d1d1f;color:#fff;border-radius:6px;font-size:11.5px;font-weight:600;padding:1px 7px}
.card-head a{color:#007aff;text-decoration:none;font-size:14px;font-weight:600}
.kind{margin-left:auto;color:#86868b;font-size:11.5px}
dl{display:grid;grid-template-columns:70px 1fr;gap:3px 8px;font-size:12.5px;line-height:1.6}
dt{color:#86868b}
dd{color:#1d1d1f}
code{font-size:11.5px;color:#6e6e73;word-break:break-all}
.note{margin-top:28px;background:#fff;border:1px solid #d2d2d7;border-radius:12px;padding:16px 18px}
.note h2{margin-top:0}
.note ul{margin:8px 0 0 20px;font-size:13px;line-height:1.9;color:#3a3a3c}
</style></head><body><div class="wrap">
<h1>${esc(batch)}验收总索引</h1>
<div class="sub">${rows.length} 件产物按页面族（缺则按域）分组，每份一张卡（念哪句话／命令／文件／该确认什么）。
手机墙 <b>390 宽 × 3 列</b>看塌列，桌面墙 <b>1280 宽 × 1 列</b>看排布；两张墙、本索引与产物**同目录**，
格数 ＝ 本索引卡的件数 ＝ 清单 <code>manifest.json</code> 的条目数（少任何一件，生成器自检点名并 exit 1）。
本页：${wallLine}</div>
<div class="note">
  <h2>本批机器读数（照单抄，生成器不自己算）</h2>
  <ul>
${readingsHtml}
  </ul>
</div>
${groups}
<div class="note">
  <h2>有意不出产物及其原因</h2>
  <ul>
${notShippedHtml}
  </ul>
</div>
</div></body></html>
`;
  writeFileSync(join(dir, INDEX), html, 'utf8');
  return { name: INDEX, html };
}

/** ④ 自检：`dropped`（清单点名却没有文件）与 `dead`（页上引用却落不到）**一起判**，另加字节校验与禁 lazy 断言。 */
function selfCheck(dir, rows, pages) {
  const dropped = rows.filter((r) => !r.file || !existsSync(join(dir, r.file)));
  const byteBad = [];
  for (const r of rows) {
    if (r.bytes === undefined || r.bytes === null) continue;
    const p = join(dir, r.file);
    if (!existsSync(p)) continue;
    let size = null;
    try { size = statSync(p).size; } catch { size = null; }
    if (size !== null && size !== Number(r.bytes)) {
      byteBad.push(`字节对不上：${r.seq} ${r.wake} -> ${r.file}（清单 ${r.bytes} B，盘上 ${size} B）`);
    }
  }
  const refs = [];
  const perPage = [];
  for (const p of pages) {
    const mine = [...p.html.matchAll(/(?:src|href)="([^"#]+\.html)"/g)].map((m) => m[1]);
    refs.push(...mine);
    perPage.push(`${p.name} ${mine.length}`);
  }
  const dead = [...new Set(refs)].filter((r) => !existsSync(join(dir, decodeURIComponent(r))));
  const lazyPages = pages.filter((p) => /<[a-z][a-z0-9-]*\b[^>]*\bloading\s*=\s*["']?lazy/i.test(p.html)).map((p) => p.name);
  const bad = [
    ...dropped.map((r) => `清单点名却没有文件：${r.seq} ${r.wake} -> ${r.file || '（file 字段缺失）'}`),
    ...byteBad,
    ...dead.map((d) => `页上引用却落不到：${d}`),
    ...lazyPages.map((n) => `墙页命中禁用的 loading="lazy"：${n}（仓规 §7 第一坑）`),
  ];
  const kept = rows.length - dropped.length;
  const clean = bad.length === 0;
  console.log(`${kept} 格；链接 ${refs.length} 条；缺失 ${bad.length} -> ${clean ? '可发' : '不可发'}`);
  if (perPage.length) console.log(`（${perPage.join(' ＋ ')}）`);
  if (!clean) {
    for (const b of bad) console.error(`  ${b}`);
    console.error(`不可发（产物目录 ${dir}）`);
  }
  process.exit(clean ? 0 : 1);
}

/** `--check` 模式：读盘上已出的墙与索引，把它们的引用也一起查。 */
function pagesOnDisk(dir, extra) {
  const names = [WALL_MOBILE, WALL_DESKTOP, INDEX];
  if (extra && !names.includes(extra)) names.push(extra);
  const pages = [];
  for (const n of names) {
    const p = join(dir, n);
    if (existsSync(p)) pages.push({ name: n, html: readFileSync(p, 'utf8') });
  }
  if (extra && !pages.some((p) => p.name === extra)) {
    console.error(`指定的墙文件不存在：${join(dir, extra)}`);
    process.exitCode = 1;
  }
  if (pages.length === 0) console.error('（盘上还没有墙页／索引：本次只判清单点名，未判页上引用）');
  return pages;
}

const argv = process.argv.slice(2);
const mode = argv[0] === '--stage' ? 'stage' : argv[0] === '--check' ? 'check' : argv[0] === '--index' ? 'index' : argv[0] === '--help' || argv[0] === '-h' ? 'help' : 'wall';
const argOf = (i, dflt) => (argv[i] === undefined || argv[i] === '' ? dflt : argv[i]);

if (mode === 'help') {
  console.log('用法：node gen-scene-wall.mjs <产物目录> <输出名> [宽] [高] ｜ --stage <源目录> <产物目录> ｜ --index <产物目录> ｜ --check <产物目录> [墙文件名]');
  process.exit(0);
} else if (mode === 'stage') {
  if (!argv[1] || !argv[2]) die(2, '用法：node gen-scene-wall.mjs --stage <源目录> <产物目录>');
  const src = resolve(argv[1]), dst = resolve(argv[2]);
  if (src === dst) die(2, '--stage 的源目录不能就是产物目录本身');
  const rows = stage(src, dst);
  const { batch, notShipped, readings } = loadManifest(dst);
  const pages = [
    buildWall(dst, rows, WALL_MOBILE, 390, 820, batch),
    buildWall(dst, rows, WALL_DESKTOP, 1280, 860, batch),
    buildIndex(dst, rows, notShipped, readings, batch),
  ];
  console.log(`墙与索引已重出：${join(dst, WALL_MOBILE)} / ${WALL_DESKTOP} / ${INDEX}`);
  selfCheck(dst, rows, pages);
} else if (mode === 'index') {
  const dir = resolve(argOf(1, '.'));
  const { rows, batch, notShipped, readings } = loadManifest(dir);
  const page = buildIndex(dir, rows, notShipped, readings, batch);
  console.log(`索引已重出：${join(dir, page.name)}（${rows.length} 件）`);
} else if (mode === 'check') {
  const dir = resolve(argOf(1, '.'));
  const extra = argv[2] && !argv[2].startsWith('-') ? argv[2] : undefined;
  const { rows } = loadManifest(dir);
  const pages = pagesOnDisk(dir, extra);
  if (process.exitCode === 1) {
    const kept = rows.filter((r) => r.file && existsSync(join(dir, r.file))).length;
    console.log(`${kept} 格；链接 0 条；缺失 1 -> 不可发`);
    console.error(`  指定的墙文件不存在：${extra}`);
    process.exit(1);
  }
  selfCheck(dir, rows, pages);
} else {
  const dir = resolve(argOf(0, '.'));
  const out = argOf(1, WALL_MOBILE);
  const w = Number(argOf(2, 390));
  const h = Number(argOf(3, 820));
  if (!Number.isFinite(w) || !Number.isFinite(h) || w <= 0 || h <= 0) die(2, `宽高必须是正数：${w}×${h}`);
  const { rows, batch, notShipped, readings } = loadManifest(dir);
  const wall = buildWall(dir, rows, out, w, h, batch);
  buildIndex(dir, rows, notShipped, readings, batch);
  console.log(`出墙：${join(dir, out)}（${rows.length} 格 × ${w} 宽${w <= 500 ? '' : '，按比例缩显示'}）`);
  selfCheck(dir, rows, [wall, ...pagesOnDisk(dir).filter((p) => p.name === INDEX)]);
}
