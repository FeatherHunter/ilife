...
在 `packages/skill-home/scripts/` 写种子脚本（幂等、可重跑、带 `--reset` 与 `--check`），灌出覆盖 70 条场景的**有真实感**数据：每场景 3–10 条，含照片路径、标签、位置树（含固定位）、保修与保养周期、证件与账号、借用记录、购物清单与阈值、盘点记录与差异、物品历史快照等；字段与册子（票 1）逐项对得上；库落 `.scratch/` 下的测试库，**绝不写** `SKILLS_DB_PATH` 指的生产目录（`D:\2Study\StudyNotes\.db`）。
