...
`node tooling/run-locked.mjs --ticket <本票号> -- node packages/skill-home/scripts/audit-separators.mjs <样例产物目录>` —— 正例（骨架页目录）**命中数 > 0 且读数与逐行清单打得出来**；反例（手工塞一处版式位 `·` 后跑 clean 目录）exit 1 并点名；两条读数写进 `docs/skills/skill-home/style-audit.md`。
