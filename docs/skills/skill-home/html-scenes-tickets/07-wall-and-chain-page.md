...
两份都要能拿一份**假清单**跑通（不依赖真页面先存在），页面与产物必须落**同一目录**（本机视觉工具只吃工作区内文件）；清单里的文件名**从清单里读**——生成器不自己算名字、也不裁命名规则（名字的唯一算法在票 2 契约里、由票 4 在产出侧落地），所以本票**不被命名裁决阻塞**。生成器还要**留出别名位**：按票 22「唤醒词层规格」的口径，变体与无场景词**不占新格**，只作宿主场景那一行／那一格的别名附注。
