...
| #175 | `改档案` **无档案时按裁定报错**（不再建行后更新） | **已做**（2026-09-12 第四棒，`e058f68`）：空库跑 `calorie.profile.update` → exit 4、不落盘、**不建行**；守卫住 `fetch/profile.ts:updateProfile`，两条写词分走 `setProfile`（设置档案＝单例 upsert）／`updateProfile`（改档案＝只改已有）；新测 `test/profile-guard-175.test.mjs`，`profile-doc-179.test.mjs` 的空库断言改为缺失阻断 | ✅ 已做 |
...
**结论**：三张票各自都还欠实打实的内容（#177 欠 6 个指标＋结论；#175 欠回执 2 项＝性别／推荐活动量，空库守卫已于 `e058f68` 补上；#176 欠回执 2 项），
...
## 七、第四棒（2026-09-12 下午）追加的裁定与交付

| # | 事项 | 结果 |
|---|---|---|
| 1 | **toast 弹窗样式** | 用户看过两版**实物对比页**后裁定取**老仓库**（深色毛玻璃）→ 已落地为票 **#246**：`packages/base-render/src/style.ts` 的 toast 区逐值换成老仓值（几何／寿命／容量／无障碍不动），账目 `docs/base/base-render/t246-toast-dark.md`；规格正本 §B-12 同步 |
| 2 | **#175 空库守卫**（第四棒第一件开发活） | 已做：改档案无档案 → exit 4、不落盘、不建行（`e058f68`）；§五 已回填 |
| 3 | **「设活动量」空库行为** | 用户授权「从第一性原理选最合适方案」→ 判定**加同一守卫**（活动量是档案的字段，「设」＝改已有档案；放开第二条创建路径会产出「只填活动量的半份档案」，下游 TDEE 一律算不出）→ 已做并立 5 条断言 |
| 4 | **「复制后 3 选 1」调查** | 用户记忆「点复制后出现 3 种形式 3 选 1」——**查实**：是**复制数据**（不是日志）在 2026-08-05～08-12 间的老实现——`卡路里/templates/crud_receipt.html` 弹 `.fmt-menu` 三选一（纯文本「粘贴给 AI／自己看」／JSON「结构化存档」／CSV「表格导入」），选中即复制并 toast 报所选格式；**2026-08-13 `2262fee1`「Base 重构 task ① · 72 业务模板接 Base 管线」把它整批换掉**，只剩一段死 CSS（该文件 `:97-101`）；老仓 Base 契约只把 `formatMenu` 写进文档（`component-contract.md:177`、`README:126`）**从未实现**。是否在新仓恢复＝**待用户裁**（08 规范 §3.4 把它列为能力项；新仓 `COPY_FORMATS`／`buildDataText(format)` 已冻结，落点现成） |

## 八、2026-09-13：#175 收口（用户亲口「全部通过」）

- **本票 100% ＋ close**（提交 e9d7e66）：设置档案 回执补 性别／推荐活动量（档位 ＋ 系数 ＋ TDEE 影响——只由页面按库内现值算，不进写命令参数）／设置时间；写前页两处配置改成**按写入词展开对应那一处**；顺手修掉「性别填中文「男」时五档 TDEE 按女性公式算」这条真缺陷（2301 卡 vs 正确 2559 卡）。
- **证据**：packages/skill-calorie/test/profile-receipt-175.test.mjs 5/5；包内 444 条 442 通过（两红＝#180 有意先红 ＋ etch-t6 的 history 那条日期依赖老红，已用 git stash 对照证明）；快照门 186 件 changed=0；边界门 PASS；示例门 100/100；UI 两关（390／1440）结论与必报五步对账都写进票面。
- **本文档的过期处**：§二 第 2 条与 §三 表里「#175／#176／#177 三张票仍 OPEN」应读作「**#176／#177 仍 OPEN**，其中 #175 已于 2026-09-13 收口」；§五 表第 70 行「❌ 只做 1/3」作废（回执 3 项已补）。§六 第 3 条是 2026-09-12 的当时裁定，留档不改。
- **用户验收动作**＝桌面 ilife-scene07\t175\ 四张页双击看过（档案预检-设置档案／档案预检-改档案／设置档案-回执／改档案-回执）＋ 390px／1440px 两档截图看过。
- 地图 #152 的 Decisions so far 已追加一行（两处配置的形态、抽到公共层的部分）。
