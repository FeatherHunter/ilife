/** T351-a2 接任补齐 · 场景 05 fitness 全量重跑：order176–207（结果页／过程页／写后回执）＋机检。
 *
 ...
 *
 * 与上一轮 `final-v3/` 的差别（T351-a2：六列 → 一排四列的行内版式）：
 *   ① 动作明细表改判四列（`动作／部位／组数×次数／重量`）：声明本页有动作明细表的，必须**实测到四列
 *      表头按序齐**；页内一张表都没有即红（旧口径 `expectTable:'other'` 在零表时也判绿，那是后门）。
 *   ② 新增三条：⑥ 动作格＝加粗名＋块级副行小字，副行里出现「主要／孤立」；⑦ 正文不出现 `main`／`iso`
 *      裸词（`data-t` 载荷不算正文，口径见 `bodyNoPayload`）；⑧ 节奏上移会话标题行（两面都判）。
 *   ③ 夹具的备注／类型改成生产库真形状（`胸整体 [W1 10reps×35.0kg, 20-30 RPM(2-2.5秒/次)]`／
 *      `main`／`iso`），边界三条（休息日／多组不同次数／空 sets）照旧。
 *
 * T351-a2 接任者改动的四处（v3 → v4）：
 *   ① **堵洞扩到全表**：声明细化为 `four`（动作明细表）／`own1`（本页自己的表，如 185 的四列对照表、
 *      过程／回执的改前改后表、复盘页统计表）／`none`（本页无表）三态，**每份都必须量到该有的表头**；
 *      再补一条「不许有产物落在兜底 `other`」，把「零表判绿」这条路整条封死。
 *   ② ⑧ 由「有动作表的页标题含节奏」改成**逐页真值两面判**：夹具里带节奏的场次（第 1 周上肢 20-30 RPM／
 *      下肢 18-25 RPM）必须在标题里逐字出现，不带节奏的场次（第 2 周背／腿，备注无方括号或为空）**不许**
 *      出现「节奏」——单向的「有表就得有节奏」在这套夹具上会把 177／181 误判成红（那两页的备注本就没有节奏）。
 *      另加：全量页 order184 恰两条带节奏标题、周级标题（`第 N 周 · M 场`）不许带节奏、休息日标题不许带。
 *   ③ ⑦ 的正文口径写明：`data-t` 载荷（复制按钮里那段要复制的文本）不算正文，另记 `bareTypeInPayload` 备查。
 *   ④ 新增 ⑨：两条无损删减的旁证——正文不出现「reps×」与「W<n> 」这类只在方括号内逗号前那截才有的记号。
 *   ⑤ 夹具补一条**生产库真形状**（`背 iso 主 [W2 5reps×42.5kg, 15-20 RPM(3-4秒/次)]`：细化词里混着与该行
 *      `type` 同值的裸词），⑧ 的期望改成逐页条数真值；⑥ 另断这条读作「背 主 · 孤立」——这条是 a2 在
 *      真数据页上实测出来的缺陷（细化词里的 `iso` 原本原样入正文），补进夹具防回潮。
 *
...
function inspect(name, html) {
  ...
  const allTitles = [...html.matchAll(/<summary[^>]*>([\s\S]*?)<\/summary>/g)]
    .map((m) => m[1].replace(/<[^>]*>/g, '').trim());
  const dayTitles = allTitles.filter((t) => /^周[一二三四五六日]/.test(t));
  /** 正文口径：属性里的载荷（`data-t` ＝复制按钮要复制的那段文本）不算正文，⑦ 只判真正文。 */
  const bodyNoPayload = body.replace(/data-t="[^"]*"/g, 'data-t=""');
  const tempoTexts = dayTitles.map((t) => (/· 节奏 (.+)$/.exec(t) ?? [, ''])[1]).filter((x) => x !== '');
  return {
    name,
    bytes: html.length,
    body,
    ...
    dayTitles,
    allTitles,
    tempoTitles: dayTitles.filter((t) => t.includes(' · 节奏 ')),
    tempoTexts,
    weekTitlesWithTempo: allTitles.filter((t) => /^第 \d+ 周/.test(t) && t.includes('节奏')),
...
check('机检⑧ 节奏上移会话标题行（两面）：逐页日级标题带节奏的条数＝夹具真值（177／181 各 1 场带、184 恰三条且逐字等于夹具原文），'
...
check('195 撤销状态不丢：回执页「写后现值」写「计划已撤销（配置＋会话均为空）」',
  raw195.includes('计划已撤销（配置＋会话均为空）'), '命中=' + raw195.includes('计划已撤销（配置＋会话均为空）'));
