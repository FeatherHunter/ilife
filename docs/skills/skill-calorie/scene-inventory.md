...
|---|---|
| 唤醒词总数（SoT） | 436 |
| ✅ 打通（拿标准种子库真跑出 HTML 样例） | **350** |
| ❌ 没打通（新仓没有单命令入口） | **76** |
| ⛔ 没开发（架构明确不做） | **10** |
| 唯一命令数 ／ 实跑命令数 | 261 ／ 261 |
| **HELP 速查台里挂的命令可执行** | **84 ／ 436** |
...
1. **「没打通」＝路由桶 `legacy-chain`（75 条）**：老技能当年是多步交互链（或压根没单命令），新仓还没接——与「命令坏了」不是一回事，逐条账目在 `docs/skills/skill-calorie/t180-command-gap-classification.md`。
2. **「没开发」＝路由桶 `out-of-scope`（10 条）**：拍营养表 2（并入 #180）＋ 落地训练 3／同步训记 2（#182）＋ 定时复盘 3（#182）。
3. **另一条独立缺口、数字更大且不在上面 95 条里**：**HELP 速查台里 352 条唤醒词挂的还是老脚本命令**（如 `python scripts/render_health_dashboard.py`），只有 84 条是可执行的 `calorie-cmd-read …`——这是 **#180 数据清理第二步没做**：路由层（AI 真走那条）已有 341 条可执行，HELP 呈现层却还回老命令。**用户看到的 HELP HTML 受影响最大的就是这一条。**
