...
本票按编排者单用途授权更新其中计数行＋注释（见§十五）；D2④逻辑、A7 名单、T71、快照重生成仍归主人与合流方。
...
## 十五、冻结计数单用途改数（编排者授权，已独立提交）

- 授权：只动 `test/calorie-routing-81.test.mjs` 计数行＋注释／标题；列式（本票 15 条点名＋他席差值归因）；主人异议即重改。
- 改数（dist 时点实测）：桶 390／46、汇总 exec390／nonExec46／oos8／legacy38／new68、覆盖面 390、总量 505、新拟 68（两处）、exec 桶 459、明确不做 8。列式：`#383 ＋15`（order398–412：预测体重自定义目标 1＋模拟减重 7＋摄入预测 7，逐条见§七）＋`#376 ＋1`（order353 看整体趋势含目标对比）＋他席在途约＋7（拍营养表×2 由明确不做转入 exec 等，余量系 body／weight 在途翻转，票号待主人认领）＋他席新键＋5。
- 同文件主人两处在途 hunk：229 行覆盖面计数已按授权并入改数（367→390）；331 行数据依赖登记（看里程碑回溯）未动一字，原样携带，commit 信息中点名。
- 未动：D2④逻辑、A7 名单、T71_DIFFS、`checked` 行、smoke 快照重生成（归主人与合流方）。
- 验证：`node --check` 过；套件重跑初遇 dist 中毒（他席把破损 scene-03-weight 编进 dist，`runId=b8489ba2` 加载失败，已自愈）→ 重跑后计数断言全过，仅剩非计数红（覆盖键 118 缺路由、D2④他席词、快照未重生成、A7 名单），归主人与合流方。日志 `.scratch/t383/routing81-r2.log`（中毒）／`routing81-r3.log`（计数过）。
- 提交：`098c5ed4`（单文件独立 commit）。
