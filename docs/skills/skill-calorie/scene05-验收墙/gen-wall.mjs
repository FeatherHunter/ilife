#!/usr/bin/env node
/** 场景05「健身计划 · 27 条唤醒词」视觉验收墙生成器（入仓件 · 无依赖）。
 *
 * 口径出处：仓规 `docs/agents/视觉验收墙.md`（§3 收口七步／§4 完成判据／§5 证据件落哪／
 * §6.2 骨架／§6.3 四条形制／§7 坑）。本件**照 `docs/skills/skill-calorie/scene01-验收墙/gen-wall.mjs`
 * 改**（不是从白纸写，见票 #351 票面），保留它已有的两处硬做法：
 *   - `dropped`（清单点名却没有文件）与 `dead`（页上引用却落不到）**一起判** —— 仓规 §6.2／§7
 *     点名的那处**假绿灯**（只查页上引用、缺件被静默剔掉，自检照样报「缺失 0」）在此堵掉；
 *   - **不加 `loading="lazy"`**（§7 第一坑：加了下半页格子永远空白）；清单**不带 BOM**（读到即告警）。
 *
 * 与 scene01 件的**唯一一处改动**（照票面「同一条词有过程页与回执页两份」的命名需要）：
 *   `stage()` 复制时读 `rows[].src`（本批产物在原批目录里的文件名，如 `order176-result.html`）。
 *   `src` 缺省＝`file`（scene01／scene06 的清单不带 `src`，行为与它们逐字相同）。
 *   **发布名恒是 `rows[].file`**，仍只在 `manifest.json` 一处算；`src` 只是「从哪个原名复制过来」。
 *
 * 用法（位置参数，照票面）：
 ...
 *   正例 `node gen-wall.mjs --check .` → 打印「N 格；链接 M 条；缺失 0 -> 可发」**exit 0**；
...
/** 清单是唯一权威：`rows[].file` 就是最终发布名（`rows[].src` ＝ 只是从哪个原名复制过来）。 */
 ...
 *  源名取 `rows[].src`（本批产物原名 `orderNNN-<card>.html`）；清单没给 `src` 的批次＝源名同发布名。 */
function stage(srcDir, dstDir) {
  ...
  for (const r of rows) {
    ...
    if (!existsSync(from)) { miss.push(`${r.seq} ${r.wake} -> ${r.src || r.file}（发布名 ${r.file}）`); continue; }
...
<h1>场景05 验收墙 · ${rows.length} 格 × ${w} 宽（格高 ${h}）</h1>
...
${cards}
  ...
  const html = `<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1"><title>场景05 验收总索引（${rows.length} 件）</title>
<style>
