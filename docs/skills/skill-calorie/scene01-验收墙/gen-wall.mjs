#!/usr/bin/env node
/** 场景01「主页族 ＋ 跨域概览 ＋ 目标进度」视觉验收墙生成器（入仓件 · 无依赖）。
 *
...
<h1>场景01 验收墙 · ${rows.length} 格 × ${w} 宽（格高 ${h}）</h1>
...
${cards}
  ...
  const html = `<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1"><title>场景01 验收总索引（${rows.length} 件）</title>
<style>
...
<h1>场景01「主页族 ＋ 跨域概览 ＋ 目标进度」验收总索引</h1>
