...
> **更正（2026-09-24 补）**：本报告表格「每个域页」那一行（及任何写「`<details class="subgroup" open>` ……默认展开」处）**已失效**——共享 help 模板自 2026-09-24 起改成**每个分组页只第一个二级分组默认展开**、其余折叠（用户裁定；A 路 `assets/help-template.html` 与 B 路 `src/help.ts:renderSubgroup` 同口径，回归用例 `packages/base-render/test/help-subgroup-open-default.test.mjs`）。行锚随之漂移，引用请按锚点字符串。规范面口径见 `docs/visual-spec-blocks.md` §B-08。
