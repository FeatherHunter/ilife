...
已完成：① 图的准备工作（建图 ＋ 15 张子票 ＋ 原生 sub-issue 与 blocked_by 边 ＋ 建图前六条裁决 ＋ 三轮对抗式审查与一轮调整落地）；② **15 张子票全关（15/15）**——八张域票（#783–#790）、【结构】（#780）、【清单·取证】（#781）、【形状】（#782，人裁 B A A）、【种子】（#844）、【交付面】（#843）、**【链路页】（#791）**、**【收口】（#792）**。八张域票串成写者互斥链跑完，主干全程单写者、未出现撞笔。

【收口】（#792）交付与验收：双端墙各 61 格（手机 390／桌面 1280）＋ 总索引（8 族 ＋ 「有意不出」7 条）＋ 逐格缺陷清单；五张缺陷票（#891 页内目录／#892 重复事实与命中区／#893 字号／#894 半角括号／#895 飞书命令行文案／#896 探测第二道门）全部落地后**重出 61 件再复评**：`逐页 ≥90 且每维 ≥ 满权 80% ＝ 61/61`（均分 98.93／最低 95），墙与索引正反自检（正例 exit 0、反例 exit 1 点名）、出页门 `pages=66 red=0`、三档非范围外 61 件全 0 溢出；两条口径由用户当场裁（向导页 `HELP` 不算扣分；公共层图表图例窄屏 11.5px 照现状收口、由 #897 承接），并据用户指令关闭该票。逐页表在 `docs/skills/skill-schedule/t792-打分表.md`，读数与裁定在 `t792-验收墙-证据.md`／`t792-签字记录.md`。

**一处如实登记（别读成「当刻这一代有人签过字」）**：用户滚过并签的是**第 1 次那一代手机墙 390**，而它已被五张缺陷票换代（页内目录、字号、半角括号、飞书命令行文案都改过）⇒ 第 1 签按票面「作废条件」作废；**当刻这一代两张墙没有逐格签字**，用户是在看过复评读数与两条口径裁定后直接下令收口的。补签办法写在 `t792-签字记录.md` 末尾。
...
- [【收口】双端墙 ＋ 总索引 ＋ 逐格缺陷清单 → 修完复评 ≥90 → 人签字](https://github.com/FeatherHunter/ilife/issues/792) — 两道墙各 61 格（390／1280，格高一致、每格真产物、未加 `loading="lazy"`）＋ 总索引（8 族 ＋ 有意不出 7 条）＋ 逐格缺陷清单；五张缺陷票落地后按当刻域产物重出 61 件再复评：`逐页 ≥90 且每维 ≥ 满权 80% ＝ 61/61`（均分 98.93／最低 95，三轮 47→58→61），墙生成器正反自检（正例 exit 0／反例 exit 1 点名两条路径）、出页门 `pages=66 red=0`、三档非范围外 61 件全 0 溢出，另补一条独立自查（无 `lazy`／格数对账／名字逐字／死链 0／两份副本逐字节同）；**顺手修掉一处会让整轮作废的地基问题** —— 读数镜像目录没人刷、复评会量到旧一代（假绿 58/61），`t792-清单.mjs` 现在发布副本与读数镜像两份一起刷。两条口径由用户当场裁（向导页 `HELP` 不算扣分；公共层图表图例窄屏 11.5px 照现状收口、由 #897 承接）并据指令关票；**第 1 代手机墙签字因产物换代作废，当刻这一代两张墙未逐格签字**（如实登记在 `t792-签字记录.md`）。
