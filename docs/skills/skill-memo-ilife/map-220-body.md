...
- **锚点〈拿到 help HTML 文件〉**：在 DSH 对 AI 说「备忘录 help」→ **明确拿到 help HTML 文件**（落盘＋可打开）：产物落 `<SKILLS_DB_PATH>/memo_html/备忘录_HELP_<YYYYMMDD_HHMMSS>.html`，回执给绝对路径。
- **锚点〈通用 help 模板渲染〉**：文件由**已经开发好的通用 help 模板**渲染（仓内真相源 `packages/base-render/assets/help-template.html` → 出口 `base-paint/help-shell`，与卡路里／饼干记账／居家管家同一套 UI），内容是备忘录自己的（老骨架 8 域／13 二级组／30 场景，新表多出的条目补进对应域）。
- **锚点〈不看老 HELP 的 UI〉**：**UI 层不与老 HELP 逐字比对**（用户 Q1=A，照 `#183` 口径）。
- **锚点〈技能侧与插件侧跑通 ＋ 肉眼终审〉**：技能侧（`skill-memo-ilife`）与插件侧（`dsh-memo-ilife` ＋ DSH 真机装机）都要跑通，并由维护者肉眼终审「过」。
...
## 进度：100%（面板 closed/total = 14/15）——**目的地达成，地图关门**

> **维护者肉眼终审「过」，原话逐字**：「**重启了 通过命令拿到了 HELP 页面 符合我的要求**」
> 验收运行产出 `备忘录_HELP_20260912_173247.html`（**130 825 B**，与编排会话验证过的样本逐字节同尺寸；模板指纹 `help-data`×2／`<title>`×1／`@media`×6 逐项同形；命令／缺失标记／实现细节字面全 0；跑完 `<db>\memo` 仍不存在）。关票决议见 `t233-resolution.md`。
> **地图 Destination 的完成条件（原文）：「两条同时达成即本图完成」——已达成。** 故地图关门。
> **原「唯一未关的子票 `#240`」已于 2026-09-12 关票**（迁移欠债已清偿：备忘录落盘改走共用件 `saveHtmlFile`、包内自持件已删；决议与逐条对账见 `docs/skills/skill-memo-ilife/t240-resolution.md`）。⇒ **本图已无未关子票**。

**画图完成（2026-09-12）**：14 张子票，原生子议题边与原生阻塞边由脚本建并逐项校验。执行期**新增 3 张**：`#240` 迁移票（收票 4 判「甲」的架构欠债，`blocked_by #237`）＋ `#241`／`#242` 两张**顺带发现的仓库级缺陷票**（不属本图目的地），另于收尾期再开 `#243`（渲染后 DOM 回归锁）。
...
**进度、frontier 与未关子票的当刻读数：见 `map-220-closing.md`（本图已关门、无未关子票）。**
...
- [备忘录HELP 迁移：出口落盘迁入共用件 `saveHtmlFile`](https://github.com/FeatherHunter/ilife/issues/240) — **执行期新增的迁移票，已关票（2026-09-12）**：还清票 4 判「甲」留下的欠债——`src/help/memoOutput.ts`（128 LF，第 4 份同逻辑实现）**整件删**，落盘只在出口 `src/cli/cmd_read.ts` 调共用件 `base-paint/save-html`（不新增文件，回到 t225 的形状）；产物名与落盘路径**逐字不变**（缺省／速查两支：131976 B、sha `059321FA…`；`--html` 含 Windows 非法字符一项按 `#237` 口径**有意偏差**，如实登记）。`#230` 五例 **5/5 绿**、包内全量 **48/48 绿**、`boundaries`／两份快照门全绿（顺手把 `#229` 遗留的 HTML 快照红关掉并给键级自证）；两次变异自证＋两席对抗式复审（形状席 乙 76／甲 72／丙 55）。决议见 `docs/skills/skill-memo-ilife/t240-resolution.md`。
