...
**仍未关的两张**（**本节为历史快照，两者均已关票**——票 10 已关、`#240` 于 2026-09-12 关票）：
...
- **`#240` 迁移欠债** —— `blocked_by #237`（共用件票 `saveHtmlFile`，属私家大厨那张图），本图不阻塞。⚠️ **已于 2026-09-12 关票**（自持件已删、落盘走共用件）。
...
**执行期新增的迁移票 `#240` 已关票**（2026-09-12）：收的是票 4 判「甲」留下的架构欠债——备忘录自持的第 4 份同逻辑落盘件已删、落盘改走共用件 `saveHtmlFile`；完成判据三条全达成，详见该票决议评论与 `docs/skills/skill-memo-ilife/t240-resolution.md`。

下一步：**等维护者肉眼终审** → 关票 13 → **地图关门**。（二者均已完成：票 13 已关、地图已关；`#240` 随后也关票 ⇒ 本图无未关子票。）
...
- **[`#240` 迁移欠债](https://github.com/FeatherHunter/ilife/issues/240)** —— `blocked_by #237`（共用件 `saveHtmlFile`，属私家大厨那张图），本图不阻塞。⚠️ **已关票（2026-09-12）**。
