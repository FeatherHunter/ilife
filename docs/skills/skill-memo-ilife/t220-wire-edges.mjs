 ...
 *
 * 用法：node docs/skills/skill-memo-ilife/t220-wire-edges.mjs
 * 依赖：`gh` 已登录（scopes 含 repo）；同目录 map-220-tickets.json 是建票时的映射。
 *
 * 阻塞边的形参是**数据库 id**（`gh api .../issues/<n> --jq .id`），不是 #号码、也不是 node_id。
 * 票面正文里的 `Blocked by:` 行只作降级兜底；本图原生边可用，故正文不写兜底行。
 */
...
const tickets = JSON.parse(readFileSync(join(here, `map-${MAP}-tickets.json`), 'utf8'));
