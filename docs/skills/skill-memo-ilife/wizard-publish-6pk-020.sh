#!/usr/bin/env bash
#
# ilife 三线发版 wizard：备忘录线首发 ＋ 卡路里线补发 ＋ 作息线首发（共 6 包）
...
banner "发布 备忘录线 0.2.0 ＋ 卡路里线 0.2.3/0.2.4 ＋ 作息线 0.2.0（共 6 包，你扫 7 次码）"

# ── Stage 1：前置门（自动，失败即停） ──────────────────────────────────
stage "1/10 · 前置门（自动，失败即停）"
say "先确认六包已归位到定死版本、依赖范围合规、工作树洁净（发版打的是工作树，不是 HEAD）。"
...
say "依赖范围（三个插件必须精确 pin 自己的技能；dsh-life-pack 走同版本线）："
node -e "
...
for d in "$D_MEMO_SKILL" "$D_MEMO_PLUGIN" "$D_CAL_SKILL" "$D_CAL_PLUGIN" "$D_SCH_SKILL" "$D_SCH_PLUGIN"; do
...
for n in skill-memo-ilife dsh-memo-ilife skill-calorie dsh-calorie skill-schedule dsh-schedule-ilife; do
  say "  $n 线上: $(reg_ver "$n")"
done
say "  当前 HEAD: $(git rev-parse --short HEAD)"

say "仓库自带的发版前置门（check-publish --pre：核 version 与依赖范围，精确 pin / 同版本线）："
if node tooling/check-publish.mjs --pre --only dsh-memo-ilife,skill-memo-ilife,dsh-calorie,skill-calorie,dsh-schedule-ilife,skill-schedule; then
...
stage "4/10 · 发布 skill-memo-ilife@$V_MEMO_SKILL（你扫码）"
...
stage "5/10 · 发布 dsh-memo-ilife@$V_MEMO_PLUGIN（你扫码）"
...
stage "6/10 · 发布 skill-calorie@$V_CAL_SKILL（你扫码）"
...
stage "7/10 · 发布 dsh-calorie@$V_CAL_PLUGIN（你扫码）"
...
# ── Stage 8：发 skill-schedule ─────────────────────────────────────────
stage "8/10 · 发布 skill-schedule@$V_SCH_SKILL（你扫码）"
...
# ── Stage 9：发 dsh-schedule-ilife ─────────────────────────────────────
stage "9/10 · 发布 dsh-schedule-ilife@$V_SCH_PLUGIN（你扫码）"
...
if [[ "$MS" == "$V_MEMO_SKILL" && "$MP" == "$V_MEMO_PLUGIN" && "$CS" == "$V_CAL_SKILL" && "$CP" == "$V_CAL_PLUGIN" && "$SS" == "$V_SCH_SKILL" && "$SP" == "$V_SCH_PLUGIN" ]]; then
  say ""
  say "六包版本全部对上 —— 发布成功。"
  say ""
  say "下一步（由编排者做）："
  step "记证据（本 wizard 的输出 ＋ npm view 原始结果）→ 落 docs/skills/skill-memo-ilife/"
  step "真机验证：这六包是 npm 侧发布；本地 DSH 仍在用 Junction 直连工作区，不必重装 —— 若要验第三方安装路径，须先解除 Junction（否则证据假绿）。"
