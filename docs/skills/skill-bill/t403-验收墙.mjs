 ...
 *
 * 用法（位置参数，照票面）：
 *   node docs/skills/skill-bill/t403-验收墙.mjs .scratch/t403-wall
 *     出两张墙＋总索引并自检：手机墙-390.html（390宽×3列1:1）＋桌面墙-1280.html（1280宽×1列整体缩）
 *     ＋总索引.html，格高一致980。
 *   node docs/skills/skill-bill/t403-验收墙.mjs --check .scratch/t403-wall
 *     只跑自检（反例测试就是这条：改清单某行file为不存在名再跑即红）。
 *
 * 命名规则（全仓只此一处算）：发布名＝`t403-页-<唤醒词>-<采集页/回执页>.html`，即清单
 * `.scratch/t403-wall/manifest.json` 的 `rows[].file`（＝`pages[].file`同一套名，重出脚本与
 * 本生成器都只读它，别处不许再拼；做法§1两侧不一致＝全墙集体死链）。墙页、索引页与产物**必须
 * 同目录**（iframe走相对路径；索引file列另给绝对路径URL可点）。
 *
 * 自检正反两面（做法链接自检正反两面都走）：
 *   正例 `node t403-验收墙.mjs .scratch/t403-wall` → 打印「18格；链接M条；缺失0->可发」**exit 0**；
 ...
 *
 * 本批读数（照单抄，生成器不自己算）：查询域17词结果页＋1附带空态＝**18格**，71-84KB／页；
 * 真跑32/32全绿见`.scratch/t403-wall/run.log`（SEED-OK＋RESULT 32/32）；他域61词归他图见索引末尾。
 */
...
function buildWall(dir, rows, out, w, h) {
  ...
  const html = '<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8">\n'
    ...
    + '<h1>t403 查询域验收墙 · ' + rows.length + '格 × ' + w + '宽（格高' + h + '）</h1>\n'
...
function buildIndex(dir, rows, notShipped) {
  ...
  const html = '<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8">\n'
    + '<meta name="viewport" content="width=device-width,initial-scale=1"><title>t403 查询域验收总索引（' + rows.length + '件）</title>\n'
...
if (argv[0] === '--check') {
  const dir = resolve(argv[1] ?? '.scratch/t403-wall');
  const { rows } = loadManifest(dir);
  selfCheck(dir, rows, pagesOnDisk(dir));
} else {
  const dir = resolve(argv[0] ?? '.scratch/t403-wall');
