...
> 判据来源：`.scratch/t92/test-full.txt`（#92 施工期全量测试输出，未入仓）。本文件是**入仓的常驻判据**，供后续票判断「新增失败 = 0」时使用：只看**失败用例名多重集**（含次数），不看 suite 级名。
> 总失败次数：22；唯一名：19。
...
- B 组：`plugin-{bill,chef,home,schedule}-ilife` 缺 `build:client: tsdown`（产物无 loader 注册头，单跑可复现）→ 归 #57–#60／打通图 #64。
