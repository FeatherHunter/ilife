...
## ⑪ 附：审查窗口内新裁定 §6 的独立复核（范围外 · 供编排者参考）
- 窗口内编排者提交 `4c96b8a`（delta 判据**修订 2** §6 跨票 WIP 三步判定）与 `7b4e891`（`check-gate-audit --since/--until`）。§6 对 `m5-receipt-97.test.mjs` 的裁定是「单独复跑 **2×仍红**（35/40）→ **#97 真实缺陷（S2，夹具顺序依赖）**」。按 `gate-runs.log` 复核其证据链：
  - `13:17:35Z`／`13:18:46Z`（ticket=97）exit=1 —— 变异 `MUT-97-2` 生效期（`receipt.ts` mtime 21:18:27，`mut2-build` 21:18:31）；
  - `13:20:15Z`／`13:20:31Z`（ticket=88，即 §6 引用的「2×仍红」）exit=1 —— **源码已于 21:19:38 还原**，但 `dist/` 仍是 21:18:31 用变异源码构建的产物（测试经 `dist/` 断言）⇒ 两轮红是**陈旧 dist**，非测试缺陷；
  - `13:21:21Z`（ticket=97，runId `2f278cc0`）**40/40 pass／fail 0**，紧随 `g1-build.log`（21:21:05 重建）之后。
  - ⇒ 同文件在「变异还原 ＋ 重建」后全绿；「2×仍红」**不足以支撑**「夹具顺序依赖」的真实缺陷判定（建议评估是否出修订 3）。
- 与 D-5 同源：**跨 session 的变异残留（源码 ＋ `dist/`）无机制隔离**。建议在 `gate-runs.log` 之外另发 `MUTATION-OPEN`／`MUTATION-CLOSED` 机读行，或规定「变异还原后强制重建 ＋ 变异期禁跨 session 全量」。

## ⑫ 审查者自报事故（如实登记，不自行豁免）
- 第 2 次提交 `5716b83`（本文件附录）**连带提交了 `docs/research/t88-review-impl-b-red.md`（64 行，红队席报告）**：该文件在我 `git add` **之前**已由红队 session 暂存于**共享 index**；我的 `git add` 只加了本文件，但 `git commit` 提交了整个已暂存集。
- 定性：**未用 `git add -A`**（首 commit 的 `git diff --cached --name-only` 仅本文件）、**未改写他人文件内容**、无内容丢失、**未 push**、**未改写历史**；但「提交里含他人路径」违反 §3 提交纪律 → 记**审查者自身的 S1-过程违规（自认）**，交编排者裁定。
- 补救：后续提交一律 `git commit --only <本文件>`；建议协议补一条——`git commit` 前必须核对 `git diff --cached --name-only` **全集**，或用 `--only`。
