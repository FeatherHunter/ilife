...
**附：提交事故登记（red5）**：本报告首次 `git add`（持锁、锁已释放）后，被蓝队 session 的提交 `5716b83` **连带提交**（`git show --stat 5716b83` 含 `t88-review-impl-b-red.md` 64 行；byte-exact 比对 `HEAD:` 与工作区**逐字一致**，内容无缺失）。成因＝**add 与 commit 跨两次持锁窗口**（§2／§3 并发风险）；已改为 add＋commit **同一持锁调用**，并在本附节留痕。建议 `run-locked` 增组合子命令（add＋commit 原子化）以消除该窗口。
