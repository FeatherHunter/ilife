...
复制**成功**后，被点击的复制按钮获得规格逐字类名 `copied`（computed 背景 = 成功色 `--ok` = `rgb(52, 199, 89)`），**约 450ms 后回落**（干净重建后 5 连跑实测 456.0–460.5ms，页面侧 1ms 轮询计时）；复制**失败**时按钮**不变绿**且出 danger toast（显式失败）。JS 侧落点仍是 `buildSharedHelpersJs()`，**冻结签名不变**、挂**既有 `boot()`／既有委派**、**零新增 marker**、**零新契约面**。

> 返修记录（红队 `t121-review-red.md` 判 FAIL 84，**无 S1**）：S2-1（A6 时长断言在干净 dist 上 6 次复跑 3 次 exit 1）＋ S3-1（指纹对 CSS 面失明）已闭环——**只改实证脚本，产品代码一行未动**（`controls.ts`／`style.ts` 自 `d0e0546` 起未变，sha256 见 §6）。
...
（该轮用的是**改前的 21 断言版**脚本；返修后脚本为 22 断言，新增 A10 双计时器互证，判定面只增不减。）

## 3. 改后（CDP 真手势实测）

脚本：`node tooling/run-locked.mjs --ticket 121 -- node docs/research/t121-browser-evidence.mjs --label <tag>`
（**必须经持锁包装器**——脚本读共享 `dist/`，他席变异构建窗口会污染读数，红队 S3-2／`t121-review-red.md:57`）
→ **`RESULT: 22/22 PASS, 0 FAIL`**（exit 0）。

**存活时长的计时口径（S2-1 返修）**：驱动侧 CDP 采样（~31ms 粒度）会**系统性低估**类名存活时长，
干净 dist 上曾复现 `394/395/398ms < 400` 的伪红（红队 `t121-review-red.md:55`）。现改为
**页面侧两个独立只读计时器**：① 1ms 轮询（主口径）；② `MutationObserver`（交叉校验）；
驱动侧采样值**只用于背景过渡取证**，不再参与时长断言。断言窗口 **440–520ms**：
**下界 440 比红队要求的 400 更严**（不是放宽），上界只容忍浏览器定时器抖动；
规格值 450ms 本身由单测 `S2` 跨文件钉死（`HELP_COPY_COPIED_MS` ≡ CSS `.45s`）。

**5 连跑台账**（干净重建后连续 5 轮，每轮各经一次持锁包装器；`node .scratch/t121/rerun-5.mjs`，
机读台账 `.scratch/t121/rerun-5.log`）：

| 轮 | 结论 | A6 轮询存活 | MutationObserver | 驱动侧采样（仅参照） | 两计时器差 | addLatency |
|---|---|---|---|---|---|---|
| r1 | **22/22 PASS** exit 0 | 456.0ms | 451.4ms | 420ms | 4.6ms | 8.5ms |
| r2 | **22/22 PASS** exit 0 | 460.5ms | 455.3ms | 434ms | 5.2ms | 16.3ms |
| r3 | **22/22 PASS** exit 0 | 459.5ms | 455.5ms | 397ms | 4.0ms | 8.7ms |
| r4 | **22/22 PASS** exit 0 | 456.8ms | 450.9ms | 394ms | 5.9ms | 9.5ms |
| r5 | **22/22 PASS** exit 0 | 457.2ms | 453.7ms | 401ms | 3.5ms | 4.2ms |

→ **5/5 全绿**，且**驱动侧采样值仍在 394–434ms 间漂移**（正是旧断言的抖动源），
而页面侧两计时器稳定在 450.9–460.5ms、互差 ≤ 5.9ms。定稿把 A6 上界从 465 放宽到 520
**只为容忍他机定时器抖动**（本机 5 轮最大值 460.5ms），**下界 440 未动**。

现场（`--label r5`，`C` = 类名含 `copied`；驱动侧时间线只证背景过渡）：

```
A 卡级复制按钮（.ilife-help-shell-card-copy，helpers 注入 8 个）
A 时间线：2:-/rgb(255,255,255) 31:C/rgb(255,255,255) 92:C/rgb(138,223,159) 154:C/rgb(67,203,102)
          216:C/rgb(53,199,90) 247:C/rgb(52,199,89) … 433:C/rgb(52,199,89) 464:-/rgb(52,199,89) …
          926:-/rgb(255,255,255)
A 摘要：pollCopiedMs=457.2 obsCopiedMs=453.7 copiedMs=402 settledBg=rgb(52, 199, 89)
        spring=transform, background-color / 0.45s, 0.2s addLatency=4.2ms polls=194
A 剪贴板回读="请你执行第 0 项：帮我复盘今天的饮食。\r\n\r\n天数:7"  toasts=1

B Sheet prompt 按钮：copied 类 → rgb(52, 199, 89) → 回落 rgb(0, 122, 255)
C 通用 .ilife-copy-btn：copied 类 → rgb(52, 199, 89) → 回落 rgb(255, 255, 255)
D 失败路径（通道 1 拒 ＋ execCommand 假）：pollCopiedMs=0／obsCopiedMs=0（**从未**出现 copied）
...
| **450ms 后回落** | A6 页面侧轮询 456–460.5ms（5 轮）、A4／B4／C4 类名已移除、A5 背景回到常态 |
| 450ms 弹簧 | A7 `transition-duration 0.45s`（改前 `all / 0s`） |
| 双反馈的另一通道（toast） | A8 真剪贴板 `readText()` 逐字 = 该卡 `<pre>` 原文；A9 单次点击恰 1 枚 toast |
| 计时可信度 | A10 两独立页面侧计时器互差 ≤ 25ms（实测 3.5–5.9ms）＋ 点击→加类延迟 < 30ms |
...
2. **实证脚本缺浏览器即 exit 2**：`docs/research/t121-browser-evidence.mjs:80-84`——找不到 Chrome／Edge 直接 `die(2, …)`，绝不静默跳过（本机实测有浏览器，故走完整 22 断言）。
...
基线 sha256：`controls.ts 82575C2102624AAC13789D9D87797D8919553DB966E2BE73C701D0BE7BD67DCF`、`style.ts 04182D2D46410B185983558072C87BE1E08838596F18C5C1A30661BD54BEEC5F`（四次变异还原后逐一相等）。
...
| **M3** | `HELP_COPY_COPIED_MS 450` → `0`（旧断言版本） | **红 5/10**（S2 跨文件同源／B1／B2／B4／B5） | **红 7/21**（含 A6 存活时长） | sha 相等 → 10/10 绿；CDP 21/21 |
| **M4** | `HELP_COPY_COPIED_MS 450` → `0`（**S2-1 返修后的脚本**，红队要求复做） | **红 5/10** | **红 8/22**（A2 A3 **A6** **A10** B2 B3 C2 C3） | sha 相等（`82575C21…7DCF`）→ 10/10 绿；CDP **22/22** |

M1 同时充当**与最终 harness 同口径的「改前」对照**（页面侧仅只读计时器、驱动侧时间线只证背景），与 §2 的改前记录互为印证。
M4 的 A6 红行实测：`pollCopiedMs=0`（类名存活塌缩到 0）→ 证明 S2-1 改后的时长断言**仍有鉴别力**，不是「靠放窗口变绿」。
...
返修轮**另跑**（持锁，逐条声明见 §7.1）：干净重建 `clean-build.mjs` exit 0；实证脚本 **5 连跑全 exit 0**（22/22×5）；M4 变异轮红 → 还原重建 → 复绿（CDP 22/22 ＋ 单测 10/10）。产品代码零改动，故四门结论沿用。
...
本票在窗口内的全部 **44 条** `RUN` 条目逐条声明如下（含**自证红轮**、**两次尝试失败**与**返修轮的 5 连跑**——
后三类**不是**门禁证据，但必须对账以免「无人声明的 RUN」）。因存在 exit≠0 的条目，使用 `--allow-nonzero`：

**对账窗口（右端封口于最后一次 `git commit` 之前）**：`--since 2026-09-09T14:30:00Z --until 2026-09-09T15:03:30Z`；实测 `matched=44/44 scoped=44 undeclared=0`，`gate-audit: PASS`（exit 0）。复核命令：

```
node tooling/check-gate-audit.mjs --evidence docs/research/t121-copied-runtime.md --log docs/research/t121-gate-runs.log --ticket 121 --since 2026-09-09T14:30:00Z --until 2026-09-09T15:03:30Z --allow-nonzero
```

> 窗口右端之后本票仅剩一次 `git commit`（证据入仓），按 §2.4 只对窗口内条目做一对一，不属未声明项。
> 他席（#121 蓝队／红队审查）的运行大多记在 `ticket=121-review-red` 等**不同票号**下，被 `--ticket 121` 过滤排除；
> 本窗口实测 `undeclared=0`，即窗口内不存在任何未声明的 `ticket=121` 条目。

GATE-RELAX flag=--allow-nonzero reason=证据同时登记：变异自证四轮红（M1/M2/M3/M4）、写测试期与返修期的红轮、一次 `git commit --only` 对未跟踪文件失败、一次包装器内 `pwsh` 不在子进程 PATH 导致的失败、返修轮 M4 的红；这些条目按 §2.4.2④ **不得**充作门禁证据，此处仅作**逐条对账**，门禁结论只取 exit=0 的条目。
...
GATE-RUN runId=52ef91f0-45a0-43f4-80a1-2badb591dd8f cmd=git add docs/research/t121-copied-runtime.md docs/research/t121-gate-runs.log .changeset/t121-copy-copied-runtime.md
GATE-RUN runId=a065e0ba-f3f3-452b-8101-4daaf8fc73a9 cmd=git commit --only docs/research/t121-copied-runtime.md docs/research/t121-gate-runs.log .changeset/t121-copy-copied-runtime.md -F .scratch/t121/commit-2.txt
GATE-RUN runId=95fff23b-a66d-4627-8b83-06ee04ac1a40 cmd=pnpm build
GATE-RUN runId=4016aa7c-f18f-475e-85ac-9d0d04fcdb32 cmd=node docs/research/t121-browser-evidence.mjs --label fix1
GATE-RUN runId=4e9cc0c6-fc35-4df1-b5ac-4841c708e286 cmd=node D:\ilife\.scratch\t121\clean-build.mjs
GATE-RUN runId=feb3db9f-6143-4268-81b9-f1c5fde8af9d cmd=node D:\ilife\docs\research\t121-browser-evidence.mjs --label r1
GATE-RUN runId=6af3ae0a-ea74-425b-95ac-043246af8501 cmd=node D:\ilife\docs\research\t121-browser-evidence.mjs --label r2
GATE-RUN runId=27b153ba-9cac-4534-b93a-07fd171b07c5 cmd=node D:\ilife\docs\research\t121-browser-evidence.mjs --label r3
GATE-RUN runId=10c1c98f-08e8-4786-a610-0c54813c8d9f cmd=node D:\ilife\docs\research\t121-browser-evidence.mjs --label r4
GATE-RUN runId=82156706-260f-4c4c-9c4b-049af7be96c5 cmd=node D:\ilife\docs\research\t121-browser-evidence.mjs --label r5
GATE-RUN runId=cb2aed36-17b0-4d7c-b88d-73cc02503a62 cmd=pnpm build
GATE-RUN runId=24b11c27-cfb2-400d-8d87-7cdf2055a514 cmd=node docs/research/t121-browser-evidence.mjs --label m4
GATE-RUN runId=e386ace7-59aa-4220-8b58-e3531db0c5e7 cmd=node --test packages/base-render/test/copy-copied-121.test.mjs
GATE-RUN runId=b6bdbd3e-dba7-4e6e-94ef-28dc3de87b08 cmd=pnpm build
GATE-RUN runId=35f680b1-27f0-469b-8c5d-f8ce20c3be81 cmd=node docs/research/t121-browser-evidence.mjs --label after-m4
GATE-RUN runId=d5898f3d-0c12-454c-8f77-e9f811130771 cmd=node --test packages/base-render/test/copy-copied-121.test.mjs
```
...
**返修轮（S2-1／S3-1）的持锁运行**（逐条声明见 §7.1）：`node .scratch/t121/clean-build.mjs`（干净重建：删 base-render `dist`＋`tsconfig.tsbuildinfo` 后 `pnpm build`，runId `4e9cc0c6`，exit 0）→ 5 连跑实证（runId `feb3db9f`／`6af3ae0a`／`27b153ba`／`10c1c98f`／`82156706`，**全 exit 0**）→ M4 变异轮（`cb2aed36` build／`24b11c27` CDP 红／`e386ace7` 单测红）→ 还原重建（`b6bdbd3e` exit 0）＋ 复绿（`35f680b1` CDP 22/22、`d5898f3d` 单测 10/10）。
...
3. **连点不延长窗口（红队 S3-3 记账）**：第二次点击会重置一个 450ms 计时器，但**前一个计时器到期仍会移除类名**（`addClass` 幂等、`removeClass` 幂等）→ 第二击可见绿窗＝首击剩余窗口（他席实测末端仅 ~48ms）。H-16 未规定连点语义，单测 `B4` 把该语义**钉死为现状**（不静默改语义）；如需「延长窗口」须另开票。
4. **注释与实现一致**：`controls.ts:574-578` 的产出内容契约 docstring 已同步记账本票行为；**注释里不含 `classList` 字面量**（否则会命中纯度扫描，见 §5）。
5. **实证脚本的计时口径与指纹（S2-1／S3-1 返修）**：存活时长改用**页面侧只读计时器**（1ms 轮询主口径 ＋ `MutationObserver` 交叉校验），驱动侧 CDP 采样值只证背景过渡；A6 窗口 440–520ms（下界比红队要求的 400 更严）。指纹纳入 **`dist/style.js`**（此前只含 `index.js`＋`controls.js`，对 CSS 面失明）。
6. **实证脚本必须持锁跑（红队 S3-2 建议落地）**：脚本读共享 `dist/`，故其运行方式在脚本 docstring 与本节写死为 `node tooling/run-locked.mjs --ticket 121 -- node docs/research/t121-browser-evidence.mjs …`；返修轮 5 连跑＋M4 轮全部如此执行（§7.1 可逐条对账）。
...
- 未在他机（不同 CPU／负载）复跑 5 连跑；本机 5 轮 A6 落在 456.0–460.5ms，上界 520ms 留 ~60ms 余量。

## 10. 风险 top3

1. **跨票 CSS 面**：`style.ts` 是 #75／#88／#121 共同产出面，后续票若再动 helpShell 区需重跑本票 CDP 脚本（一条命令，见 §8.6）。
2. **450ms 与 CSS `.45s` 的同源性靠测试守**：`copy-copied-121` 的 S2 交叉断言（常量逐值 ≡ CSS `.45s`）＋ CDP A6（运行时存活 440–520ms）；若有人只改一侧，单测即红（已用 M2／M3／M4 自证鉴别力）。
