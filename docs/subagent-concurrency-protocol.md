...
1. 一切 `pnpm build`／`pnpm test`／`tsc -b`／`node --test`（含门禁四门、靶向测试、变异轮重建）**必须**经持锁包装器执行：
   ...
   包装器负责：按 §2 抢锁（目录锁 ＋ >10 分钟死锁抢回）、按 §2.3 落盘留痕、`finally` 释放锁。
2. **证据对账**：证据文件里**声称**跑过的每次 build／test 运行，都必须在审计日志 `.scratch/locks/gate-runs.log` 里找到对应条目（票号 ＋ 命令）。对账命令：
   `node tooling/check-gate-audit.mjs --evidence <证据文件> [--ticket <票号>]`；缺失即 **exit ≠ 0** 并逐条列出缺失项。
3. **机读声明写法**：证据里声称一次运行的规范写法是单独一行 `GATE-RUN cmd=<命令>`（可附 `ticket=<票号>`）。**没有** `GATE-RUN` 声明的运行**不计入**对账，也**不得**当作门禁证据引用。
4. **裸跑**（不经包装器跑 build／test／`tsc -b`／`node --test`）＝ §6 的 **S1-过程违规**。本条款的作用是让「有没有走锁」**可机械判定**，而不是靠自报。
