...
> **怎么切票、怎么派单、怎么收窗**见 [`agents/编排纪律.md`](agents/编排纪律.md)（下发任务十条 ＋ 拆票十条 ＋ 简报八格）；本文件只管窗口内的机械步骤与量刑。
...
## 7. 收官批量补记（2026-09-14 · #293 收口实测新增，规则 38–41）

> 来源：`#293` 收官批量的四件实测教训（原始记述见 `.scratch/t314-319/编排状态.md` §五「会话内已知坑」）。**编号承接本仓落点硬规则系列**，本批新增 38–41。

- **规则 38 · 窗口回滚必须重建派生面并重签印记**：回滚若只还原 `src`（`git status` 为空）而不同步 `dist`，下一窗一开就吃 **`GEN-STALE` 假红** —— 冻结测试是从 `dist/` 导入的，`dist` 里留着回滚前写法的编译产物。回滚后必须两步连做（缺一即假）：
  `node node_modules/typescript/bin/tsc -b packages/skill-calorie --force`（作用域只到该包，不触仓根 `--force`）
  **紧跟** `node packages/skill-calorie/scripts/gen-cli.mjs --stamp`。
  **只重编不重签** ⇒ `gen:check` 记陈旧；**只重签不重编** ⇒ 拿旧产物签字。判「树干净」时 `dist/` 与印记**一并纳入**（§2.4 的加强版）。本条与 `#325` 的印记假绿**互为反面**。
- **规则 39 · 席位中途死掉留下的未提交改动＝无主 diff**：一律**不许当既成事实采信**。接手席必须先量现场（`git status --porcelain -uall`／`git diff --stat`／`--numstat` ＋ 旧新 `sha256`），再**自行判定完成度** —— 补齐、`git checkout --` 回退、重做都合法，且接手席保持「非受益者」身份。判「能不能接管」的最小证据＝`node --check` 过 ＋ diff 的 hunk 标题能逐条对上任务清单。
- **规则 40 · 取证与成文分两段派**：取证席只负责把**原始输出按维落盘**（机器读数、命令、runId）；成文席**从盘上编译报告**，不重跑、不改数。一段一人 ⇒ 报告里每个数字都有可指认的原始件，成文席也无法顺手把没跑过的读数写进去。
- **规则 41 · 提交与证据的三条限定**：
  1. **提交一律限定路径**：先 `git diff --cached --name-only` 复核暂存区**只应含自己的件**，再 `git commit -- <路径>`（不靠 `git add -A`／`git add .`）。
