...
- 本仓库 `dist/` 不进 git：装完目录里只有 SKILL.md + 源码 + 模板，
  没有可执行文件；运行时走 npm（`skill-calorie@0.3.0` 已发布）。
...
- 卡路里 `0.3.0`／`0.2.5` 发版窗口：同一三处硬编码同步升到 `0.3.0`（`packages/skill-calorie/SKILL.md` 运行时小节、本文档发现小节、`test/skills-export-47.test.mjs` 的 `NPM_PIN`）；`dsh-calorie` 同步到 `0.2.5`（精确 pin `skill-calorie@0.3.0`）。上一段的 `0.2.3` 是历史登记，保留不回改。
