// #676 读数：设置页那几项值「保存后重启宿主仍然在」的三连读数。
//
// 三连 = 改前 / 改后 / 换一个新进程再读。
// 「新进程」就是宿主重启后要面对的局面：配置文件是唯一真相，值不在内存里、只在盘上。
//
// 走的是**已编译的插件宿主侧**（`packages/plugin-calorie/dist/bridge.js`），
// 也就是面板按按钮时真正跑的那条路：RPC 端点的处理函数 → 技能 CLI 的三个配置 key。
// 不另写一套读写，也不 import 技能实现。
//
// 用法：
//   node docs/plugins/plugin-calorie/t676-读数.mjs              # 真机口径（默认落点 ~/.ilife/）
//   node docs/plugins/plugin-calorie/t676-读数.mjs --self-test  # 临时目录自证（CI/复跑用）
//
...
const BRIDGE = join(REPO, 'packages', 'plugin-calorie', 'dist', 'bridge.js');

/** 改哪一项：库文件名（最常用的一项，改了立刻看得见）。 */
const KEY = 'db.name';
const MARKER = 't676-reading.db';

const selfTest = process.argv.includes('--self-test');
const tmpDir = selfTest ? mkdtempSync(join(tmpdir(), 't676-read-')) : null;
...
try {
  ...
  if (fresh.status !== 0 || freshLine === undefined) {
    ...
    inProcess(env, 'write', selfTest ? 'calorie_data.db' : String(before.values?.db?.name ?? ''));
...
/** 在当前进程里跑一次宿主侧实现（沿用同一个 env 块；bridge 是 ESM，用一次子进程求值更省事）。 */
