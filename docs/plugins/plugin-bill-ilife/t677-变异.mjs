// #677 自证：改坏一处必须变红、改回必须变绿（验收第 5 条）。
//
// 改哪儿：`packages/plugin-bill-ilife/src/settings.ts` 里设置页行表的一个键
...
//   RESTORED-GREEN fail=<n> exit=<n> byteIdentical=<bool>
...
const TARGET = join(REPO, 'packages', 'plugin-bill-ilife', 'src', 'settings.ts');
const TEST = join(REPO, 'packages', 'plugin-bill-ilife', 'test', 'config-surface-677.test.mjs');
...
function build() {
  const r = spawnSync(process.execPath, [join(REPO, 'node_modules', 'typescript', 'bin', 'tsc'), '-b', join(REPO, 'packages', 'plugin-bill-ilife')], {
    encoding: 'utf8',
    cwd: REPO,
  });
  if ((r.status ?? -1) !== 0) {
