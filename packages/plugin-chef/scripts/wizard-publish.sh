...
#     CLI 单轨桥；精确 pin skill-chef@0.3.0）
#
# 前提（硬门，第 1 stage 会自动查，不满足即停）：
# skill-chef@0.3.0 必须已在 registry 上 —— 插件对它是精确 pin，
...
# ──────────────────────────────────────────────────────────────────────────
# STAGES — 私家大厨插件发版（版本号已定死，只走交互式认证）。
...
banner "发布 dsh-chef $V_PLUGIN（1 包，你扫码）"

# ── Stage 1：前置门（自动，失败即停） ──────────────────────────────────
stage "1/5 · 前置门（自动，失败即停）"
say "先确认版本归位、精确 pin 合规、工作树洁净（发版打的是工作树，不是 HEAD）。"
say "硬前提：skill-chef@$V_SKILL 必须已在 registry —— 插件对它是精确 pin，技能没落上去就发插件，第三方会装到旧技能。"

node -e "
const j = require('./packages/$D_PLUGIN/package.json');
if (j.version !== '$V_PLUGIN') { console.error('  ✗ dsh-chef = ' + j.version + '（期望 $V_PLUGIN）—— 停下来找编排者'); process.exit(1); }
console.log('  ✓ dsh-chef = ' + j.version);
...
console.log('  dsh-chef → skill-chef = ' + dep['skill-chef'] + '（必须精确 pin 工作区版本 ' + w + '，#129）');
console.log('  dsh-chef → dsh-life-pack = ' + dep['dsh-life-pack'] + '（须与工作区 ' + m + ' 同版本线）');
if (dep['skill-chef'] !== w) { console.error('  ✗ 精确 pin 与工作区技能版本不一致'); process.exit(1); }
"
...
say "  skill-chef 线上: $SB（期望 $V_SKILL）"
if [[ "$SB" != "$V_SKILL" ]]; then
  warn "skill-chef@$V_SKILL 还没发 —— 先跑 packages/skill-chef/scripts/wizard-publish.sh，落上去再回来。"
  exit 1
fi
...
if node tooling/check-publish.mjs --pre --only dsh-chef,skill-chef,dsh-life-pack; then
...
if already dsh-chef "$V_PLUGIN"; then
  ...
  pause "看到 + dsh-chef@$V_PLUGIN 且浏览器已批准？按回车继续"
  cd "$ROOT"
fi
...
say "从官方源实时查询线上版本与依赖："
PV=$(reg_ver dsh-chef)
say "  dsh-chef : $PV（期望 $V_PLUGIN）"
say ""
say "**「装插件要能装到最新技能」就看下面这行** —— registry 上该版插件的 skill-chef 必须是 $V_SKILL 精确值："
npm view "dsh-chef@$V_PLUGIN" dependencies --registry="$REG" --prefer-online 2>/dev/null | sed 's/^/    /' || warn "  查不到（registry 可能同步延迟，等 1 分钟重跑本 stage）"
if npm view "dsh-chef@$V_PLUGIN" dependencies --registry="$REG" --prefer-online 2>/dev/null | grep -qi 'workspace'; then
...
if [[ "$PV" == "$V_PLUGIN" ]]; then
  ...
  say "版本号对上 —— 插件发布成功。"
  say ""
  say "下一步（由编排者收口）：G3 安装态断言（插件 cliPath 落安装态技能包内 ＋ chef.help.lookup 打通）＋ check-publish --post（registry 侧复核）。"
else
