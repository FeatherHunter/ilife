// dsh-chef 烟囱：槽位定案 + 设置页住单品 + 桥缺失阻断（不返空）+ #50 安装布局与 envelope 契约（照抄 #48 卡路里样板）。
...
describe('dsh-chef 烟囱', () => {
  ...
  it('#50 envelope 契约：SKILL 直执行 chef.help.lookup 回执 key/shape/data 全字段（空库安全）', () => {
    const r = spawnCli(['chef.help.lookup']);
    assert.equal(r.status, 0);
    const env = JSON.parse(String(r.stdout));
    assert.equal(env.key, 'chef.help.lookup');
  ...
  it('#50 envelope 契约：面板路 readViaCli 同键打通不返空', () => {
    ...
    try {
      ...
      if (e instanceof SkillBridgeError && e.code === 'bad-json') data = readViaCli('chef.help.lookup');
      else throw e;
    }
