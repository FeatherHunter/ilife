 ...
 *
 * #696：本包原先**没有任何 RPC 通道**。设置页要读改配置，这里按卡路里／记账那套补齐：
 * `inject` 加 `connection`／`webServer`，`apply` 注册 `/api/ilife-chef` 路由，端点分发到 bridge 的
 * 三个配置口。client 侧禁 node，只经这条通道取用。
 */
import { RPC_CHANNEL, RPC_ENDPOINT_CONFIG_GET, RPC_ENDPOINT_CONFIG_SAVE, RPC_ENDPOINT_CONFIG_RESET, ok, fail, parseSavePayload } from './contract.js';
import type { RpcResult } from './contract.js';
import { SkillBridgeError, readConfigSurface, writeConfigValues, resetConfigToDefaults } from './bridge.js';
...
/** RPC 处理函数：endpoint 分发 → 载荷校验 → bridge 配置口 → 信封包装（#56 卡路里样板同形）。
 *
 * #696 起只有三个配置端点（`config.get`／`config.save`／`config.reset`）：设置页只经它们
 * 读改自家配置，不 spawn 技能进程，也不直读磁盘（client 侧禁 node）。 */
const handleChefRpc: RpcHandler = async (endpoint, payload): Promise<RpcResult> => {
  try {
...
export function apply(ctx: SkillHostCtx): void {
  ...
  if (conn === undefined || typeof conn.fetch?.register !== 'function') {
    ...
    logger.warn?.('[dsh-chef] connection.fetch 缺席：设置页的配置端点未注册');
    return;
  }
  ...
  try {
    const dispose = conn.fetch.register({
      ...
      async fetch(request: any) {
        ...
        if (message?.type !== 'client-request' || typeof message.rpcId !== 'string'
          ...
          return reply(rpcId, { ok: false, error: { code: 'gateway/bad-request', message: 'Invalid chef request.', details: {} } });
        }
        try { return reply(rpcId, await handleChefRpc(call.method, call.payload)); }
        catch { return new Response('chef handler failed', { status: 500 }); }
      },
    });
    ctx.effect?.(() => () => dispose(), 'dsh-chef: fetch route cleanup');
  } catch (error) {
    ...
    if (/(duplicate prefix route|already registered)/.test(String((error as Error)?.message ?? error))) {
      logger.warn?.('[dsh-chef] rpc channel ' + RPC_CHANNEL + ' already registered by another instance; yielding');
      return;
    }
    throw error;
  }
...
export { SKILL_PACKAGE, SKILL_CLI, SKILL_CLI_REL, HOST_CALL_METHOD, MANAGER_MISSING_HINT, SkillBridgeError, cliPath, assertCliPresent, handleHostCall, requestViaHost, readViaCli, resolveNodeBin, SPAWN_TIMEOUT_MS, readConfigSurface, writeConfigValues, resetConfigToDefaults, CONFIG_READ_KEY, CONFIG_WRITE_KEY, CONFIG_RESET_KEY } from './bridge.js';
export { RPC_CHANNEL, RPC_ENDPOINT_CONFIG_GET, RPC_ENDPOINT_CONFIG_SAVE, RPC_ENDPOINT_CONFIG_RESET, ok, fail, parseSavePayload, isRpcResult } from './contract.js';
...
// 值导出会让插件树在启动期报「The requested module './client.js' does not provide an export named …」
// 而整棵树起不来；连 type-only 引用也会把 client.ts 拉进宿主 `tsc -b` 的编译程序，
// 把 loader 工厂包覆写成 tsc 直出的裸 ESM（#150 事故）。
