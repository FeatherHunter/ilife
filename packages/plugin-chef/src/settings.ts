 ...
 * 取值一律由宿主经技能 CLI 的 `chef.config.read` 端点现取（见 bridge.ts）。
 *
 * 口径：
 *   · 配置表形状（键与默认值）的**唯一定义地**是技能侧 `packages/skill-chef/src/config.ts`
 *     的 `CHEF_CONFIG_DEFAULTS`（本文件是它在页面上的投影，逐键对齐由
...
const COMMON: readonly ConfigItem[] = [
  ...
  },
  {
    ...
    tier: 'common',
    control: 'text',
    hint: '数据目录下的数据库文件名。改它等于换一个菜谱库，老菜谱留在旧文件里不会跟过来。',
  },
  {
    key: 'html.dir',
    title: 'HTML 产物目录名',
    tier: 'common',
    control: 'text',
    hint: '数据目录下放交付页面的子目录名，可带一层：默认两段 cook_html/help。改它之后新页面落新地方。',
  },
];
