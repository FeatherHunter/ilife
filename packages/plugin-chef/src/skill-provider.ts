/** dsh-chef 打包技能提供方（#218 大厨线，照 #56 卡路里样板／#150 记账同形）。
 *
 ...
 *
 * 单份 SKILL.md 按包名解析（`createRequire.resolve('skill-chef/package.json')`
 * 再拼 `SKILL.md`，**不复制**，避免双份腐化；不走 `exports` 子路径，免补 `./SKILL.md` 导出）；
 * `name` 用 `skill-chef`（与 SKILL.md frontmatter 同值，描述取文件实测值，不在插件内硬编码第二份）；
...
export const PROVIDER_NAME = 'dsh-chef' as const;

/** 技能名（与 `packages/skill-chef/SKILL.md` frontmatter `name` 同值，由 test 逐字锁）。 */
export const SKILL_NAME = 'skill-chef' as const;
...
/** 按包名定位技能包根目录（安装态 `node_modules/skill-chef`，单仓 dev 态 workspace 链接）。 */
