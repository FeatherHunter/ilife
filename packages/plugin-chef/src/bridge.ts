/** dsh-chef 取数桥（P10 脚手架：纯 CLI 单轨；#696 起兼设置页的配置读写口）。
 *
 * 面板与跨技能只经 host.call 触发本桥，本桥只经 spawn 调技能包唯一出口
 * packages/skill-chef/dist/cli/cmd_read.js（argv+JSON+exit，stdout 纯 envelope JSON 一行）。
...
export const SKILL_PACKAGE = 'skill-chef' as const;
export const SKILL_CLI = 'packages/skill-chef/dist/cli/cmd_read.js' as const;
export const HOST_CALL_METHOD = 'ilife.chef.read' as const;
export const MANAGER_PACKAGE = 'dsh-life-pack' as const;

export const MANAGER_MISSING_HINT =
  '总管缺席，请补装：dsh plugin add dsh-life-pack dsh-chef（不许单卸总管）';
...
/** 设置页的三个配置 key。
 *
 * **唯一定义地是技能侧** `packages/skill-chef/src/cli/config.ts` 的 `CONFIG_KEYS`
