// #150 agent 回路：桩 ctx 抓打包技能提供方，list/get 断言（#56 卡路里样板同形）。
// 锁四件：① inject 声明 skills；② apply 注册且只注册一个提供方、重名退让不炸；
// ③ list 给唯一的 skill-bill 摘要（bundled/600/单份 SKILL.md 按包名解析、不复制）；
// ④ get 给全文（frontmatter 后正文，含唯一出口调用形）；
// 另加本技能特有的两把锁：description 必须覆盖 WAKE_TABLE 全部唤醒词（说明面与路由表不许漂移）、
// 打包清单必须带 SKILL.md（少了它，安装态提供方读不到说明面——本次实测的断链点）。
...
import { WAKE_TABLE } from 'skill-bill/policy';

/** 解析工作区里的技能包（插件 dependencies 已声明 skill-bill）。 */
...
describe('#150 打包技能提供方（记账线）', () => {
  ...
  it('list 给出唯一的 skill-bill 摘要（bundled/600/单份 SKILL.md）', async () => {
    const { ctx, calls } = stubCtx();
    apply(ctx);
    ...
    assert.equal(c.name, 'skill-bill');
    ...
    assert.ok(raw.includes(`name: ${SKILL_NAME}`), 'SKILL.md frontmatter 名须为 skill-bill');
  ...
  it('get 给全文（frontmatter 后正文，含唯一出口），过期候选失效', async () => {
    ...
    assert.ok(def.content.includes('bill-cmd-read'), '正文须含唯一出口调用形');
    assert.ok(def.content.includes('饼干记账_HELP_'), '正文须写明缺省交付物＝HELP 文件');
  ...
  it('重装配时提供方重名退让（抛 already registered 不炸，且 warn 留痕）', () => {
    ...
    ctx.skills.registerProvider = () => {
      throw new Error('a skill provider named "dsh-bill-ilife" is already registered');
    };
  ...
  it('说明面与路由表不漂移：description 覆盖 WAKE_TABLE 全部唤醒词', async () => {
    ...
    assert.ok(String(c.description).startsWith('「饼干记账HELP」'), '带头词：先说「饼干记账HELP」这条主路');
  });

  it('打包清单带 SKILL.md（安装态提供方能读到说明面）', () => {
    const skillPkg = JSON.parse(readFileSync(require.resolve('skill-bill/package.json'), 'utf8'));
    assert.ok(skillPkg.files.includes('SKILL.md'), 'skill-bill files 须含 SKILL.md（缺它＝安装态断链）');
  });
});
