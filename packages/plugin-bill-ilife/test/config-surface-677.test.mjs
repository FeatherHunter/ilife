...
describe('#677 记账设置页 · 配置面', () => {
  ...
  describe('H 目录行与系统文件夹选择器入口（#736；#743 按平台回执信封订正；#909 收进共用件）', () => {
    ...
    it('命名空间拿不到 ⇒ 没有入口（软依赖；守卫拒绝也当没有）', () => {
      ...
      // 宿主压根没有服务查找那一格 ⇒ 回 undefined（共用件据此不出入口，绝不把设置页带下来）。
      ...
      assert.equal(serviceWith(() => undefined)('remote.directoryPicker'), undefined);
      // 本家不认形状（判断只有一处，住共用件的 `resolvePicker`）：宿主给什么就原样交出去。
      const oddShape = {};
    ...
    it('点按钮 → 把那**一行**的键交给入口回填；取消一字不动', () => {
      ...
      assert.deepEqual(seen, ['db.dir'], '点了就该把这一行的键交出去（入口据此回填这一行）');
    ...
    it('这条路供不了 ⇒ 屏上出那句人话（就地失败那一支），不写值', () => {
      ...
      const tree = PanelBody({
        title: '饼干记账',
        items: CONFIG_ITEMS,
        state: { kind: 'ready', surface: { path: 'C:\\x\\bill.yaml', dataDir: 'D:\\d', created: false, values: { db: { dir: '' } } } },
        draft: toDraft({ db: { dir: '' } }, { dataDir: 'D:\\d' }),
        busy: false,
        notice: null,
        writeError: null,
        error: phrase,
