...
#   dsh-bill-ilife 0.2.0（槽位 ilife:cookie／order 95、打包技能提供方、
#     CLI 单轨桥；精确 pin skill-bill@0.2.0）
#
# 前提（硬门，第 1 stage 会自动查，不满足即停）：
# skill-bill@0.2.0 必须已在 registry 上 —— 插件对它是精确 pin，
# 先发插件＝第三方装到旧技能（#129）。先跑
# packages/skill-bill/scripts/wizard-publish.sh 把技能落上去。
#
# 前置已由编排者做完：版本已归位、依赖精确 pin 合规、dist（含 loader 工厂包
# client.js）已重建、门禁 `check-publish --pre/--tarball` 已绿、
# 包级测试 7/7（含 WAKE_TABLE 防漂移锁）。
# **你（人）只做一件事：扫码。** 登录扫一次，发布扫一次。
#
# 运行（必须 Git Bash，脚本必须 LF；发布命令绝不重定向输出，
# 否则 stdout 非 TTY 会直接 EOTP —— 见 SKILLS/npm-publish/SKILL.md §4）：
#   "C:\Program Files\Git\bin\bash.exe" D:/ilife/packages/plugin-bill-ilife/scripts/wizard-publish.sh
#
...
# ──────────────────────────────────────────────────────────────────────────
# STAGES — 饼干记账插件发版（版本号已定死，只走交互式认证）。
# 前置已由编排者做完：版本归位 ＋ 精确 pin 合规 ＋ dist（含 loader 工厂包
# client.js）重建 ＋ 门禁绿 ＋ 包级测试 7/7。
...
banner "发布 dsh-bill-ilife $V_PLUGIN（1 包，你扫码）"

# ── Stage 1：前置门（自动，失败即停） ──────────────────────────────────
stage "1/5 · 前置门（自动，失败即停）"
say "先确认版本归位、精确 pin 合规、工作树洁净（发版打的是工作树，不是 HEAD）。"
say "硬前提：skill-bill@$V_SKILL 必须已在 registry —— 插件对它是精确 pin，技能没落上去就发插件，第三方会装到旧技能。"

node -e "
const j = require('./packages/$D_PLUGIN/package.json');
if (j.version !== '$V_PLUGIN') { console.error('  ✗ dsh-bill-ilife = ' + j.version + '（期望 $V_PLUGIN）—— 停下来找编排者'); process.exit(1); }
console.log('  ✓ dsh-bill-ilife = ' + j.version);
...
console.log('  dsh-bill-ilife → skill-bill = ' + dep['skill-bill'] + '（必须精确 pin 工作区版本 ' + w + '，#129）');
console.log('  dsh-bill-ilife → dsh-life-pack = ' + dep['dsh-life-pack'] + '（须与工作区 ' + m + ' 同版本线）');
if (dep['skill-bill'] !== w) { console.error('  ✗ 精确 pin 与工作区技能版本不一致'); process.exit(1); }
"
...
SB=$(reg_ver skill-bill)
say "  skill-bill 线上: $SB（期望 $V_SKILL）"
if [[ "$SB" != "$V_SKILL" ]]; then
  warn "skill-bill@$V_SKILL 还没发 —— 先跑 packages/skill-bill/scripts/wizard-publish.sh，落上去再回来。"
  exit 1
fi
...
if node tooling/check-publish.mjs --pre --only dsh-bill-ilife,skill-bill,dsh-life-pack; then
...
# ── Stage 4：发 dsh-bill-ilife ────────────────────────────────────────
stage "4/5 · 发布 dsh-bill-ilife@$V_PLUGIN（你扫码）"
say "饼干记账插件 0.2.0：槽位 ilife:cookie／order 95、打包技能提供方（DSH 靠它认到"
say "skill-bill）、CLI 单轨桥。精确 pin skill-bill@$V_SKILL —— 这一步发出后，第三方"
say "装插件就会连带装到**这一版新技能**（402 写入 16 词 ＋ 403 查询 17 词都在里面）。"
if already dsh-bill-ilife "$V_PLUGIN"; then
  warn "registry 已有 dsh-bill-ilife@$V_PLUGIN —— 跳过本次发布（不重复占版本号）。"
else
  ...
  pause "看到 + dsh-bill-ilife@$V_PLUGIN 且浏览器已批准？按回车继续"
  cd "$ROOT"
fi
...
stage "5/5 · 验证（自动）"
say "从官方源实时查询线上版本与依赖："
PV=$(reg_ver dsh-bill-ilife)
say "  dsh-bill-ilife : $PV（期望 $V_PLUGIN）"
say ""
say "**「装插件要能装到最新技能」就看下面这行** —— registry 上该版插件的 skill-bill 必须是 $V_SKILL 精确值："
npm view "dsh-bill-ilife@$V_PLUGIN" dependencies --registry="$REG" --prefer-online 2>/dev/null | sed 's/^/    /' || warn "  查不到（registry 可能同步延迟，等 1 分钟重跑本 stage）"
if npm view "dsh-bill-ilife@$V_PLUGIN" dependencies --registry="$REG" --prefer-online 2>/dev/null | grep -qi 'workspace'; then
...
if [[ "$PV" == "$V_PLUGIN" ]]; then
  ...
  say "版本号对上 —— 插件发布成功。"
  say ""
  say "下一步（由编排者收口）：G3 安装态断言（插件 cliPath 落安装态技能包内 ＋ bill.help.lookup 双路打通）＋ check-publish --post（registry 侧复核）。"
else
