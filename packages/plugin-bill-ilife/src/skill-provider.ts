/** dsh-bill-ilife 打包技能提供方（#150 记账线，照 #56 卡路里样板同形）。
 *
 ...
 *
 * 单份 SKILL.md 按包名解析（`createRequire.resolve('skill-bill/package.json')`
 * 再拼 `SKILL.md`，**不复制**，避免双份腐化；不走 `exports` 子路径，免补 `./SKILL.md` 导出）；
 * `name` 用 `skill-bill`（与 SKILL.md frontmatter 同值，描述取文件实测值，不在插件内硬编码第二份）；
...
export const PROVIDER_NAME = 'dsh-bill-ilife' as const;

/** 技能名（与 `packages/skill-bill/SKILL.md` frontmatter `name` 同值）。 */
export const SKILL_NAME = 'skill-bill' as const;
...
/** 按包名定位技能包根目录（安装态 `node_modules/skill-bill`，单仓 dev 态 workspace 链接）。 */
