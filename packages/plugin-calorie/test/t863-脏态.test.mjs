/**
 * #863 · 脏标记与跟随态（卡路里）——改动行有标记、派生行进跟随态、保存栏吸底常显。
 * 运行：`node --test packages/plugin-calorie/test/t863-脏态.test.mjs`。
 */
...
/** 整面那一张卡的输入（就绪态 ＋ 一张脏行表）。 */
const SURFACE = {
  path: 'C:\\Users\\x\\.ilife\\calorie.yaml',
...
describe('#863 脏标记与跟随态（卡路里）', () => {
  it('跟随映射：数据目录脏带四行，照片目录脏只带 GIF 行', () => {
    assert.deepEqual(followKeysOf(['db.dir']).sort(),
      ['db.name', 'html.dir', 'photos.gifs', 'xunji.stateDir']);
    assert.deepEqual(followKeysOf(['photos.dir']), ['photos.gifs']);
