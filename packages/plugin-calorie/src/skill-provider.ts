/** dsh-calorie 打包技能提供方（#56 卡路里样板，badge 同形）。
 *
 ...
 *
 * 单份 SKILL.md 按包名解析（`createRequire.resolve('skill-calorie/package.json')`
 * 再拼 `SKILL.md`，不复制，避 R2 修法②双份腐化；不走 `exports` 子路径，
 * 免补 `./SKILL.md` 导出）；`name` 用 `skill-calorie`（与 SKILL.md frontmatter 同值，
 * 描述取文件实测值，不在插件内硬编码第二份）；`rank` 内联 600（`BUNDLED_SKILL_RANK`，
 * 不从宿主 import，保持零依赖）；`source` 为 `bundled`；`content` 取 frontmatter
 * 后正文（filesystem `parseSkillFile` 同形；badge 无 frontmatter 才整文返回，不照抄）。
 *
...
export const PROVIDER_NAME = 'dsh-calorie' as const;

/** 技能名（与 packages/skill-calorie/SKILL.md frontmatter `name` 同值）。 */
export const SKILL_NAME = 'skill-calorie' as const;
...
/** 按包名定位技能包根目录（安装态 `node_modules/skill-calorie`，单仓 dev 态 workspace 链接）。 */
...
/** 最小 frontmatter 解析（host 零依赖，不引 yaml；缺头/缺字段返回 null，由调用方按缺席记）。 */
...
/** badge 同形 provider（`list` 给摘要，`get` 给全文；`get` 忽略 locator 外的候选字段，只认名）。 */
