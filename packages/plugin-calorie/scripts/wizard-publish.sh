#!/usr/bin/env bash
#
# 卡路里插件发版 wizard：dsh-calorie（共 1 包，人扫码）
#   dsh-calorie 0.2.7（槽位 ilife:calorie／order 75、打包技能提供方、
#     CLI 单轨桥；精确 pin skill-calorie@0.2.6）
#
# 前提（硬门，第 1 stage 会自动查，不满足即停）：
# skill-calorie@0.2.6 必须已在 registry 上 —— 插件对它是精确 pin，
# 先发插件＝第三方装到旧技能（#129）。先跑
# packages/skill-calorie/scripts/wizard-publish.sh 把技能落上去。
#
...
banner "发布 dsh-calorie $V_PLUGIN（1 包，你扫码）"

# ── Stage 1：前置门（自动，失败即停） ──────────────────────────────────
stage "1/5 · 前置门（自动，失败即停）"
say "先确认版本归位、精确 pin 合规、工作树洁净（发版打的是工作树，不是 HEAD）。"
say "硬前提：skill-calorie@$V_SKILL 必须已在 registry —— 插件对它是精确 pin，技能没落上去就发插件，第三方会装到旧技能。"

node -e "
const j = require('./packages/$D_PLUGIN/package.json');
if (j.version !== '$V_PLUGIN') { console.error('  ✗ dsh-calorie = ' + j.version + '（期望 $V_PLUGIN）—— 停下来找编排者'); process.exit(1); }
console.log('  ✓ dsh-calorie = ' + j.version);
...
console.log('  dsh-calorie → skill-calorie = ' + dep['skill-calorie'] + '（必须精确 pin 工作区版本 ' + w + '，#129）');
console.log('  dsh-calorie → dsh-life-pack = ' + dep['dsh-life-pack'] + '（须与工作区 ' + m + ' 同版本线）');
if (dep['skill-calorie'] !== w) { console.error('  ✗ 精确 pin 与工作区技能版本不一致'); process.exit(1); }
"
...
SB=$(reg_ver skill-calorie)
say "  skill-calorie 线上: $SB（期望 $V_SKILL）"
if [[ "$SB" != "$V_SKILL" ]]; then
  warn "skill-calorie@$V_SKILL 还没发 —— 先跑 packages/skill-calorie/scripts/wizard-publish.sh，落上去再回来。"
  exit 1
fi
...
if node tooling/check-publish.mjs --pre --only dsh-calorie,skill-calorie,dsh-life-pack; then
...
# ── Stage 4：发 dsh-calorie ───────────────────────────────────────────
stage "4/5 · 发布 dsh-calorie@$V_PLUGIN（你扫码）"
say "卡路里插件 0.2.7：槽位 ilife:calorie／order 75、打包技能提供方、CLI 单轨桥。"
say "精确 pin skill-calorie@$V_SKILL —— 这一步发出后，第三方装插件就会连带装到**这一版技能**。"
if already dsh-calorie "$V_PLUGIN"; then
  warn "registry 已有 dsh-calorie@$V_PLUGIN —— 跳过本次发布（不重复占版本号）。"
else
  ...
  pause "看到 + dsh-calorie@$V_PLUGIN 且浏览器已批准？按回车继续"
  cd "$ROOT"
fi
...
stage "5/5 · 验证（自动）"
say "从官方源实时查询线上版本与依赖："
PV=$(reg_ver dsh-calorie)
say "  dsh-calorie : $PV（期望 $V_PLUGIN）"
say ""
say "**「装插件要能装到最新技能」就看下面这行** —— registry 上该版插件的 skill-calorie 必须是 $V_SKILL 精确值："
npm view "dsh-calorie@$V_PLUGIN" dependencies --registry="$REG" --prefer-online 2>/dev/null | sed 's/^/    /' || warn "  查不到（registry 可能同步延迟，等 1 分钟重跑本 stage）"
if npm view "dsh-calorie@$V_PLUGIN" dependencies --registry="$REG" --prefer-online 2>/dev/null | grep -qi 'workspace'; then
...
if [[ "$PV" == "$V_PLUGIN" ]]; then
  ...
  say "版本号对上 —— 插件发布成功。"
  say ""
  say "下一步（由编排者收口）：G3 安装态断言（插件 cliPath 落安装态技能包内 ＋ calorie.help.center 打通）＋ check-publish --post（registry 侧复核）。"
else
