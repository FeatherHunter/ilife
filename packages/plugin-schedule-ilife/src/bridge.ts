/** dsh-schedule-ilife 取数桥（P10 脚手架：纯 CLI 单轨；#696 起兼设置页的配置读写口）。
 *
 ...
 * 配置读写与取数都拿不到回执。照 #676 卡路里／#677 记账／#696 大厨三家补齐。 */
...
/** 设置页的三个配置 key。
 *
 * **唯一定义地是技能侧** `packages/skill-schedule/src/cli/config.ts` 的 `CONFIG_KEYS`
 ...
 */
export const CONFIG_READ_KEY = 'schedule.config.read' as const;
export const CONFIG_WRITE_KEY = 'schedule.config.write' as const;
export const CONFIG_RESET_KEY = 'schedule.config.reset' as const;
/** #706 配置体检：只读一条，回一份报告（判据由技能侧出，本包只透传，不重写一个字）。 */
export const CONFIG_CHECK_KEY = 'schedule.config.check' as const;

/** 设置页整面：文件在哪、数据在哪、当前值、是不是这次新建的（取自技能 `schedule.config.read`）。 */
...
/** 重置为默认（技能侧先另存 `<配置目录>/schedule.yaml.bak`）。 */
 ...
 *  `packages/skill-schedule/src/health.ts`，认形状是面板的事
