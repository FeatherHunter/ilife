...
/** 配置文件主体名：落点 `~/.ilife/schedule.yaml`（`ILIFE_CONFIG_DIR` 可整体接管）。 */
...
export interface ConfigItem {
  /** 配置文件里的键路径，一层嵌套用 `.` 连接，例 `lark.cliPath`。 */
  readonly key: string;
  /** 页面上那一行的中文标题。 */
...
const COMMON: readonly ConfigItem[] = [
  ...
  },
  {
    ...
    tier: 'common',
    control: 'text',
    hint: '数据目录下的数据库文件名。改它等于换一个作息库，老记录留在旧文件里不会跟过来。',
  },
  {
    key: 'html.dir',
    title: 'HTML 产物目录名',
    tier: 'common',
    control: 'text',
    hint: '数据目录下放 HELP 页面的子目录名，可带一层：默认两段 schedule_html/help。改它之后新页面落新地方。',
  },
];
...
const ADVANCED: readonly ConfigItem[] = [
  ...
  },
  {
    ...
    tier: 'advanced',
    control: 'text',
    hint: '飞书命令行工具的完整路径。留空＝没有显式值，走自动探测找装好的那一份。',
  },
];
