/** dsh-schedule-ilife 端口（port）：DSH 世界与作息管家六边形之间的契约。
 *
 * browser 安全：零 node 导入。host 适配器（注册 RPC 通道）与 client 适配器（调用 RPC）
 * 共用同一份定义。通道命名遵守宿主约束：单段且不等于 /api（见 cookbook「RPC 通道」）。
 *
 * 照 #56 卡路里／#677 记账／#696 大厨样板同形：作息插件原先**没有任何 RPC 通道**，本件是它
 * 第一份端口定义（#696：设置页要读改配置，走的就是这条通道）。
 */
export const RPC_CHANNEL = '/ilife-schedule-ilife' as const;
...
/** 解析后的绝对路径组（#764，照 #749 样板）：技能侧算好的落点，设置页的只读行**只显示、不计算**
 *  （口径「面板不算默认值、不拼路径」）。
 *
 *  格名与技能侧 `packages/skill-schedule/src/fetch/paths.ts` 的 `ScheduleResolvedPaths` 逐字同形
 *  （那是算式与取值的唯一定义地）。 */
...
/** 配置面回执：形状与技能侧 `schedule.config.read` 的 data 逐字段同形
 * （唯一定义地在那边的 cli/config.ts）。
 *
 * 这里不带默认值表——页面不猜默认值，「留空＝按默认落点」由行文案说清，
 * 「重置为默认」由技能侧 `schedule.config.reset` 执行。
 */
