#!/usr/bin/env bash
#
# 作息管家插件发版 wizard：dsh-schedule-ilife（共 1 包，人扫码）
#   dsh-schedule-ilife 0.2.1（槽位 ilife:schedule／order 80、打包技能提供方、
#     CLI 单轨桥；精确 pin skill-schedule@0.3.0）
#
# 前提（硬门，第 1 stage 会自动查，不满足即停）：
# skill-schedule@0.3.0 必须已在 registry 上 —— 插件对它是精确 pin，
# 先发插件＝第三方装到旧技能（#129）。先跑
# packages/skill-schedule/scripts/wizard-publish.sh 把技能落上去。
#
...
# ──────────────────────────────────────────────────────────────────────────
# STAGES — 作息管家插件发版（版本号已定死，只走交互式认证）。
...
# 目录名（非 npm 名）
D_PLUGIN="plugin-schedule-ilife"
...
banner "发布 dsh-schedule-ilife $V_PLUGIN（1 包，你扫码）"

# ── Stage 1：前置门（自动，失败即停） ──────────────────────────────────
stage "1/5 · 前置门（自动，失败即停）"
say "先确认版本归位、精确 pin 合规、工作树洁净（发版打的是工作树，不是 HEAD）。"
say "硬前提：skill-schedule@$V_SKILL 必须已在 registry —— 插件对它是精确 pin，技能没落上去就发插件，第三方会装到旧技能。"

node -e "
const j = require('./packages/$D_PLUGIN/package.json');
if (j.version !== '$V_PLUGIN') { console.error('  ✗ dsh-schedule-ilife = ' + j.version + '（期望 $V_PLUGIN）—— 停下来找编排者'); process.exit(1); }
console.log('  ✓ dsh-schedule-ilife = ' + j.version);
...
console.log('  dsh-schedule-ilife → skill-schedule = ' + dep['skill-schedule'] + '（必须精确 pin 工作区版本 ' + w + '，#129）');
console.log('  dsh-schedule-ilife → dsh-life-pack = ' + dep['dsh-life-pack'] + '（须与工作区 ' + m + ' 同版本线）');
if (dep['skill-schedule'] !== w) { console.error('  ✗ 精确 pin 与工作区技能版本不一致'); process.exit(1); }
"
...
SB=$(reg_ver skill-schedule)
say "  skill-schedule 线上: $SB（期望 $V_SKILL）"
if [[ "$SB" != "$V_SKILL" ]]; then
  warn "skill-schedule@$V_SKILL 还没发 —— 先跑 packages/skill-schedule/scripts/wizard-publish.sh，落上去再回来。"
  exit 1
fi
...
if node tooling/check-publish.mjs --pre --only dsh-schedule-ilife,skill-schedule,dsh-life-pack; then
...
# ── Stage 4：发 dsh-schedule-ilife ─────────────────────────────────────
stage "4/5 · 发布 dsh-schedule-ilife@$V_PLUGIN（你扫码）"
say "作息管家插件 0.2.1：槽位 ilife:schedule／order 80、打包技能提供方、CLI 单轨桥。"
say "精确 pin skill-schedule@$V_SKILL —— 这一步发出后，第三方装插件就会连带装到**这一版技能**。"
if already dsh-schedule-ilife "$V_PLUGIN"; then
  warn "registry 已有 dsh-schedule-ilife@$V_PLUGIN —— 跳过本次发布（不重复占版本号）。"
else
  ...
  pause "看到 + dsh-schedule-ilife@$V_PLUGIN 且浏览器已批准？按回车继续"
  cd "$ROOT"
fi
...
stage "5/5 · 验证（自动）"
say "从官方源实时查询线上版本与依赖："
PV=$(reg_ver dsh-schedule-ilife)
say "  dsh-schedule-ilife : $PV（期望 $V_PLUGIN）"
say ""
say "**「装插件要能装到最新技能」就看下面这行** —— registry 上该版插件的 skill-schedule 必须是 $V_SKILL 精确值："
npm view "dsh-schedule-ilife@$V_PLUGIN" dependencies --registry="$REG" --prefer-online 2>/dev/null | sed 's/^/    /' || warn "  查不到（registry 可能同步延迟，等 1 分钟重跑本 stage）"
if npm view "dsh-schedule-ilife@$V_PLUGIN" dependencies --registry="$REG" --prefer-online 2>/dev/null | grep -qi 'workspace'; then
...
if [[ "$PV" == "$V_PLUGIN" ]]; then
  ...
  say "版本号对上 —— 插件发布成功。"
  say ""
  say "下一步（由编排者收口）：G3 安装态断言（插件 cliPath 落安装态技能包内 ＋ schedule.help.lookup 打通）＋ check-publish --post（registry 侧复核）。"
else
