// #734 路线①：dsh-memo-ilife 把技能唯一出口开成 agent 工具。
...
before(() => {
  ...
  writeFileSync(join(fixtureDir, 'package.json'), JSON.stringify({
    name: 'fixture-memo', version: '0.0.0', bin: { 'fixture-cmd-read': './bin/entry.mjs' },
  }));
...
describe('#734 agent 工具：技能唯一出口的宿主调用通道', () => {
  it('inject 声明 tools；apply 注册一个同名工具，交付通道不受影响', () => {
    ...
    assert.deepEqual(calls.routes.map(([p]) => p), ['/api/ilife-memo'], 'RPC 通道照常注册');
  ...
  it('PATH 里没有 node 也跑通（宿主运行时代跑，工具不读 PATH）', async () => {
    ...
    try {
      const out = await tool.execute({ key: 'memo.help.lookup', params: { mode: 'file' } });
      const parsed = JSON.parse(out);
      assert.equal(parsed.key, 'memo.help.lookup');
  ...
  it('Electron 形态的宿主 ⇒ spawn 带 ELECTRON_RUN_AS_NODE=1，argv 指向包内入口', async () => {
    ...
    const out = await tool.execute({ key: 'memo.search' });
    ...
    assert.equal(seen[0].argv[1], 'memo.search');
