/** dsh-memo-ilife 打包技能提供方（#232 备忘录线，照 #150 记账／#218 大厨样板同形）。
 *
 ...
 *
 * 单份 SKILL.md 按包名解析（`createRequire.resolve('skill-memo-ilife/package.json')`
 * 再拼 `SKILL.md`，**不复制**，避免双份腐化；不走 `exports` 子路径，免补 `./SKILL.md` 导出）；
 * `name` 用 `skill-memo-ilife`（与 SKILL.md frontmatter 同值，描述取文件实测值，不在插件内硬编码第二份）；
...
export const PROVIDER_NAME = 'dsh-memo-ilife' as const;

/** 技能名（与 `packages/skill-memo-ilife/SKILL.md` frontmatter `name` 同值，由 test 逐字锁）。 */
export const SKILL_NAME = 'skill-memo-ilife' as const;
...
/** 按包名定位技能包根目录（安装态 `node_modules/skill-memo-ilife`，单仓 dev 态 workspace 链接）。 */
...
export function parseSkillText(text: string): ParsedSkill | null {
  ...
  // name 正则**逐字对齐宿主**（`dsh-skill/lib/index.js:17` 的 `SKILL_NAME`）——不许比宿主宽一格。
  // 宽一格的后果不是「这个技能坏了」：不合格候选会在宿主 `validateCandidate`（`dsh-skill/lib/index.js:360`，
  // 该处无 try）抛出 → 一路冒到 `dsh-tool-skill/lib/index.js:207` 每次模型请求的 `ctx.skills.snapshot()`
  // （该文件全文无 try/catch）→ **全机所有会话的技能目录一起塌**。插件与宿主口径不一致时，以宿主为准。
