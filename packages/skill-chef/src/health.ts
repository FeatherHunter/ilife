...
// #796：落点算式只有一处定义地（`src/fetch/paths.ts`）——体检报的就是那几个落点，两处不许走散。
// 本件只调**纯算式**（`*Of` 一族，不读配置、不碰盘）：体检不许调 `loadChefConfig()`（文件不在即落一份默认件）。
...
/** 目录那一项的形状是**段串**（本家默认 `cook_html/help` 两段）：算式唯一定义地是
 *  `src/fetch/paths.ts` 的 `htmlDirOf`，本件只调它，不自写第二份。 */
