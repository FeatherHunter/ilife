...
import { CHEF_CONFIG_DEFAULTS, CHEF_CONFIG_STEM } from './config.js';
...
export interface ChefHealthReport {
...
/** 库表数门槛：`src/fetch/db.ts` 的建库 DDL 里那 17 张表齐了才算正常（少了报黄：还能用，但缺功能的表要重建）。 */
export const DB_TABLE_THRESHOLD = 17 as const;

const SKILL = 'chef' as const;
 ...
 */
export function readChefConfigReadOnly(): ConfigRead {
...
function projectOnDefaults(values: Record<string, unknown>): Record<string, Record<string, unknown>> {
  ...
  for (const [group, def] of Object.entries(CHEF_CONFIG_DEFAULTS)) {
...
/** 目录那一项的形状是**段串**（本家默认 `cook_html/help` 两段），段数与 `src/config.ts` 自己的
...
export function buildChefHealthReport(): ChefHealthReport {
  const paths = configPaths(CHEF_CONFIG_STEM);
  const items: HealthItem[] = [];
  const read = readChefConfigReadOnly();
  ...
  const defaultConfigFile = join(homedir(), '.ilife', CHEF_CONFIG_STEM + '.yaml');
  ...
  const dbName = dbNameConfigured !== '' ? dbNameConfigured : String(CHEF_CONFIG_DEFAULTS.db.name);
  ...
  const htmlDir = join(dataDir, ...splitDirSegments(htmlDirValue !== '' ? htmlDirValue : String(CHEF_CONFIG_DEFAULTS.html.dir)));
  ...
  // ⑥ 包内模板目录（大厨特有）：业务页模板是包内固定件，缺了页面就渲染不出来 ⇒ 红。报文给件数。
