...
import { loadChefConfig } from '../config.js';

/** 库文件名的默认值：＝改造前的代码常量 `DB_FILENAME`（配置项 `db.name` 空串即用它）。 */
export const DEFAULT_DB_FILENAME = 'chef_data.db';
...
export function resolveDbDir(): string {
  const dir = loadChefConfig().values.db.dir;
  return dir === '' ? loadChefConfig().dataDir : dir;
}
