...
export function toStep(r: Record<string, unknown>): StepRow {
...
// 与未来唯一索引的二进制语义一致；新增与改名两处写路径（现住 add／update 域）＋本件 mustRecipe 引用它。
// 三处用户（add.run-write／update.run-write／mustRecipe），留守共享原语。
export function canonicalRecipeName(name: unknown): string {
...
// `deprecateRecipe`／`updateRecipe` 已搬入 `src/update/run-write.ts`（本域独占；旧址由 `src/fetch/index.ts` 转出）。
// `addRecipe`／`addIngredient`／`addStep` 已搬入 `src/add/run-write.ts`（同上）。
