...
export const CONFIG_KEYS = {
  read: 'chef.config.read',
  write: 'chef.config.write',
  reset: 'chef.config.reset',
} as const;
...
/** 本技能的 envelope skill 名（与 `render/envelope.ts` 的 `createEnvelope({skill:'chef'})` 同值）。 */
const CHEF_SKILL = 'chef' as const;
...
function envelope(key: string, shape: EnvelopeShape, data: unknown): string {
  return JSON.stringify({ version: ENVELOPE_VERSION, skill: CHEF_SKILL, shape, key, data });
}
...
export function runConfigKey(key: string, params: Record<string, unknown>): string {
  ...
  if (key === CONFIG_KEYS.write) {
    ...
    const current = withHumanError(() => loadChefConfig()).values as unknown as ConfigRecord;
    const merged = mergeValues(current, raw as ConfigRecord);
    const r = withHumanError(() => saveChefConfig(merged));
    return envelope(key, 'receipt', { path: r.path, values: merged });
  }
  const r = withHumanError(() => resetChefConfig());
