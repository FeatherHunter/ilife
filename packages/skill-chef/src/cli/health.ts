...
/** 体检命令名（插件侧镜像同值，见 `packages/plugin-chef/src/bridge.ts`）。 */
export const HEALTH_CHECK_KEY = 'chef.config.check' as const;

/** 本技能的 envelope skill 名（与 `src/render/envelope.ts` 的 `createEnvelope({skill:'chef'})` 同值）。 */
...
export function runHealthCheckKey(key: string): string {
  const report = buildChefHealthReport();
  const envelope = {
