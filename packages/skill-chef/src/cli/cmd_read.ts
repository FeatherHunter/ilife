 ...
 * 显式 `reuseHours` ＝ **窗口内复用**：已有那份不超过这么多小时就返回它、**不新建**——给「反复读」的调用方用
 * （不给就是落一份新的；「说一句 help 就该拿到那一份产物」这条口径不变）。
...
function dispatchHelp(params: Record<string, unknown>): HelpDispatch {
  ...
  // `reuseHours`：窗口内复用（小时）。缺省不给＝落一份新的；给了必须是正的有限数（坏参阻断，不静默当 0）。
  const reuseHours = params.reuseHours === undefined ? undefined : Number(params.reuseHours);
  if (reuseHours !== undefined && (!Number.isFinite(reuseHours) || reuseHours <= 0)) {
    fail(2, 'reuseHours 非法（' + String(params.reuseHours) + '）：须为正的小时数');
  }
