...
export const CHEF_HTML_MAX_BYTES = 256 * 1024;

export function escapeHtml(s: string): string {
  return s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#39;');
}

function titleOf(n: Record<string, unknown>): string {
  for (const k of ['name', 'action', 'message', 'summary', 'title']) {
    const v = n[k];
    if (typeof v === 'string' && v) return v;
  }
  const id = n.id;
  return id !== undefined ? '条目 ' + String(id) : '条目';
}

function metaOf(n: Record<string, unknown>): string {
  const parts: string[] = [];
  for (const k of ['id', 'sequence', 'cook_date', 'rating', 'difficulty', 'status', 'quantity', 'unit']) {
    const v = n[k];
...
function chefItemHtml(n: Record<string, unknown>): string {
  const title = escapeHtml(titleOf(n));
  const meta = escapeHtml(metaOf(n));
  return '<div class="item"><div class="item-head"><span class="badge">' + title + '</span><span class="time">' + meta + '</span></div></div>';
}

function listHtml(items: unknown[]): string {
  if (!items.length) return '<div class="hm-empty">暂无记录</div>';
  return items.map((x) => chefItemHtml(x as Record<string, unknown>)).join('');
}

// 按形状渲染 envelope 为 section 页；未知形状 throw 不返空页。
export function renderEnvelopeHtml(env: Envelope): string {
  const head = '<section data-skill="chef" data-shape="' + env.shape + '" data-key="' + escapeHtml(env.key) + '">';
  const d = env.data as Record<string, unknown>;
  switch (env.shape) {
    case 'list': return head + listHtml(d.items as unknown[]) + '</section>';
    case 'detail': return head + chefItemHtml(d.item as Record<string, unknown>) + '</section>';
    ...
    default: throw new ChefRenderError('CHEF_SHAPE_MISMATCH', '无法渲染形状：' + String((env as { shape?: unknown }).shape));
  }
}
...
export function assertHtmlSize(html: string, max = CHEF_HTML_MAX_BYTES): void {
  if (estimateBytes(html) > max) {
    throw new ChefRenderError('CHEF_HTML_TOO_LARGE', 'HTML 超体积：' + estimateBytes(html) + ' > ' + max);
  }
}
...
export function fillTemplate(template: string, contentHtml: string): string {
  for (const m of [SHARED_CSS_MARKER, SHARED_HELPERS_MARKER, CONTENT_MARKER]) {
    if (template.split(m).length - 1 !== 1) {
      throw new ChefRenderError('CHEF_MARKER_INVALID', '标记须恰出现 1 次：' + m);
    }
  }
  return template
