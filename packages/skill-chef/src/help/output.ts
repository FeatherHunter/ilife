 ...
 *
 * ## `reuseMs`：反复读时不再涨目录（**缺省不启用**）
 *
 * **缺省（不给 `reuseMs`）＝落一份新的**——「说一句 help 就该拿到那一份产物」这条口径不变。
 * 需要「一段时间内只留一份」的调用方给窗口毫秒数，例如 `reuseMs: 86_400_000`（一天内已有同一主体的一份
 * 就直接返回它、不新建）。判据在共用件里按**本件自己产出的名字里的时间戳**算（不看 mtime）。
 * 只在 `target` 路上生效；`explicit` 是用户逐字落点，不吃复用。
 *
