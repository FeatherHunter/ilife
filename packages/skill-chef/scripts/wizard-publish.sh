#!/usr/bin/env bash
#
# 私家大厨技能线发版 wizard：skill-chef（共 1 包，人扫码）
#   skill-chef 0.2.1（菜谱 HELP、本地菜谱增改、跟着做、买菜清单合并、
#     做菜记录与历史；唯一出口 chef-cmd-read）
#
# 前提：base-paint@0.3.6 已在 registry（2026-09-19 夜：#725 的 docShell、#728 的写入域形状件与逐页审计后的一刀），
# 本脚本只断言它在位，不重发。
# 插件 dsh-chef 不在本脚本发 —— 走
# packages/plugin-chef/scripts/wizard-publish.sh（技能必须先落 registry，
# 插件的精确 pin 才解析得到）。
#
...
# ──────────────────────────────────────────────────────────────────────────
# STAGES — 私家大厨技能线发版（版本号已定死，只走交互式认证）。
...
banner "发布 skill-chef $V_SKILL（1 包，你扫码）"
...
if (j.version !== '$V_SKILL') { console.error('  ✗ skill-chef = ' + j.version + '（期望 $V_SKILL）—— 停下来找编排者'); process.exit(1); }
console.log('  ✓ skill-chef = ' + j.version);
console.log('  skill-chef → base-paint = ' + j.dependencies['base-paint'] + ' ／ base-link-core = ' + j.dependencies['base-link-core']);
"
...
say "registry 现状（官方源）：skill-chef 线上: $(reg_ver skill-chef)；HEAD: $(git rev-parse --short HEAD)"

say "仓库自带的发版前置门（check-publish --pre）："
if node tooling/check-publish.mjs --pre --only skill-chef,base-paint; then
...
step "对照 files 白名单：dist、templates/*.html、SKILL.md。"
pause "清单没问题？按回车进入发布（下面开始弹浏览器了）"

# ── Stage 4：发 skill-chef ────────────────────────────────────────────
stage "4/5 · 发布 skill-chef@$V_SKILL（你扫码）"
say "私家大厨技能 0.2.1：菜谱 HELP、本地菜谱增改、跟着做、买菜清单、做菜记录，唯一出口 chef-cmd-read。"
if already skill-chef "$V_SKILL"; then
  warn "registry 已有 skill-chef@$V_SKILL —— 跳过本次发布（不重复占版本号）。"
else
  ...
  pause "看到 + skill-chef@$V_SKILL 且浏览器已批准？按回车继续"
  cd "$ROOT"
fi
...
stage "5/5 · 验证（自动）"
say "从官方源实时查询线上版本与依赖："
SB=$(reg_ver skill-chef)
say "  skill-chef : $SB（期望 $V_SKILL）"
say ""
say "skill-chef 在 registry 上的依赖（都不许出现 workspace:）："
npm view "skill-chef@$V_SKILL" dependencies --registry="$REG" --prefer-online 2>/dev/null | sed 's/^/    /' || warn "  查不到（registry 可能同步延迟，等 1 分钟重跑本 stage）"
if npm view "skill-chef@$V_SKILL" dependencies --registry="$REG" --prefer-online 2>/dev/null | grep -qi 'workspace'; then
...
if [[ "$SB" == "$V_SKILL" ]]; then
  ...
  say "版本号对上 —— 技能发布成功。"
  say ""
  say "下一步：跑插件 wizard：packages/plugin-chef/scripts/wizard-publish.sh（dsh-chef 精确 pin skill-chef@$V_SKILL，必须等这一版落 registry 才能发）。"
else
