/** #245 · 私家大厨 HELP 产物的**复用窗口**锁（真 spawn 出口；不调模块）。
 *
 * 与同包 `help-delivery-216.test.mjs` 的分工：那个文件锁「缺省交付、模板逐字、独立递补（用
 * `reuseHours:0` 验）、两支互不串、退出码矩阵」；本文件**只锁复用那一套**（#245：缺省＝一天、
 * 两支都吃窗口、复用＝只读、窗口可换、坏参阻断）。
 *
 * 口径（维护者 2026-09-12）：「**1 天内直接返回一份**，不会再创建新的。又或者 3 天这种参数。」
 *
 * 运行：先 `node node_modules/typescript/bin/tsc --build packages/skill-chef/tsconfig.json`，再
 *   node --test packages/skill-chef/test/help-reuse-245.test.mjs
 */
...
const HELP_NAME_RE = /^私家大厨_HELP_\d{8}_\d{6}(_\d+)?\.html$/;
...
test('#245 ② 两支都吃窗口：HELP 文件与速查表各自复用，目录稳定在两份', () => {
  ...
  assert.notEqual(s1.env.delivery.path, help1.env.delivery.path, '两支仍分名');
...
test('#245 ④ 坏参阻断：负数／非数／0 以外的小数一律 exit 2，且不落盘', () => {
...
test('#245 ⑤ 不吃窗口的路照旧：`--html` 逐字覆盖、`q` 现找不落盘', () => {
  ...
  const q = runOk(dir, [KEY, '--params', JSON.stringify({ q: '帮我搜个虾球菜' })]);
