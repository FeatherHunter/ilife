...
import { mkdtempSync, readFileSync, existsSync, writeFileSync, readdirSync, statSync } from 'node:fs';
...
import { billConfigDir, billEnv, saveHomeEnv } from './helpers/config-base.mjs';
...
describe('t407 E1 · 撤销／恢复候选的过筛（recordPicker.ts 的 keeps）', () => {
  ...
  it('候选读不通照实报（不拿空表冒充「库里没有记录」）', () => {
    ...
    try {
      billConfigDir(join(DB, 'cfg-blocked'), { db: { dir: join(blocker, 'sub') } });
      const html = pickerBlock({ mode: 'undo' });
  ...
  it('库里一条候选都没有时出空态，不拿最近一笔顶替', () => {
    const restore = saveHomeEnv();
    try {
      billConfigDir(mkdtempSync(join(tmpdir(), 'billpick-empty-')));
      const html = pickerBlock({ mode: 'undo' });
