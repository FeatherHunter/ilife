...
- 技能发版脚本：`scripts/wizard-publish.sh` —— 发 `base-paint@0.3.5`（前置：registry 旧版缺 `save-html`、#725 的 `docShell` 子路径与 #728 的写入域形状件，不先发技能装上就崩）＋ `skill-bill@0.2.1`（402 写入 16 词 ＋ 403 查询 17 词）。
- 插件发版脚本：`packages/plugin-bill-ilife/scripts/wizard-publish.sh` —— 发 `dsh-bill-ilife@0.2.1`。它住插件自己的目录（发谁的包，脚本就住谁的家）；硬前提是技能已落 registry（插件精确 pin 技能版本，先发插件会装到旧技能，wizard 第 1 stage 自动拦）。
