 ...
 * 文件名主体是编译期常量 `HELP_FILE_STEM`（#762 起配置项 `html.helpStem` 已退休，本件不再读盘）。
...
/** 「饼干记账help」交付文件的文件名主体：**代码常量**（#762 起配置项 `html.helpStem` 已退休——
 *  用户改的曾经是配置文件，现在没有可改处；对外也不接受调用方传入，照 #139 S3-3）。
 *  「空串＝按默认落点」那条通则随配置项一起退休：这里只有一个值。 */
