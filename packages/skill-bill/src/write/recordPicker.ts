 ...
 *
 * 谁在用（本票实数，三个调用点，指名）：
 *   ① `src/write/scene-update.ts`——缺 `id` 时列候选（未撤销的那些），挑完再说改哪个字段；
 *   ② `src/write/scene-undo.ts`——缺 `id` 时列候选（未撤销的那些；撤销＝软删打标，打完还能恢复）；
 *   ③ `src/write/scene-restore.ts`——缺 `id` 时列候选（**只列已打标撤销的那些**）。
