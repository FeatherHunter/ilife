 ...
 *
 * 谁在用（两个调用点，指名）：
 *   ① `src/write/` 的 16 件场景件——过程型采集页：第三枚形状报「还没写库」（红档）；
...
export interface TypeBadgeInput {
  ...
  /** **这一页是哪一种页**（如「采集页」「回执」「补齐槽位」）；空串＝不出这一枚。
   *  t728 去重（用户逐字要求 4「里面的文字不能出现冗余」）：改前这一行第一枚是**唤醒词**，
   *  而 H1 恒写着同一个唤醒词（实测采集页 5 次、回执页 6 次）⇒ 第一枚改成页型，
   *  一行里每一枚都是一件**别处没说过的**事。 */
...
/** 类型徽章：页型 ＋ 这一页的口径 ＋ 页面状态徽章 ＋ 下一步动作。四枚形状各自独立，不拼分隔符串。
 ...
 *  t728：两枚徽章与状态徽章收进**同一行**（`renderChipRow` 的 `tailHtml`）——它们在版面上
