 ...
 *
 * 谁在用（两个调用点，指名）：
 *   ① `src/write/` 的 16 件场景件——类型徽章的唤醒词与状态说明、字段名的中文标签都取自本件；
 ...
 *  认不得的型名与空串退回写入域的通用词（`记一笔`），口径与 `src/write/scene.ts` 的兜底同一处。 */
...
/** 内部型名 → 唤醒词；认不得的型名与空串都给「记一笔」（通用词那一件，与 `src/write/scene.ts` 的兜底同口径）。
