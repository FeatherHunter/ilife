 ...
 *
 * 谁在用（本票实数）：
 *   ① `src/write/scene-*.ts` 的 16 件场景件都指它——`Scene.receipt` 这一格现在是本件；
 *   ② `src/write/receipt.ts`——分派位只取件、不自己装配，故不直接引本件。
 *  后续三族窗口填各自那张页时，把**那件场景件**的 `receipt` 换成自己的装配体即可，本件一行不动。
 *
 * 这一段是从拆件前的 `src/write/receipt.ts` 原样搬来的（同一段代码、同一串块、同一句文案）。
 *
 * 页内块按施工图第一节的页面积木拼，一件不自造：类型徽章（`src/write/typeBadge.ts` 的 `typeBadge`）／
 *  结论摘要行 `src/write/summaryRow.ts` 的 `summaryCards`（与状态卡、这次记了几笔、写进去的项并进同一张网格）／
 *  重复检测提示条 `src/write/duplicateNote.ts`（写完再报一次，排除本次这条编号）／明细表 `renderDataTable`／
 *  页尾对账折叠区与状态卡 `src/write/receiptParts.ts`／退出口与复制区三件 `src/shared/copyArea.ts`／
 *  整页包裹 `src/shared/pageShell.ts`。
 * 页标题由**这一页是哪条唤醒词**派生，见下面 `wakeWord` 那两行的说明。
 * 未用到的 base 组件（记成遗留，不硬塞）：`renderEmptyBlock`——回执页恒有数据；失败那一面走
 *  `src/write/errorReceipt.ts`（共用位内部件），它的整页替换落点是后票的事（见该文件头）。
 */
