 ...
 *
 * 谁在用（本票实数）：`src/write/scene-expense.ts` 这一件——`Scene.collect` 就是本件。
 ...
 *
 * 本页**不写库**：写库那一半在 `src/write/write.ts`，落点是结果型回执整页（`./receiptBody.ts`）。
 */
