 ...
 *  也不出页，只把普通数据加工成普通数据。
 *
 * 谁在用（本票实数，指名）：`src/record/template-{expense,flow,batch,installment,update}.ts`
 *  五张模板各取自己用得到的那几个名字。
 */
...
import { prefillHint } from '../shared/prefillNote.js';
import type { PrefillMark } from '../shared/prefillNote.js';
import { optionsFor } from '../shared/recentPicks.js';
