 ...
 *           流程三段式（待报销记录→到账→结果，第一段用候选单选）、缺项阻断条、复制 prompt 区、复制区；
...
function collectPage(input: CollectInput): string {
  ...
  const content = [
    ...
    renderKpiGrid([
      { label: '到账额', value: money2(amount), detail: '收入记正数 · 分类「' + CATEGORY + '」' },
      {
        label: '待报销原记录',
      ...
      steps: [
        {
          ...
          html: candidatePick({
            ...
            candidates,
            selectedId: source,
            hint: '这一格要的是打了 ' + TAG_WAIT + '、钱还没回来的那一笔；拿不准就让 AI 先查「查待报销」。',
          }),
        },
        {
          title: '到账（这一步）',
