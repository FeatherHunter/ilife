 ...
 *
 * 谁在用（两个调用点，指名）：
 *   ① `src/record/write.ts` 的 `writeRecordAdd`——`bill.record.add` 有阻断项（缺必需槽位／金额方向不符）时出这一页；
 *   ② `src/record/write.ts` 的 `writeRecordUpdate`——`bill.record.update` 有阻断项（缺 `id`）时出这一页。
