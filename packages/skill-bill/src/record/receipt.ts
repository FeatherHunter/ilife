 ...
 *
 * 谁在用（两个调用点，指名）：
 *   ① `src/record/write.ts` 的 `writeRecordAdd`——记一笔写库成功后出这一页；
 *   ② `src/record/write.ts` 的 `writeRecordUpdate`（改字段／撤销／恢复三支共用）——改记录写库成功后出这一页。
 *
 * 本件只剩两件事：按落点取件（`./scene.ts` 的 `sceneFor`）、把入参原样交给那一件场景件。
 *  拆件前这里装着回执页的九块；本票把那九块原样搬进 `./receiptBody.ts`，16 件场景件现在都指它。
 *
