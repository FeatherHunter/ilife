/** 查询域的命令声明（**权威源**，四条）。
 *
 * 每条声明六件事：命令名／形状／标题（用户看到的中文名）／代表唤醒词／可执行示例／处理函数。
 *   - 代表唤醒词**必须是** `src/policy/wakewords.ts` 的 `WAKE_TABLE` 里真有的词（`test/t411-query-skeleton.test.mjs` 逐条核）；
 ...
 * 本票只登记**命令**：17 条唤醒词到这几条命令的路由住 `src/policy/wakewords.ts`（`WAKE_TABLE`，本票不搬、不改）。
 */
