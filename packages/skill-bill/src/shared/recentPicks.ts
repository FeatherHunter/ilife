 ...
 *   ① `src/shared/photoEscape.ts`——记收入／记报销／记一笔／拍账单四个场景件的字段卡，它的 `pickOf` 一行转发到本件；
 *   ② `src/record/collectBody.ts`——通用采集页装配体的那一格，它的 `pickOf` 同样一行转发到本件。

 * 本件只吃普通数据、零跨目录引用：入参是「近期记录那几列」与「本次参数」这种普通数据，
 *  **不引** `src/record/` 下的件，也不认 `CollectInput` 这类页面入参形状。可引的只有
