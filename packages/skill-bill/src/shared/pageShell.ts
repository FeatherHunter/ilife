 ...
 *      眉标由该域自报（`src/query/pageParts.ts` 的 `QUERY_EYEBROW`）。
 *  本票实测：本包 `src/` 下真引用本件的就是上面两个域，再无第三处。
 *
 * 分工（三件不要混）：
 *   - `./docPage.ts`＝文档装配那一层（`fillTemplate` 包裹、样式资产注入），本件往下调它，不重写；
 *   - 本件＝页面模板与页头的取值口径（**眉标由调用方的域自报**、标题三件套只出一处；`data-page` 是选页那一枚标记）；
 *   - 页内块序列（表单／摘要行／复制区…）归各域自己，本件不管；类型徽章另立 `../write/typeBadge.ts`。
 *
...
export interface PageShellInput {
  ...
  /** 这一页的眉标（正文标题上方那一行）：**由调用方自己的域报**——共用位不持「域名→取值」表。 */
