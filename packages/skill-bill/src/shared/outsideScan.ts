 ...
 * 谁在用（唯一调用点，指名）：
 *   `src/record/scene-photo.ts`——拍账单那件场景件的采集页取本件这五个名字；
 ...
 *  这类普通数据，**不引** `src/record/` 下的件，也不认 `CollectInput`／`ReceiptInput`。
 ...
 *   - **缺项即不出复制指令**：与代表页同口径，阻断判定与写库指令原文仍由 `src/shared/blockedSlots.ts`
 *     与 `src/shared/photoEscape.ts` 的 `blockedPromptOf` 管，本件只把那段原文串进那段话里。
