 ...
 *  `src/shared/outsideScan.ts`；② 表单那六个名字并成三个粗入口（`pickOf`＋`probeOf`＋`factsOf` 合成
 *  `valuesOf`；`commandsOf` 与 `promptOf` 合成 `blockedPromptOf`）。

 * 谁在用（四个调用点，指名；每个调用点后头写明它取哪几个名字）：
 *   ① `src/record/scene-income.ts`——记收入：`valuesOf`（候选落收入侧）／`fieldCardOf`／`blockedPromptOf`；
 *   ② `src/record/scene-reimburse.ts`——记报销：同一套三个名字，只是 `kind` 换成 `reimburse`；
 *   ③ `src/record/scene-plain.ts`——记一笔：同一套三个名字，`kind` 给空串（分类候选两侧都给，方向按金额符号判）；
 *   ④ `src/record/scene-photo.ts`——拍账单：同一套三个名字，`kind` 给 `photo`；
 *     外部识别通道的五个名字走 `src/shared/outsideScan.ts`。
 ...
 *     这一半的唯一定义地是 `src/shared/recentPicks.ts`，本件的 `pickOf` 只留一行转发；
 *   - **缺项即不出复制指令**：阻断判定在 `src/shared/blockedSlots.ts`，写库指令原文在本件的
