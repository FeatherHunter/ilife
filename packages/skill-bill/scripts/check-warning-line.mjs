 ...
 * 对实况。
 *
 * **`src/triggers/wake-assets.ts` 的挂号行不再存在，这不是删账**：它 986 那个读数是历史事实（t406
 * 那张表逐字记着），但 #686 起它按生成器 `scripts/gen-wake-assets.mjs` 自己的输出声明
 * （`const OUT = join(SRC_DIR, 'triggers', 'wake-assets.ts')`）被判为**生成物**、移出扫描面；
 * 冻结行只对「仍在扫描面内的挂号件」有意义，故 `REQUIRED` 里不再有它——缘由写在 `AGENTS.md`
 * 「本包现状」一节，不靠一条查不到的冻结值扛着。
 *
