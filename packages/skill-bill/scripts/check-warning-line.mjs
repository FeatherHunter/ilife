#!/usr/bin/env node
/**
 * #686 立门（形状照抄 #445 的 `packages/skill-calorie/scripts/check-warning-line.mjs`，数字本包现场测）：
 * 本包告警线检查——**台账齐全且与实况逐件一致**即绿，漏挂号一个超线件、或台账某行行数与实况不符，
 * 都必红。
 *
 * 判据（缺一即 FAIL，exit 1）：
 *   ①②③ 静态三条（#686 沿用卡路里 #354 口径）：含告警线数字 350；含数法（LF 口径，只数换行符）；
 ...
 *      每个 LF > 350 的件都必须在台账里有一行（主判据：硬清单只查「清单里的行存在」，
 *      拦不住「盘上另有超线件没进台账」）。
 *   ⑦ 挂号台账行齐全：`REQUIRED` 的件仍在、其「挂号值」等于冻结值（删台账任意一行必红）。
 ...
 *
 * 口径两说（`AGENTS.md` 里同义写明）：**挂号值**＝第一次挂号时的 LF，历史事实、永不回改
 * （本包冻结值 558 来源＝`packages/skill-bill/AGENTS.md` 里 t406 那张表逐字「558」），本脚本只核对
 * 它没被改写；**当场实测**＝当刻盘上的 LF（节点口径 `split('\n').length - 1`），台账里只有这一列
 * 对实况。
 *
 * **`src/triggers/wake-assets.ts` 的名字在本脚本里出现过两次，两次都不是剔除理由**：它曾按生成器
 * `scripts/gen-wake-assets.mjs` 自己的输出声明被判为**生成物**、移出扫描面（986／962 那两个读数是
 * 历史事实，t406 那张表逐字记着）；**#721 起那件生成物变成手写的薄合并件、生成器已退役**，
 * 于是它**回到扫描面内**（当刻读数远在 350 以内，不进台账）。判别法一个字没改：仍只认生成器自己的
 * 输出声明——本包今天只剩 `scripts/gen-cli.mjs` 一件生成器、一条输出声明（`src/cli/registry.ts`）。
 * 缘由写在 `AGENTS.md`「文件行数告警线」与「本包现状」两节，不靠一条查不到的冻结值扛着。
 *
 ...
 * `SCAN-ROOT:`／`LEDGER:` 两行认口；本脚本**不提供**关掉扫描面的开关（缩面＝放宽）。
...
/** 告警线：数字由包内 `AGENTS.md` 定（本包 350／LF 数法），本脚本只读不改。 */
const WARN_LINE = 350;
const OVER_LINE = '已超线，需要根据规则进行重构。';
const LEDGER_BEGIN = '<!-- warning-line-ledger:begin -->';
const LEDGER_END = '<!-- warning-line-ledger:end -->';
/** 挂号台账冻结值：来源＝`packages/skill-bill/AGENTS.md` 里 t406 那张表逐字「558」。 */
const REQUIRED = [
  { path: 'src/cli/cmd_read.ts', lf: 558 },
];
/** 同步器给新补的行写这句，提醒人补超因／拆法（不许静默替人下判断）。 */
const AUTO_WHY = '超因与拆法待补（本行由 `--sync` 自动补出，请补写超因与拆法）。';
/** 生成器识别口径＝本包 `scripts/gen-*.mjs`；生成物印记＝ `gen-cli` 写进产物件头那句。 */
const GENERATOR_RE = /^gen-.*\.mjs$/;
const GENERATED_STAMP = '勿手改';
const FIX_HINT = '修法：node packages/skill-bill/scripts/check-warning-line.mjs --sync';
  ...
  for (const r of REQUIRED) {
    ...
    if (row.挂号值 !== String(r.lf)) {
      check(`挂号台账行：${r.path}`, false,
        `挂号值被改写：${r.path}（台账 ${row.挂号值} ≠ 登记原文冻结值 ${r.lf}）`
