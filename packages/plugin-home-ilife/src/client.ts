...
export function humanizeConfigFailure(code: string, message: string): string {
  ...
  if (/缺席|missing-cli/i.test(text)) return `${text}\n技能出口没找着：先确认技能包已构建（packages/skill-home/dist/cli/cmd_read.js）。`;
...
function HomeConfig(props: { getCall: GetCall }): React.ReactElement {
  ...
  return React.createElement(
    ...
    React.createElement('div', { style: S.muted }, `${PLUGIN} 只配置，不干活：查物品与盘物品在技能功能页。`),
  );
}
...
export function apply(ctx: ClientCtx): void {
  ...
  ctx.slots.inject('ilife.config-tab', () =>
    ctx.slots.register(
      ...
      () => React.createElement(HomeConfig, { getCall }),
    ),
  );
}
