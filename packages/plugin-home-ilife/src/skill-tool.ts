/** 居家管家技能的 agent 调用通道（#734 路线①）：把「宿主进程里跑技能唯一出口」这点开给模型。
 *
 * 为什么需要（`docs/agents/技能调用契约.md` §二）：DSH 会话 PATH 上只有宿主自己的
 * `dsh`／`pnpm` 两条命令，纯 DSH 机器上会话里连 `node` 都没有 ⇒ 技能说明面里那句
 * 「跑 home-cmd-read」在 DSH 里结构性不可解析。本工具**不经会话 shell、不读 PATH**：
 ...
 * `node_modules/skill-home/dist/cli/cmd_read.js`；目录若搬动，这里不用改。
 */
...
export const SKILL_TOOL_NAME = 'run_home_command' as const;

const TOOL_DESCRIPTION =
  '运行居家管家技能的唯一出口（argv+JSON+exit），返回 envelope JSON。'
  + '写完／看完居家管家数据的唯一途径；不经 shell、不依赖 PATH。'
  + 'key 例：home.help.lookup（居家管家帮助）、home.item.search（查物品）。';
...
export function createSkillTool(deps: SkillToolDeps = {}): ToolDefinitionMirror {
  ...
  return {
    ...
    parameters: {
      type: 'object',
      properties: {
        key: { type: 'string', description: '命令（home.* 键），例如 home.help.lookup' },
    ...
    async execute(args: Record<string, unknown>): Promise<string> {
      ...
      if (typeof key !== 'string' || key.trim().length === 0) {
        throw new Error('缺 key：要跑哪条居家管家命令（例如 home.help.lookup）');
      }
      const entry = entryOf();
