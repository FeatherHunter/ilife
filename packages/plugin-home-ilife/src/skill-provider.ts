/** dsh-home-ilife 打包技能提供方（#193 居家线，照 #56 卡路里／#150 记账／#206 作息样板同形）。
 *
 ...
 *
 * 单份 SKILL.md 按包名解析（`createRequire.resolve('skill-home/package.json')`
 * 再拼 `SKILL.md`，**不复制**，避免双份腐化；不走 `exports` 子路径，免补 `./SKILL.md` 导出）；
 * `name` 用 `skill-home`（与 SKILL.md frontmatter 同值，描述取文件实测值，不在插件内硬编码第二份）；
...
export const PROVIDER_NAME = 'dsh-home-ilife' as const;

/** 技能名（与 `packages/skill-home/SKILL.md` frontmatter `name` 同值）。 */
export const SKILL_NAME = 'skill-home' as const;
...
/** 按包名定位技能包根目录（安装态 `node_modules/skill-home`，单仓 dev 态 workspace 链接）。 */
 ...
 *  只认**扁平** `key: value` 行——嵌套结构（如老家那份的 `metadata:` 子块）会让本函数返 null。 */
