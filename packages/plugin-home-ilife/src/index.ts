 ...
 *
 * #734 路线①：`inject` 加 `tools`，`apply` 把技能唯一出口注册成 agent 工具（见 skill-tool.ts）——
 * 宿主进程里 spawn 技能出口，不经会话 shell、不读 PATH（照 #56 卡路里／#150 记账同形）。
 */
...
export function apply(ctx: SkillHostCtx): void {
  ...
  try {
    ...
    if (disposeTool) ctx.effect?.(() => disposeTool, 'dsh-home-ilife: skill tool cleanup');
    else logger.warn?.('[dsh-home-ilife] tools face missing; agent tool ' + SKILL_TOOL_NAME + ' not registered');
  } catch (error) {
    // 工具名是宿主按名去重的单例：重装配时上一实例可能已注册，此时退让（提供方／通道退让同形）。
    if (/already registered|duplicate/.test(String((error as Error)?.message ?? error))) {
      logger.warn?.('[dsh-home-ilife] agent tool ' + SKILL_TOOL_NAME + ' already registered by another instance; yielding');
    } else {
      throw error;
    }
  }
