 ...
 *
 * #696：本包原先**没有任何 RPC 通道**。设置页要读改配置，这里照大厨那份补齐：
 * `inject` 加 `connection`／`webServer`，`apply` 注册 `/api/ilife-home-ilife` 路由，端点分发到 bridge 的
...
/** RPC 处理函数：endpoint 分发 → 载荷校验 → bridge 配置口 → 信封包装（照大厨那份逐字同形）。
 *
 * #696 起只有三个配置端点（`config.get`／`config.save`／`config.reset`）：设置页只经它们
 * 读改自家配置，不 spawn 技能进程，也不直读磁盘（client 侧禁 node）。 */
const handleHomeRpc: RpcHandler = async (endpoint, payload): Promise<RpcResult> => {
  try {
...
export function apply(ctx: SkillHostCtx): void {
  ...
  if (conn === undefined || typeof conn.fetch?.register !== 'function') {
    ...
    logger.warn?.('[dsh-home-ilife] connection.fetch 缺席：设置页的配置端点未注册');
    return;
  }
  ...
  try {
    const dispose = conn.fetch.register({
      ...
      async fetch(request: any) {
        ...
        if (message?.type !== 'client-request' || typeof message.rpcId !== 'string'
          ...
          return reply(rpcId, { ok: false, error: { code: 'gateway/bad-request', message: 'Invalid home request.', details: {} } });
        }
        try { return reply(rpcId, await handleHomeRpc(call.method, call.payload)); }
        catch { return new Response('home handler failed', { status: 500 }); }
      },
    });
    ctx.effect?.(() => () => dispose(), 'dsh-home-ilife: fetch route cleanup');
  } catch (error) {
    ...
    if (/(duplicate prefix route|already registered)/.test(String((error as Error)?.message ?? error))) {
      logger.warn?.('[dsh-home-ilife] rpc channel ' + RPC_CHANNEL + ' already registered by another instance; yielding');
      return;
    }
    throw error;
  }
