// dsh-home-ilife 烟囱：槽位定案 + 设置页住单品 + 桥缺失阻断（不返空）+ #50 安装布局与 envelope 契约（照抄 #48 卡路里样板）。
...
describe('dsh-home-ilife 烟囱', () => {
  it('槽位 id 与 order 与 P3 定案一致', () => {
    assert.equal(SLOT_ID, 'ilife:home');
    assert.equal(SLOT_ORDER, 90);
    assert.equal(slotDescriptor().slotId, 'ilife:home');
  });
  it('设置页住单品包', () => {
    assert.equal(SETTINGS_OWNER, 'dsh-home-ilife');
  });
  it('桥缺失阻断不返空（home 缺席抛 missing-cli）', () => {
    const cli = SKILL_CLI;
    assert.match(cli, /cmd_read\.js$/);
    assert.ok(typeof assertCliPresent === 'function');
    assert.throws(() => assertCliPresent('/nonexistent/skill-home-cmd_read.js'), (e) => e instanceof SkillBridgeError && e.code === 'missing-cli');
  });
  it('#50 安装布局：单品声明总管同版本线 ＋ 技能精确 pin（正式版号，无 workspace；#129 防旧 skill 残留）', () => {
    ...
    // 断言与版本号解耦（#123 口径）：总管＝工作区版本的**精确版**（2026-09-23 改：caret 会让「装到哪一版
    // 总管」由机器存量决定——只更新一家时顶层留旧总管、插件包内自带新总管）；技能＝#129 精确 pin
    // （caret ＋ 存量 lockfile 会让旧 skill 残留，exact 才强制重解）。两个期望值都从工作区 package.json 现取，发版不红。
    const skillVer = JSON.parse(readFileSync(join(here, '..', '..', 'skill-home', 'package.json'), 'utf8')).version;
    ...
    assert.equal(dep['skill-home'], skillVer);
  ...
  // #695：技能侧不再读环境变量 `SKILLS_DB_PATH`（配置走 `~/.ilife/home.yaml`），测试隔离改用它唯一的口子
  ...
  function spawnCli(args) {
    const cfg = mkdtempSync(join(tmpdir(), 'home-smoke-'));
    let r = spawnCliOnce(args, cfg);
    if (r.status === 0 && String(r.stdout).trim() === '') r = spawnCliOnce(args, cfg);
    return r;
  }
  it('#50 envelope 契约：SKILL 直执行 home.help.lookup 回执 key/shape/data 全字段（空库安全）', () => {
    const r = spawnCli(['home.help.lookup']);
    assert.equal(r.status, 0);
    const env = JSON.parse(String(r.stdout));
    assert.equal(env.key, 'home.help.lookup');
  ...
  it('#50 envelope 契约：面板路 readViaCli 同键打通不返空', () => {
    const cfg = mkdtempSync(join(tmpdir(), 'home-smoke-'));
    // 面板路走当刻进程的环境：技能读配置只看**家目录**（`<家>/.ilife/home.yaml`），故这里就地接管。
    useHome(cfg);
    let data;
    try {
      data = readViaCli('home.help.lookup');
    } catch (e) {
      // 同上：仅 bad-json（空白 stdout 腐坏的下游签名）重跑一次；他错直抛。真回归必两次皆红。
      if (e instanceof SkillBridgeError && e.code === 'bad-json') data = readViaCli('home.help.lookup');
      else throw e;
    }
