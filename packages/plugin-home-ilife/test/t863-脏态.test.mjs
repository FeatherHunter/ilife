/**
 * #863 · 脏标记与跟随态（居家）——改动行有标记、派生行进跟随态、保存栏吸底常显。
 * 运行：`node --test packages/plugin-home-ilife/test/t863-脏态.test.mjs`。
 */
...
/** 整面那一张卡的输入（就绪态 ＋ 一张脏行表）。 */
const SURFACE = {
  path: 'C:\\Users\\x\\.ilife\\home.yaml',
...
describe('#863 脏标记与跟随态（居家）', () => {
  it('跟随映射：数据目录一脏，四项只读派生行全进跟随', () => {
    assert.deepEqual(followKeysOf(['db.dir']).sort(), ['backup.dir', 'db.name', 'html.dir', 'key.file']);
