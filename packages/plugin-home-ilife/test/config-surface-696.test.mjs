...
describe('#696 居家设置页 · 配置面', () => {
  ...
  describe('F 设置页只配置、不干活', () => {
    ...
    it('设置页组件里没有干活入口（没有查物品／盘物品之类的取数调用）', () => {
      ...
      assert.equal(JSON.stringify(props).includes("home.item."), false, '设置页的接入面里不该出现干活命令的 key：' + "home.item.");
      assert.equal(JSON.stringify(props).includes("home.inventory."), false, '设置页的接入面里不该出现干活命令的 key：' + "home.inventory.");
    });
  ...
  describe('H 目录行与系统文件夹选择器入口（#736；#743 按平台回执信封订正；#909 收进共用件）', () => {
    it('目录档只发给目录类行：数据目录与备份目录', () => {
      ...
      assert.deepEqual(dirs, ["backup.dir","db.dir"].sort(), '目录行集合＝{backup.dir, db.dir}');
    });
