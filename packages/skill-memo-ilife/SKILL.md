...
description: "「备忘录HELP」→memo.help.lookup 出备忘录自己的 HELP 文件（老骨架 8 域／13 二级组／30 场景，走仓内通用 help 模板）；唯一出口 memo-cmd-read。触发词：备忘录 HELP（不分大小写）。文件 DB 笔记：按关键词／时间／分类／子分类搜与看、记一条、改一条、删一条（真删须 confirm）、批量改分类、提醒（设／查／废弃／完成）、心愿排期、飞书同步。"
---
# 备忘录（memo）SKILL
...
| 搜备忘 | memo.search | list | `memo-cmd-read memo.search --params '{"q":"牛奶"}'` |
| 查备忘 | memo.search | list | `memo-cmd-read memo.search` |
| 看备忘 | memo.detail | detail | `memo-cmd-read memo.detail --params '{"id":1}'` |
| 按时间搜备忘 | memo.search | list | `memo-cmd-read memo.search --params '{"start":"YYYY-MM-DD","end":"YYYY-MM-DD"}'` |
| 查打卡 | memo.search | list | `memo-cmd-read memo.search --params '{"category":"打卡"}'` |
| 查情绪 | memo.search | list | `memo-cmd-read memo.search --params '{"category":"情绪日记"}'` |
| 查心愿 | memo.wish | list | `memo-cmd-read memo.wish` |
| 看提醒 | memo.remind | list | `memo-cmd-read memo.remind` |
| 查已提醒备忘 | memo.remind | list | `memo-cmd-read memo.remind --params '{"mode":"done"}'` |
| 设提醒 | memo.reminder | receipt | `memo-cmd-read memo.reminder --params '{"note_id":15,"content":"该做保养了","remind_at":"2026-10-01 09:00"}'` |
| 记备忘 | memo.create | receipt | `memo-cmd-read memo.create --params '{"title":"买牛奶","category":"备忘"}'` |
| 改备忘 | memo.update | receipt | `memo-cmd-read memo.update --params '{"id":1,"body":"买牛奶两盒"}'` |
...
| 改心愿 | memo.update | receipt | `memo-cmd-read memo.update --params '{"id":1,"body":"学游泳"}'` |
| 记打卡 | memo.create | receipt | `memo-cmd-read memo.create --params '{"content":"跑步5公里","category":"打卡","sub_category":"跑步"}'` |
| 删打卡 | memo.remove | receipt | `memo-cmd-read memo.remove --params '{"id":1,"confirm":true}'` |
| 改打卡 | memo.update | receipt | `memo-cmd-read memo.update --params '{"id":1,"content":"跑步6公里"}'` |
| 记情绪 | memo.create | receipt | `memo-cmd-read memo.create --params '{"title":"今天很开心","category":"情绪日记"}'` |
| 删情绪 | memo.remove | receipt | `memo-cmd-read memo.remove --params '{"id":1,"confirm":true}'` |
| 改情绪 | memo.update | receipt | `memo-cmd-read memo.update --params '{"id":1,"body":"今天很开心"}'` |
| 备忘录同步 | memo.sync | receipt | `memo-cmd-read memo.sync` |
| 首次使用 | memo.init | receipt | `memo-cmd-read memo.init --params '{"data":{"items":[{"name":"数据目录","status":"ok"}],"todos":[],"verify":[]}}'` |
| 记提醒 | memo.create | receipt | `memo-cmd-read memo.create --params '{"title":"取牛奶","category":"备忘","remindAt":"2026-10-01 09:00"}'` |

相关场景：memo.auth、memo.batch、memo.create、memo.detail、memo.help.lookup、memo.init、memo.remind、memo.reminder、memo.remove、memo.search、memo.sync、memo.update、memo.wish（13 联动，key 字符串 P8 落表时冻结）。
<!-- HELP-AUTO-END -->
...
- **要速查表才加参数**：`--params '{"mode":"lookup"}'` 落同目录 `备忘录_速查表_<YYYYMMDD_HHMMSS>.html`（29 条**场景主名**一行一条：唤醒词／命令／形状／调用形／一句话；别名不列）。与 HELP 文件**分名**，两份产物不撞车；这一支同样吃上面的复用窗口。
...
- **与上一节的关系**：正文「联动速查」块是**构建期注入的索引**（`scripts/build-help.mjs` 从各域路由声明 `src/<域>/routes.ts` 的生成物注入 SKILL.md，勿手改），给 AI 按唤醒词找命令；**HELP 文件是缺省交付物**，给用户打开看。两者同源（同一张路由表＝同一处场景词面）、不是一份东西；显式 `mode:"lookup"` 落的那份是索引的页面版（`备忘录_速查表`），也不是 HELP 文件。
- **入口只认一条**：「备忘录 HELP」（不分大小写，用户 2026-09-12 裁定）；其余唤醒词是场景**别名**，只住 HELP 资产的 `aliases`（**不列进上一节的速查块**，但按词说仍能路由到同一场景），不另立 HELP 入口。
