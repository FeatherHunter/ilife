...
#   skill-memo-ilife 0.2.0（记一条／改／删／分类／提醒／心愿／统计／飞书同步；
#     唯一出口 memo-cmd-read）
#
...
# ──────────────────────────────────────────────────────────────────────────
# STAGES — 备忘录技能线发版（版本号已定死，只走交互式认证）。
...
banner "发布 skill-memo-ilife $V_SKILL（1 包，你扫码）"
...
if (j.version !== '$V_SKILL') { console.error('  ✗ skill-memo-ilife = ' + j.version + '（期望 $V_SKILL）—— 停下来找编排者'); process.exit(1); }
console.log('  ✓ skill-memo-ilife = ' + j.version);
console.log('  skill-memo-ilife → base-paint = ' + j.dependencies['base-paint'] + ' ／ base-link-core = ' + j.dependencies['base-link-core']);
"
...
if [[ "$BP" != "$V_BASEPAINT" ]]; then
  warn "base-paint@$V_BASEPAINT 还没发 —— 先发它，落上去再回来。"
  exit 1
fi
...
if ls "$ROOT/packages/$D_SKILL/templates/"*.html >/dev/null 2>&1; then
  say "  ✓ templates/*.html 在位（$(ls "$ROOT/packages/$D_SKILL/templates/"*.html | wc -l | tr -d ' ') 件）"
else
  warn "  ✗ templates/ 为空"; exit 1
fi

say "registry 现状（官方源）：skill-memo-ilife 线上: $(reg_ver skill-memo-ilife)；HEAD: $(git rev-parse --short HEAD)"

say "仓库自带的发版前置门（check-publish --pre）："
if node tooling/check-publish.mjs --pre --only skill-memo-ilife,base-paint; then
...
step "对照 files 白名单（见包内 package.json 的 files 段）。"
pause "清单没问题？按回车进入发布（下面开始弹浏览器了）"

# ── Stage 4：发 skill-memo-ilife ───────────────────────────────────────
stage "4/5 · 发布 skill-memo-ilife@$V_SKILL（你扫码）"
say "备忘录技能 0.2.0：记一条／改／删／分类／提醒／心愿／统计／飞书同步，契约键 memo.stats。"
if already skill-memo-ilife "$V_SKILL"; then
  ...
  pause "看到 + skill-memo-ilife@$V_SKILL 且浏览器已批准？按回车继续"
  cd "$ROOT"
fi
...
say "  skill-memo-ilife : $SB（期望 $V_SKILL）"
say ""
say "skill-memo-ilife 在 registry 上的依赖（都不许出现 workspace:）："
npm view "skill-memo-ilife@$V_SKILL" dependencies --registry="$REG" --prefer-online 2>/dev/null | sed 's/^/    /' || warn "  查不到（registry 可能同步延迟，等 1 分钟重跑本 stage）"
if npm view "skill-memo-ilife@$V_SKILL" dependencies --registry="$REG" --prefer-online 2>/dev/null | grep -qi 'workspace'; then
...
if [[ "$SB" == "$V_SKILL" ]]; then
  ...
  say "版本号对上 —— 技能发布成功。"
  say ""
  say "下一步：跑插件 wizard：packages/plugin-memo-ilife/scripts/wizard-publish.sh（dsh-memo-ilife 精确 pin skill-memo-ilife@$V_SKILL，必须等这一版落 registry 才能发）。"
else
