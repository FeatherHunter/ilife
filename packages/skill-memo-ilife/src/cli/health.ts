 ...
 *
 * 为什么这个 key 与三个配置命令同住 `cli/config.ts` 的旁边、而不进唤醒词路由表（各域 `src/<域>/routes.ts`，
 * 记录面 `src/triggers/routes.generated.ts`）：那张表管的是**唤醒词命令**（短语→key→形状→示例），要进 HELP
 * 与唤醒词计数。体检是**设置页专用**的只读
...
/** 本技能的 envelope skill 名（与 `src/render/envelope.ts` 的 `createEnvelope({skill:'memo'})` 同值）。 */
