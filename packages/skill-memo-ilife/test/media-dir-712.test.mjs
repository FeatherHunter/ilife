...
//
// 接缝与注入点照 `cli.test.mjs`：**真出口**＝spawn `dist/cli/cmd_read.js`，临时库走 `SKILLS_DB_PATH`，
// 附件目录走 `MEMO_MEDIA_DIR`；**不碰活库** `D:\2Study\StudyNotes\.db`。
//
...
function run(args, envExtra, cwd) {
  return spawnSync(process.execPath, [BIN, ...args], {
    cwd: cwd ?? here,
    encoding: 'utf8',
    maxBuffer: 64 * 1024 * 1024,
    env: { ...process.env, SKILLS_DB_PATH: DB, ...(envExtra || {}) },
  });
}
...
function createWithMedia(media, cwd) {
  const r = run(['memo.create', '--params', JSON.stringify({ title: '附件用例 ' + media, body: '', media })], {
    MEMO_MEDIA_DIR: MEDIA,
  }, cwd);
  let id = null;
  if (r.status === 0) {
    const items = dataOf(run(['memo.search', '--params', JSON.stringify({ q: '附件用例 ' + media })], { MEMO_MEDIA_DIR: MEDIA }));
...
before(() => {
  ...
  MEDIA_BAK = process.env.MEMO_MEDIA_DIR;
  process.env.MEMO_MEDIA_DIR = MEDIA;
...
after(() => {
  if (MEDIA_BAK === undefined) delete process.env.MEMO_MEDIA_DIR;
  else process.env.MEMO_MEDIA_DIR = MEDIA_BAK;
...
describe('#712 附件目录：真目录 ＋ 真包含判定', () => {
  ...
  it('① 目录内的路径 → 能存能读（存的是相对路径）', () => {
    ...
    const detail = dataOf(run(['memo.detail', '--params', JSON.stringify({ id: r.id })], { MEMO_MEDIA_DIR: MEDIA }));
  ...
  it('③ 同开头不同目录（mediafoo/x.jpg）→ 被拒', () => {
    ...
    const n = dataOf(run(['memo.search', '--params', JSON.stringify({ q: '附件用例 ' + samePrefix })], { MEMO_MEDIA_DIR: MEDIA }));
  ...
  it('⑤ 附件目录不存在 → 给人话原因（提到变量名与那个路径），不静默降级', () => {
    const gone = join(TMP, 'nope', 'media');
    const r = run(['memo.create', '--params', JSON.stringify({ title: '目录不在', body: '', media: 'x.jpg' })], {
      MEMO_MEDIA_DIR: gone,
    });
    assert.equal(r.status, 2, 'stderr：' + r.stderr);
    assert.match(r.stderr, /附件目录不存在/);
    assert.ok(r.stderr.includes(gone), '报错里点出那个路径：' + r.stderr);
    assert.match(r.stderr, /MEMO_MEDIA_DIR/);
    const n = run(['memo.search', '--params', JSON.stringify({ q: '目录不在' })], { MEMO_MEDIA_DIR: gone });
    assert.equal(dataOf(n).total, 0, '被拒的输入一条都没落库');
  });

  it('老数据不迁：库里已有的相对路径读出来与改动前一致', () => {
    const items = dataOf(run(['memo.search', '--params', JSON.stringify({ q: '老附件那条' })], { MEMO_MEDIA_DIR: MEDIA }));
    assert.equal(items.total, 1);
    assert.equal(items.items[0].media_path, IMG, '那一列原样读出（不重写、不补目录）');
  });

  it('真出口只多这一步：不带 media 的记一条照旧能落（回 null）', () => {
    const r = run(['memo.create', '--params', JSON.stringify({ title: '没有附件', body: '' })], { MEMO_MEDIA_DIR: MEDIA });
    assert.equal(r.status, 0, 'stderr：' + r.stderr);
    const items = dataOf(run(['memo.search', '--params', JSON.stringify({ q: '没有附件' })], { MEMO_MEDIA_DIR: MEDIA }));
