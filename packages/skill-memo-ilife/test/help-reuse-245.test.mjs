/** #245 · 备忘录 HELP 产物的**复用窗口**锁（真 spawn 出口；不调模块）。
 *
 * 病灶（#245）：HELP 被读一次就在落点目录多出一份 HTML——生产上 `memo_html/` 实测 **216 件**。
 * 口径（维护者 2026-09-12）：「**1 天内直接返回一份**，不会再创建新的。又或者 3 天这种参数。」
 *
 * 本文件锁五件事：
 *  ① **缺省＝一天**：连读 3 次目录**文件数不增**，三次回执同一份、字节同值、那份一字未改；
 *  ② **两支都吃窗口**：缺省 HELP 文件与 `mode:"lookup"` 速查表各自在窗口内复用（同键两种产物都要停涨）；
 *  ③ `reuseHours` 换窗口：`3` 同效、`0`＝**每次都要一份最新的**（落新的、不覆盖旧的）；
 *  ④ **坏参阻断**：负数／非数一律 exit 2（不静默当 0、不静默当缺省），且不落盘；
 *  ⑤ **不吃窗口的路照旧**：`--html` 逐字覆盖、`q` 现找不落盘、`help` 全程不建库。
 *
 * 运行：先 `pnpm build`（或逐包 `tsc -b`），再
 *   node --test packages/skill-memo-ilife/test/help-reuse-245.test.mjs
 */
...
const mkDir = (tag) => mkdtempSync(join(tmpdir(), 't245memo-' + tag + '-'));
...
test('#245 ⑤ 不吃窗口的路照旧：`--html` 逐字覆盖、`q` 现找不落盘、看帮助不建库', () => {
  ...
  const q = runOk(dir, [KEY, '--params', JSON.stringify({ q: '查提醒' })]);
  assert.equal(q.env.delivery, undefined, '现找（q）只回命中，不落盘');
  assert.equal(namesOf(dir).length, before, '现找不许往目录里加东西');

  assert.deepEqual(readdirSync(dir), ['sub'], '看帮助不建库目录（只多出用户给的落点子目录）');
});
