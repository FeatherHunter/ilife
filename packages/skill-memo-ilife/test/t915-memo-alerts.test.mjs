/**
 * #915 第二步 · 备忘录技能侧行告警（`memo.config.read` 的 `alerts`）。
 *
 * 三条断言（缺一不可）：
 *   ① 存在且在用的自定义数据目录 ⇒ 回执里没有 `db.dir` 这一格（`alerts` 缺席或空；
 *     附件默认落点还没建也不亮——那是空值档，不叫异常）；
 *   ② 真用不了的值（不存在的目录／同名文件占位）⇒ 有 `db.dir` 这一格，
 *     且 `message` 说那件故障本身；
 *   ③ 回执 JSON 里不出现「现在生效的是」（#915 修掉的假警报，复活它等于白干）。
 *
 * 判定复用体检 `db.dir` 项（`src/cli/health/` 的 `dirVerdict`），`message` 原样取体检那句，
 * 不新编口径。只给 `db.dir`（本家唯一体检判红的格；`media.dir`／`html.dir` 最高只到黄，
 * 缺席＝不亮，体检视图里照旧可见）。
 *
 * 隔离：每个用例一个临时家目录，真实 `~/.ilife` 一行都不碰。经真 CLI 跑（需先编 `skill-memo-ilife`）。
 *
 * 运行：`node --test packages/skill-memo-ilife/test/t915-memo-alerts.test.mjs`
 */
...
beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), 'memo-915-'));
...
describe('#915 第二步 · 备忘录行告警只报真用不了', () => {
  it('① 负例：存在且在用的自定义目录 ⇒ 没有 `db.dir` 这一格（附件默认落点未建也不亮）', () => {
  ...
  it('③ 回执全文与组装处源码无合成句（读数本身即判据）', () => {
    ...
    mkdirSync(live, { recursive: true });
    writeConfigWithDbDir(live);
    assert.equal(JSON.stringify(dataOf('memo.config.read', {})).includes('现在生效的是'), false, '干净回执不许出现合成句');
    writeConfigWithDbDir(join(home, 'missing-915-2'));
    assert.equal(JSON.stringify(dataOf('memo.config.read', {})).includes('现在生效的是'), false, '故障回执也不许出现合成句');
    const src = readFileSync(join(HERE, '..', 'src', 'cli', 'config.ts'), 'utf8');
    assert.match(src, /memoAlerts/);
