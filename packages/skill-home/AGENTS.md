...
- **数据事实源单列**：`src/help/scenarios.yaml`（1322 行）是老骨架原样副本，属**数据资产**——长度由内容决定，人无法靠重构缩短，与生成物同样**不计入手写代码告警线**；但**改动它必须重跑生成器并更新摘要锁**（`pnpm gen:help-assets`，锁见下）。#800 给 42 条变体各加一行 `route:` 标记（`variants:` 子树整棵不收，解析器无需扩，见下条）。
- 已在线的超线件：`src/fetch/db.ts`（434 行，超 84）——**归属＝另立票（不在本图 #183）**。`src/cli/cmd_read.ts` 已由 #800（地图 #797 票 3）重排为 211 行（21 个 case 换查表分派，逻辑搬进 8 能力目录），**退出本台账**。
- **管辖范围**照 `docs/agents/structure.md:9` 的「不管」清单：测试文件／页面模板／构建产物／生成的资产／文档；上面两条「单列」正是这条清单在本包的落地表述。**台账只登记管辖内的源码件**——现只有 `db.ts`（434）一件；测试件（如 `test/help-assets.test.mjs`）不适用本条告警线。

## 命令登记与分派（#800 地图 #797 票 3 落地）

- 一命令的事实只住它自己的能力目录：`src/<能力>/commands.ts`（权威声明）＋`src/<能力>/routes.ts`（路由声明）＋子功能文件；能力门 `src/<能力>/index.ts` 只转出声明与路由。
- 共用位：`src/shared/`（fail／参数整形／声明形状，被 8 能力共用）与三件派生件 `src/cli/keys.ts`／`src/cli/registry.ts`／`src/policy/routes.generated.ts`（入仓，人不手改）。
- 生成器：`scripts/gen-cli.mjs`（扫能力声明文本，fail-closed；`pnpm gen` 写盘／`pnpm gen:check` 比对；已接进仓根 `gen`／`gen:check`，CI 真跑）。
- `setup` 能力是空声明（care 键家在 family，setup 4 场景经 kind 预设到达）；`link` 只留登记位、不建目录。
...
- 技能发版脚本：`scripts/wizard-publish.sh` —— 发 `skill-home@0.3.0`（硬前提 `base-paint@0.3.6` 已在 registry，第 1 stage 自动查）。
- 插件发版脚本：`packages/plugin-home-ilife/scripts/wizard-publish.sh` —— 发 `dsh-home-ilife@0.3.1`。它住插件自己的目录（发谁的包，脚本就住谁的家）；硬前提是技能已落 registry（插件精确 pin 技能版本，wizard 第 1 stage 自动拦）。
