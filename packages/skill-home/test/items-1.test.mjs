...
// 认场景一律用命令中文名（`cn`，附录 70 条两两不重）；场景 id 只住事实源与机器附录，不进本件。
const SCENARIOS = [
  { wake: '录物品', cn: '录物品', family: 'add_form', key: 'home.item.add', params: { name: '探针杯', loc: '客厅/桌' }, check: '录入表单十二个字段都在，本次名称与件数对得上' },
  { wake: '拍物品', cn: '拍物品', family: 'add_form', key: 'home.item.add', params: { name: '探针碗', loc: '厨房/柜', photo: '1' }, check: '拍照录入与单条同页，照片随存有交代' },
  { wake: '批量录入', cn: '批量录入', family: 'add_form', key: 'home.item.add', params: { op: 'batch' }, check: '批量件数与全部确认对得上' },
  { wake: '补录', cn: '补录', family: 'add_form', key: 'home.item.add', params: { name: '旧物壶', loc: '阳台/架', op: 'backfill', backfill_date: '2025-01-01' }, check: '补录日期有处可填，回执字段齐全' },
  { wake: '查物品', cn: '查物品', family: 'search_list', key: 'home.item.search', params: { name: '探针' }, check: '结果卡片字段全，未命中指去录入' },
  { wake: '看物品', cn: '看物品', family: 'detail', key: 'home.item.detail', params: { by: 'idA' }, check: '详情字段与快捷操作齐，历史有空态' },
  { wake: '紧急定位', cn: '紧急定位', family: 'locate', key: 'home.item.search', params: { name: '探针', locate: true }, check: '置顶卡片字段全，空态指去扩大寻找' },
  { wake: '筛选浏览', cn: '筛选浏览', family: 'browse', key: 'home.item.search', params: { browse: true }, check: '分组与排序控件在列表上方，切换可用' },
  { wake: '拍照找物品', cn: '拍照找物品', family: 'search_list', key: 'home.item.search', params: { name: '探针', photo: true }, check: '拍照找物品与查物品同表，照片交代在备注' },
  { wake: '查重复', cn: '查重复', family: 'duplicates', key: 'home.item.search', params: { dupes: true }, check: '重复分组独立成页，合并建议可复制' },
];

function paramsFor(sc) {
  const p = { ...(sc.params || {}) };
  if (sc.cn === '录物品' || sc.cn === '补录') return { ...p, name: p.name, category_id: CID, location: p.loc };
  if (sc.cn === '拍物品') return { name: p.name, category_id: CID, location: p.loc, photo: p.photo };
  if (sc.cn === '批量录入') {
    return {
      op: 'batch',
      items: [
...
before(async () => {
  ...
  for (const sc of SCENARIOS) {
    if (sc.cn === '看物品') continue;
    const env = runOk(sc.key, paramsFor(sc), '真链 ' + sc.cn);
    assert.equal(env.key, sc.key);
    ENVS[sc.cn] = env;
  }
  const items = runOk('home.item.search', {}, 'seed search').data.items;
  IDA = items.find((x) => x.name === '探针杯').id;
  // 看物品（2-2）依赖探针杯的 id：先跑其余九条拿到库，再补跑看物品（顺序不影响其它产物）。
  ENVS['看物品'] = runOk('home.item.detail', { id: IDA }, '真链 看物品');
  ...
  ENVS['查重复'] = runOk('home.item.search', { dupes: true }, '真链 查重复');
  ...
  for (const sc of SCENARIOS) {
    ...
    const html = await render(sc.family, ENVS[sc.cn]);
...
describe('#806 物品管理域（一）：10 条真链与产物', () => {
  ...
  it('搜索查找物品的空态：无命中指去录入（真链空结果）', async () => {
  ...
  it('按条件筛选浏览物品：分组与计数在位', () => {
    ...
    // 换成一颗真的档位按钮（相关度序↔分组名序），并把「全部」按钮从空转改成真切回全部分组，
    // 故改查这个真入口，不钉那句老文案。
