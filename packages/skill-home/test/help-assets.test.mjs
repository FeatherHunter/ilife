...
//
// 本件认场景只用唤醒词与命令中文名；场景 id 只住事实源与机器附录，本件只在注释行里附码溯源。
...
const UNLANDED = [
  ...
  // ⑤ 宿主确认（#800 用户拍板）：推→收纳建议（SM2-3）、找→空间视图（SM2-4）
  '推位置', '找位置',
].sort();
...
const ROUTED_VARIANTS_NONSCENE = [
  // 宿主 录物品（1-1）
  '登记物品', '添加物品', '帮我记一下', '家里又多了个东西', '新到货了',
  // 宿主 查物品（2-1）
  '搜索物品', '找一下物品', '帮我找找', '看看家里有啥',
  // 宿主 看物品（2-2）
  '物品详情',
  // 宿主 盘点（6-1）
  '清点物品', '核对库存', '数数这里',
  // 宿主 管位置（SM2-1）
  '位置管理', '整理一下家里的位置', '位置怎么分的',
  // 宿主 收纳建议（SM2-3）
  '收纳位置建议', '帮我找个地方放',
  // 宿主 空间视图（SM2-4）
  '看看家里每个地方都有啥', '客厅里都有什么',
  // 宿主 统物品（SM4-1）
  '统计物品', '物品总览', '家里都有啥', '给我个总数', '一共多少件', '整体啥情况',
].sort();
