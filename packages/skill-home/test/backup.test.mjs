...
let CFG = '';
let CID = 0;
// #763：落点由配置文件决定 —— 隔离＝把**家目录**指到独占临时目录，配置落 `<它>/.ilife/home.yaml`，
// 数据目录＝`<它>/.ilife/data/`，备份目录＝数据目录下 `backups/`（配置项 `backup.dir` 的默认值）。
const DB = () => join(configDirOf(CFG), 'data');
...
describe('#707 居家备份导出／导入恢复', () => {
  ...
  it('库目录缺失时备份大声失败，不假报成功', () => {
    ...
    const r = run(['home.care.write', '--params', P({ kind: 'backup' })], homeEnvOf(other));
    // 新家目录下的默认数据目录里无 home.db：备份必须非 0（HOME_DB_MISSING → exit 4），不许产出空壳备份
    assert.notEqual(r.status, 0, '无库不得假报备份成功');
    assert.equal(existsSync(join(configDirOf(other), 'data', 'backups')), false, '失败的备份不得留下 backups 目录');
  });
});
