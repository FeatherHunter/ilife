...
export function renderFamilyPage(env: Envelope): string {
  ...
  const content = '<style>'
    ...
    + '<h2>备份与导出</h2>'
    + '<p class="su-lead">' + (isExport ? '导出完成，文件已经落盘，拿走即可在别处用。' : '备份完成，数据已经打包，保留份数之外的旧备份会自动清理。') + '</p></div>'
    + '<section class="su-sec" data-block="fields">'
    + '<h2 data-need="本次备份（路径/大小/时间）">本次结果</h2>'
    + '<div class="su-tablewrap"><table class="su-table">'
    + '<tr><th>文件</th><td>' + escapeHtml(file !== '' ? file : (isExport ? '见下方导出说明' : '本次没有产生新文件')) + '</td></tr>'
