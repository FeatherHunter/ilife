...
export function renderFamilyPage(env: Envelope): string {
  ...
  const metrics = '<div class="of-metrics">'
    ...
    + (occasion ? '<span>场合' + escapeHtml(occasion) + '</span>' : '')
  ...
  if (!sets.length) {
    ...
    designed = '<div class="of-card" id="ofNow"><h2>今日这一套</h2>' + slotRows
      + '<div class="of-styles"><span>' + escapeHtml(first.style) + '</span><span>拼贴穿搭</span></div>'
      + (first.reason ? '<p class="of-reason">' + escapeHtml(first.reason) + '</p>' : '')
      + '<div class="of-chips" id="ofOcc">'
      + ['上班', '约会', '运动', '家居', '正式', '自定义'].map((x) => '<button class="of-btn' + (occasion === x ? ' on' : '') + '" data-occ="' + x + '">' + x + '</button>').join('')
      + '</div>'
      + (hasLayers ? '<div class="of-chips"><button class="of-btn on" data-layer="outer">外层</button><button class="of-btn" data-layer="inner">内搭</button></div>' : '')
      + '</div>'
  ...
  const js = '<script>'
    ...
    + 'function ofShow(i){OFI=(i+OF.sets.length)%OF.sets.length;var s=OF.sets[OFI];var el=document.getElementById("ofCount");if(el)el.textContent="第"+(OFI+1)+"套共"+OF.sets.length+"套";var minis=document.querySelectorAll("#ofStrip .of-mini");for(var k=0;k<minis.length;k++){minis[k].className="of-mini"+(k===OFI?" on":"");}}'
    + 'var pv=document.getElementById("ofPrev");if(pv)pv.onclick=function(){ofShow(OFI-1);};'
    + 'var nx=document.getElementById("ofNext");if(nx)nx.onclick=function(){ofShow(OFI+1);};'
    + 'var st=document.getElementById("ofStrip");if(st)st.onclick=function(e){var t=e.target;while(t&&!t.getAttribute("data-set"))t=t.parentNode;if(t)ofShow(Number(t.getAttribute("data-set")));};'
    + 'var ad=document.getElementById("ofAdopt");if(ad)ad.onclick=function(){var s=OF.sets[OFI];if(!s)return;var L=["穿搭确认：今天穿第"+(OFI+1)+"套（"+s.style+"）",""];'
    + '["outer","inner","bottom","shoes","hat","acce"].forEach(function(k){var c=s.slots[k];if(c)L.push("穿搭："+c.name+"（今日穿这套）");});ofCopy(L.join("\\n"));}'
    + 'var cd=document.getElementById("ofCopyData");if(cd)cd.onclick=function(){ofCopy(JSON.stringify(OF.sets[OFI]||{},null,2));};'
    + 'var cl=document.getElementById("ofCopyLog");if(cl)cl.onclick=function(){ofCopy("穿搭日志：第"+(OFI+1)+"套 "+((OF.sets[OFI]||{}).style||""));};'
    + '</script>';
