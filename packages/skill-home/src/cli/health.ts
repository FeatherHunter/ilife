 ...
 *
 * 为什么这个 key 与三个配置命令同住 `cli/config.ts` 的旁边、而不进 `src/policy/wakewords.ts` 那张唤醒词表：
 * 那张表管的是**唤醒词命令**（短语→key→形状→示例），要进 HELP 与唤醒词计数。体检是**设置页专用**的只读
...
import { buildHomeHealthReport } from '../health.js';

/** 体检命令名（插件侧镜像同值，见 `packages/plugin-home-ilife/src/bridge.ts`）。 */
export const HEALTH_CHECK_KEY = 'home.config.check' as const;

/** 本技能的 envelope skill 名（与 `src/render/envelope.ts` 的 `createEnvelope({skill:'home'})` 同值）。 */
