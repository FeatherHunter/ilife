...
export function renderFamilyPage(env: Envelope): string {
  ...
  const content = PAGE_CSS
    ...
    + '<div class="fp-compare fp-compare-before"><h3 class="fp-sec-t">变更前</h3>'
    + '<div class="fp-row"><div class="fp-k">来源条目</div><div class="fp-v">被并入的重复条目</div></div>'
    + '<div class="fp-row"><div class="fp-k">主条</div><div class="fp-v">编号 ' + esc(target || '见回执') + '（合并前数量）</div></div>'
    + '</div>'
    + '<div class="fp-compare fp-compare-after"><h3 class="fp-sec-t">变更后</h3>'
    + '<div class="fp-row"><div class="fp-k">主条</div><div class="fp-v">编号 ' + esc(target || '见回执') + '（数量增加）</div></div>'
    + '<div class="fp-row"><div class="fp-k">并入数量</div><div class="fp-v">共 ' + esc(moved || '见回执') + ' 件</div></div>'
    + '</div>'
    + '</div></section>'
    + '<section class="fp-sec"><h2 class="fp-sec-t">逐条明细</h2>'
    + '<div class="fp-row"><div class="fp-k">保留条目</div><div class="fp-v">编号 ' + esc(target || '见回执') + ' <span class="fp-pill">主条</span></div></div>'
    + '<div class="fp-row"><div class="fp-k">并入件数</div><div class="fp-v">' + esc(moved || '见回执') + ' 件（含子标签随条目迁移）</div></div>'
    + '<div class="fp-row"><div class="fp-k">回执原文</div><div class="fp-v">' + esc(msg) + '</div></div>'
    + '</section>'
