/**
 * #915 第二步 · 作息技能侧行告警（`schedule.config.read` 的 `alerts`）。
 *
 * 三条断言（缺一不可）：
 ...
 *
 * 隔离：每个用例一个临时家目录，真实 `~/.ilife` 一行都不碰。经真 CLI 跑（需先编 `skill-schedule`）。
 *
 * 运行：`node --test packages/skill-schedule/test/t915-schedule-alerts.test.mjs`
 */
...
/** 跑一条作息命令：回 `{ code, stdout, stderr }`（非 0 不抛，交给断言判）。 */
function runSchedule(key, params) {
  const args = [CLI, key];
...
function dataOf(key, params) {
  const r = runSchedule(key, params);
...
const yamlFile = () => join(configDirOf(home), 'schedule.yaml');
...
beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), 'schedule-915-'));
...
describe('#915 第二步 · 作息行告警只报真用不了', () => {
  ...
  it('③ 回执全文与组装处源码无合成句（读数本身即判据）', () => {
    ...
    mkdirSync(live, { recursive: true });
    writeConfigWithDbDir(live);
    assert.equal(JSON.stringify(dataOf('schedule.config.read', {})).includes('现在生效的是'), false, '干净回执不许出现合成句');
    writeConfigWithDbDir(join(home, 'missing-915-2'));
    assert.equal(JSON.stringify(dataOf('schedule.config.read', {})).includes('现在生效的是'), false, '故障回执也不许出现合成句');
    const src = readFileSync(join(HERE, '..', 'src', 'cli', 'config.ts'), 'utf8');
    assert.match(src, /scheduleAlerts/);
