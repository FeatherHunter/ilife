 ...
 * 接缝只有一个＝技能的统一出口（`dist/cli/cmd_read.js`）；两个注入点＝临时数据目录（配置文件 `db.dir`）与可替换的远端平台挡板（#764 起经 PATH 首位走查找链）。
 * #695 起数据目录口子从**配置文件**来（#763 起靠**家目录注入**指到独占临时家目录：配置落 `<它>/.ilife/schedule.yaml`，见 `./helpers/config-seam.mjs`；
 * 原 `SKILLS_DB_PATH`／`LARK_CLI_PATH` 两个环境变量已按 #675 的裁决删掉；配置项 `lark.cliPath` 于 #764 删除）。**绝不碰真飞书**。
 *
