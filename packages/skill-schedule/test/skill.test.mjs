...
import { buildHelpLookup, lookupHelp, routeWakeword, SCHEDULE_KEY_SHAPES, WAKE_TABLE } from '../dist/index.js';
import { buildHelpBlock, START, END } from '../scripts/build-help.mjs';

const pkgDir = join(dirname(fileURLToPath(import.meta.url)), '..');
const skill = readFileSync(join(pkgDir, 'SKILL.md'), 'utf8');

describe('作息 SKILL 与 HELP', () => {
  it('SKILL 含出口/口径/标记块/出 scope', () => {
    assert.match(skill, /schedule-cmd-read/);
    assert.match(skill, /一级固定 8 个/);
  ...
  it('速查：短语全可路由且 key 对得上', () => {
    ...
    for (const h of hits) {
      assert.ok(Object.keys(SCHEDULE_KEY_SHAPES).includes(h.key));
      assert.ok(h.shape !== '??' && h.cli.includes(h.key) && h.desc.length > 0);
      const e = WAKE_TABLE.find((x) => x.phrase === h.phrase);
      const ctx = {};
      for (const n of (e.needs || [])) ctx[n] = n === 'id' ? 1 : n === 'start' || n === 'end' ? '2026-09-01' : 'x';
  ...
  it('互联区新鲜（构建期注入可复现）', () => {
    ...
    assert.ok(lookupHelp(buildHelpLookup(), '帮我查作息').some((h) => h.key === 'schedule.record.today'));
    assert.ok(lookupHelp(buildHelpLookup(), '').length === WAKE_TABLE.length);
  });
  it('废弃词不在速查中', () => {
    const phrases = buildHelpLookup().map((h) => h.phrase).join('\n');
    for (const dead of ['同步作息', '作息计划表', '配置定时同步']) {
