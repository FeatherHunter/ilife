/** 交付面·落盘点：**本包写 HTML 文件的唯一那一个函数**（页面与 HELP 共用一条链）。
 *
 ...
 *
 *  #843 起本件**不再只是 HELP 的**：出口的 8 个键（7 个唤醒词命令 ＋ HELP）都从这里落盘，
 *  故件头与件名从 `src/help/output.ts` 摆到 `src/delivery/output.ts`——「交付面」是这条链的名字，
 *  文件名主体由 `./naming.ts` 一处定义（页面取命令标题，HELP 取 `作息管家_HELP`）。
 *
 ...
 *   - 串行语义不变：交付仍是「通式名 ＋ 首试即中」⇒ 文件名与老版逐字一致；
...
function writeFailed(path: string, e: unknown): never {
  ...
  throw new ScheduleRenderError('SCHEDULE_HTML_TOO_LARGE',
    '[skill-schedule] HTML 落盘失败：' + path + '（' + (code ? code + '：' : '') + detail + '）');
}
 ...
 *    **不吃复用窗口**（逐字落点＝说哪落哪）；
 *  - `targetDir`（落点**目录**的绝对路径；相对路径也收，写前归一为绝对）＋ `stem`（文件名主体）→
 *    **独占创建 ＋ 递补**（#128）；带 `reuseMs`（>0）时改为**窗口内复用**（#245）：已有那份不超龄
 *    就返回它、不新建；
 *  - 两者都给时 `explicit` 优先（用户指定胜过默认落点；**逐字覆盖写，不带时间戳、不递补**）。
 *  ⚠️ **时间戳只在共用件里算一次**：调用方给的是「目录 ＋ 主体」，**不**把「带戳的初候选路径」传进来
 *  ——那样共用件会用「带戳的名字」当主体再拼一次时间戳（实测会落出
