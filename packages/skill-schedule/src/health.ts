 ...
 * ＋ 作息特有 3 条（飞书 CLI／分类允许清单／包内模板＋公共组件）。
...
export function buildScheduleHealthReport(): ScheduleHealthReport {
  ...
  // ⑦ 分类允许清单（作息特有）：技能根那份 `category_whitelist.yaml` 在＝绿；不在＝红。
  // action 逐字照抄老技能报告页那一条（`scripts/setup_scenarios.py:230-231`）。
  const whitelistFile = join(packageRoot(), 'category_whitelist.yaml');
  const whitelistExists = existsSync(whitelistFile);
  items.push({
    id: 'whitelist.file', title: '分类允许清单',
    status: whitelistExists ? 'green' : 'red',
    message: whitelistExists ? '在：' + p(whitelistFile) + '。' : '不在：' + p(whitelistFile) + '。',
    action: whitelistExists ? '' : '缺失 category_whitelist.yaml,请检查技能目录完整性',
  });

  // ⑧ 包内模板 ＋ 公共组件（作息特有）：两处都在＝绿；缺一处＝红。
  const templatesDir = join(packageRoot(), 'templates');
  const sharedDir = join(packageRoot(), '..', '公共组件');
  const missingAssets: string[] = [];
  if (!existsSync(templatesDir)) missingAssets.push('模板目录 ' + p(templatesDir));
  if (!existsSync(sharedDir)) missingAssets.push('公共组件 ' + p(sharedDir));
  items.push({
    id: 'templates.dir', title: '包内模板＋公共组件/',
    status: missingAssets.length === 0 ? 'green' : 'red',
    message: missingAssets.length === 0
      ? '两处都在：' + p(templatesDir) + '、' + p(sharedDir) + '。'
      : '不在：' + missingAssets.join('；') + '。',
    action: missingAssets.length === 0 ? '' : '技能包装得不完整：重装这个技能包，或跑一次它的构建。',
  });
