 ...
 * **预检与形状表之前**拦下来（读写配置不该要求库目录已配、也不需要形状表里有它）。
 *
...
export const CONFIG_KEYS = {
  read: 'schedule.config.read',
  write: 'schedule.config.write',
  reset: 'schedule.config.reset',
} as const;
...
/** 本技能的 envelope skill 名（与 `render/envelope.ts` 的 `createEnvelope({skill:'schedule'})` 同值）。 */
const SCHEDULE_SKILL = 'schedule' as const;
...
function envelope(key: string, shape: EnvelopeShape, data: unknown): string {
  return JSON.stringify({ version: ENVELOPE_VERSION, skill: SCHEDULE_SKILL, shape, key, data });
}
...
export function runConfigKey(key: string, params: Record<string, unknown>): string {
  ...
  if (key === CONFIG_KEYS.write) {
    ...
    const current = withHumanError(() => loadScheduleConfig()).values as unknown as ConfigRecord;
    const merged = mergeValues(current, raw as ConfigRecord);
    const r = withHumanError(() => saveScheduleConfig(merged));
    return envelope(key, 'receipt', { path: r.path, values: merged });
  }
  const r = withHumanError(() => resetScheduleConfig());
