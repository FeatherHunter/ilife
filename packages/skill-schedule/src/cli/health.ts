 ...
 *
 * 为什么这个 key 与三个配置命令同住 `cli/config.ts` 的旁边、而不进唤醒词那张表：那套登记管的是
 * **唤醒词命令**（短语→key→形状→示例），要算进 HELP 与唤醒词计数。体检是**设置页专用**的只读出口
...
import { buildScheduleHealthReport } from '../health.js';

/** 体检命令名（插件侧镜像同值，见 `packages/plugin-schedule-ilife/src/bridge.ts`）。 */
export const HEALTH_CHECK_KEY = 'schedule.config.check' as const;

/** 本技能的 envelope skill 名（与 `src/render/envelope.ts` 的 `createEnvelope({skill:'schedule'})` 同值）。 */
...
export function runHealthCheckKey(key: string): string {
  const report = buildScheduleHealthReport();
  const envelope = {
