...
- 技能发版脚本：`scripts/wizard-publish.sh` —— 发 `skill-schedule@0.2.0`（硬前提 `base-paint@0.3.2` 已在 registry，第 1 stage 自动查）。
- 插件发版脚本：`packages/plugin-schedule-ilife/scripts/wizard-publish.sh` —— 发 `dsh-schedule-ilife@0.2.0`。它住插件自己的目录（发谁的包，脚本就住谁的家）；硬前提是技能已落 registry（插件精确 pin 技能版本，wizard 第 1 stage 自动拦）。
