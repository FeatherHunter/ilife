...
# ──────────────────────────────────────────────────────────────────────────
# STAGES — 作息管家技能线发版（版本号已定死，只走交互式认证）。
...
banner "发布 skill-schedule $V_SKILL（1 包，你扫码）"
...
if (j.version !== '$V_SKILL') { console.error('  ✗ skill-schedule = ' + j.version + '（期望 $V_SKILL）—— 停下来找编排者'); process.exit(1); }
console.log('  ✓ skill-schedule = ' + j.version);
console.log('  skill-schedule → base-paint = ' + j.dependencies['base-paint'] + ' ／ base-link-core = ' + j.dependencies['base-link-core']);
"
...
say "registry 现状（官方源）：skill-schedule 线上: $(reg_ver skill-schedule)；HEAD: $(git rev-parse --short HEAD)"

say "仓库自带的发版前置门（check-publish --pre）："
if node tooling/check-publish.mjs --pre --only skill-schedule,base-paint; then
...
step "清单里必须有 dist/cli/cmd_read.js（唯一出口）、SKILL.md、templates/*.html（含 help.html）。"
...
if already skill-schedule "$V_SKILL"; then
  ...
  pause "看到 + skill-schedule@$V_SKILL 且浏览器已批准？按回车继续"
  cd "$ROOT"
fi
...
say "  skill-schedule : $SB（期望 $V_SKILL）"
say ""
say "skill-schedule 在 registry 上的依赖（都不许出现 workspace:）："
npm view "skill-schedule@$V_SKILL" dependencies --registry="$REG" --prefer-online 2>/dev/null | sed 's/^/    /' || warn "  查不到（registry 可能同步延迟，等 1 分钟重跑本 stage）"
if npm view "skill-schedule@$V_SKILL" dependencies --registry="$REG" --prefer-online 2>/dev/null | grep -qi 'workspace'; then
...
if [[ "$SB" == "$V_SKILL" ]]; then
  ...
  say "版本号对上 —— 技能发布成功。"
  say ""
  say "下一步：跑插件 wizard：packages/plugin-schedule-ilife/scripts/wizard-publish.sh（dsh-schedule-ilife 精确 pin skill-schedule@$V_SKILL，必须等这一版落 registry 才能发）。"
else
