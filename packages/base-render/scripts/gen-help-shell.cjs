...
L.push('}');
L.push('');
L.push('/** 文档标题＝`skill_name · title`；`title` 已自带技能名时**不再重复前缀**（#145）。');
L.push(' *');
L.push(" * 背景：#141 把 `<title>` 从原型水印改成「由 5 键派生」，规则写作 `skill_name + ' · ' + title`。");
L.push(' * 但**老世代口径要求大标题含技能名**（老 `render_help.py` 自述的 #303 验收反馈：技能名应在大字行，');
L.push(' * eyebrow 小字不算），于是饼干记账这类技能的 `title` 本身就是 `饼干记账 · 使用手册(HELP)`——');
L.push(' * 照拼即得 `饼干记账 · 饼干记账 · 使用手册(HELP)`（标签页重复）。');
L.push(' * 故此处只加一条**结构性**判断：`title` 含 `skill_name` ⇒ 直接用 `title`；卡路里（`title` 不含技能名）');
L.push(' * 走原分支，输出逐字节不变。 */');
L.push('export function composeDocTitle(data: { readonly skill_name: unknown; readonly title: unknown }): string {');
L.push('  const skill = String(data.skill_name);');
L.push('  const title = String(data.title);');
L.push("  return title.includes(skill) ? title : skill + ' · ' + title;");
L.push('}');
L.push('');
...
L.push('  const title = escapeTitleText(composeDocTitle(data));');
...
T.push('});');
T.push('');
T.push("test('#145 文档标题：title 不含技能名时前缀；已含则原样（老世代大标题含技能名，禁重复）', () => {");
T.push("  const docTitleOf = (html) => html.slice(html.indexOf('<title>') + 7, html.indexOf('</title>'));");
T.push("  assert.equal(composeDocTitle({ skill_name: '卡路里', title: '唤醒词速查台' }), '卡路里 · 唤醒词速查台');");
T.push("  assert.equal(composeDocTitle({ skill_name: '饼干记账', title: '饼干记账 · 使用手册(HELP)' }), '饼干记账 · 使用手册(HELP)');");
T.push("  assert.equal(docTitleOf(renderHelpShellHtml(FIXTURE)), '卡路里 · 唤醒词速查台', '既有形状逐字节不变');");
T.push("  const bill = renderHelpShellHtml({ ...FIXTURE, skill_name: '饼干记账', title: '饼干记账 · 使用手册(HELP)' });");
T.push("  assert.equal(docTitleOf(bill), '饼干记账 · 使用手册(HELP)', '自带技能名不重复前缀');");
T.push('});');
T.push('');
