 ...
 *  它不在原型墙那三套里，是 #950「新增三套皮肤」三席中的一席
 *  （另两席：`terminal` 终端暗色／`blueprint` 蓝图工程）。
 *
 *  **故意与另两套新皮肤差在哪**（这是这套存在的理由，不是边角差异）：
 *   ① 对 `terminal`：那套是**暗底亮字**（底比字深），本套是**亮底暗字**；两者在「字面」这一项上正好相反
 *      ——它走等宽与高饱和的机器腔，本套走衬线与低饱和的纸面腔。
 *   ② 对 `blueprint`：那套的骨架是**网格与制图线**（几何、正交、高饱和蓝），本套的骨架是**笔锋与留白**
 *      （宋体的粗细对比、字距、发丝线）；三套新皮肤里**只有本套的强调是一个暖红**。
 *   ③ 三套新皮肤里**只有本套把数字交给衬线**（`font-num` 与 `font-display` 同栈）：读数像写在纸上的款识，
 *      不像仪表盘或图纸上的刻度。
 *  与既有三套的区别也在两处：比 `paper` 更"旧纸"（宣纸黄调更重、朱砂印刷红而非小票砖红、发丝线更实一档）；
 *  比 `broadsheet` 多一个**有色的强调**（那套的强调是墨黑，序列只能靠深浅与线型区分）。
 *
