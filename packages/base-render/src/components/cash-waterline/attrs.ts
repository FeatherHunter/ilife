...
/** 横轴最多出几枚刻度字（**算出来的**：多了挤在一起＝压字；日期本身在点名那块里一个不少）。
 *  三条不变量（判据逐长度断）：**① 枚数 ≤ 这个数**（对所有 1–31 天的长度都成立）；
 *  **② 首尾那两天必出**；**③ 今天必占一枚**（今天落在哪一列都写得出「今天」）。 */
export const CASH_WATERLINE_MAX_AXIS_LABELS = 6;

/** 横轴刻度字的长度上限（字符）：那一格只有一格柱子那么宽，长标签会在窄档压到隔壁。
 *  日期写法（`09-21`／`2026-09-24`）都在范围内；更长的请调用方自己截短（本件不替你猜）。 */
export const CASH_WATERLINE_MAX_LABEL_CHARS = 16;
...
export interface CashWaterlineDay {
  /** 日期（「09-21」）：上屏在横轴与告急日点名里；**非空**，且**至多 `CASH_WATERLINE_MAX_LABEL_CHARS` 个字符**。 */
