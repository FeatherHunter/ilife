 ...
 *  `buildDragSortJs()`（运行时段：点选拿起放下取消 ＋ 拖拽同一份状态；按钮档另有选中与两半控件）。
 ...
 *  `DRAG_SORT_MOVE_ATTR`。
 *  闭集与几何：`DRAG_SORT_FORMS`（两档：`lift`＝拖拽中／`buttons`＝按钮排序）／
 *  `DRAG_SORT_TOUCH_PX`／`DRAG_SORT_MOVE_MIN_PX`／`DRAG_SORT_ROW_MIN_PX`／`DRAG_SORT_GAP_PX`。
 *  文案：`DRAG_SORT_TEXT`（**会过屏的每一句的唯一定义地**）＋ `dragSortText(名, 值)`（取整句；
 *  渲染期与运行时段读的是同一个它——运行时段把它烘成产出 JS 里的同名函数）。
 *  三条事件：`DRAG_SORT_EVENT_PICK`（拿起／选中）／`DRAG_SORT_EVENT_DROP`（放下／挪位）／
