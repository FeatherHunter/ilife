...
export const SMALL_MULTIPLES_SLOTS = [
   ...
   *  线在刻度中线之上时另带 `is-under`（那枚标注画到**线下**，保证标注也在柱阵框内）。 */
  'mean',
 ...
 *  喂进那个映射的值一律夹在轴域内（均值也一样）⇒ 柱与均值线都在柱阵框内。 */
...
export interface SmallMultiplesPeriod {
   ...
   *  **与别的期间同名一律报错**：期间名是这一列的坐标（写在轴上、也进无障碍名），
   *  两列同名，那张图就答不了「哪一列是谁」。 */
  readonly label: string;
  /** 这一期的读数（**有限数**；**负读数照收**——差值型读数就长这样，归一化只看这组数的最低与最高）。
   *  本件**不认缺值**：某一期没数就别给这一期（见 README 常见错法）。 */
