...
/* 交互与流程（同一批）的第七件：`kanban-columns` —— 88 看板列，**只落 A 一档**：
   `status` 按状态分列（含一次挪动与一个空列），本轮 4/4/4（B 档列内二级组只 3/3/3，不落）。
   形态键住 `KANBAN_COLUMNS_FORMS`（英文键，**不许用 A 当键**）；触屏上拖拽不是唯一通路：
   点卡拿起 ＋ 点目标列的收纳键挪动、另有取消与改选；窄档改分段切换一列一屏。
   注：这里**不写「族」字**，也**不加汇总入口**（同前几件的口径）。
   **只加行、不重排**（并行席位的约定）。 */
