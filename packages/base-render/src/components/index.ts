...
/* 交互与流程（同一批）的第六件：`drag-sort` —— 83 拖拽排序清单，**两档都落**：
   `lift` 拖起行（原位空槽 ＋ 落点粗线通栏）／`buttons` 按钮式上下移（两半控件，把手只留字形，
   选中行站起来，按下去之前先看得到**落点预告**）。用户在墙上给这两档逐格 4/4/4（B 档补落）；
   形态键住 `DRAG_SORT_FORMS`（英文键，**不许用 A／B 当键**）；触屏上"按住拖"不是唯一通路：拿起与放下都有可点路径。
   注：这里**不写「族」字**，也**不加汇总入口**（同前几件的口径）。
   **只加行、不重排**（并行席位的约定）。 */
export * from './drag-sort/index.js';
/* 交互与流程（同一批）的第七件：`kanban-columns` —— 88 看板列，**两档都落**：
   `status` 按状态分列（含一次挪动与一个空列）／`grouped` 按位置分列且列内有二级组
   （组竖边 3px 常量、组名下一道 1px 发丝线、物件行不再各挂竖边——那 2px 改成透明槽位）。
   用户在墙上给这两档逐格 4/4/4（A 档本轮、B 档补落）；形态键住 `KANBAN_COLUMNS_FORMS`（英文键，**不许用 A／B 当键**）。
   触屏上拖拽不是唯一通路：点卡拿起 ＋ 点目标列的收纳键挪动、另有取消与改选；窄档改分段切换一列一屏。
   注：这里**不写「族」字**，也**不加汇总入口**（同前几件的口径）。
   **只加行、不重排**（并行席位的约定）。 */
export * from './kanban-columns/index.js';
/* 交互与流程（同一批）的第八件：`reminder-setter` —— 提醒设置，**两档都落**：
   卡式（`--shadow` ＋ `--hair` 隔线）＋「选中那一个点的三个决定」铺 `--soft` 成唯一焦点面板；
   单选胶囊给勾选留固定 14px 槽位（未选也留，一排左右沿齐平）；刻度一条贯穿基线，选中点牵引线接到尺子。
   第二档 `track` 摆的是**一天里好几条提醒**（点刻度上那枚点 / 点图例那一行改它，面板里那行「时间」可改）。
   用户在墙上给这两档逐格 4/4/4。形态键住 `REMINDER_SETTER_FORMS`（英文键，**不许用 A／B 当键**）。
   落地口径：一屏只留一层话——原型里那些旁白／口径句一律不上屏（用户对上一批打 2 分的理由就是「文本量太大」）。
   注：这里**不写「族」字**，也**不加汇总入口**（同前几件的口径）。
   **只加行、不重排**（并行席位的约定）。 */
export * from './reminder-setter/index.js';
/* 交互与流程（同一批）的第九件：`quick-capture` —— 快速录入条，**两档都落**：
   `oneline` 一行式录入 ＋ 解析预览（哪格认错了点哪格改）／`drawer` 常驻一行 ＋ 推开的六格抽屉
   （点「分开填」摊开，点哪格改哪格——解析不出来的字段多时用它）。用户在墙上给这两档逐格 4/4/4。
   形态键住 `QUICK_CAPTURE_FORMS`（英文键，**不许用 A／B 当键**）；触屏上每一格都可点开改，
   窄档「存」换到第二行也不横溢。落地口径：一屏只留一层话——原型旁白（「解析成」等）一律不上屏
   （同一批三格被打 2 分的理由就是「文本量太大」）；「解析」按铁律留在技能侧，件只吃已拆好的 `cells`。
   注：这里**不写「族」字**，也**不加汇总入口**（同前几件的口径）。
   **只加行、不重排**（并行席位的约定）。 */
