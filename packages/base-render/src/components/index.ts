...
/* 交互与流程（同一批）的第八件：`reminder-setter` —— 提醒设置，**只落 A 一档**：
   卡式（`--shadow` ＋ `--hair` 隔线）＋「选中那一个点的三个决定」铺 `--soft` 成唯一焦点面板；
   单选胶囊给勾选留固定 14px 槽位（未选也留，一排左右沿齐平）；刻度一条贯穿基线，选中点牵引线接到尺子。
   用户在墙上给这一档逐格 4/4/4（B 档只 3/3/3，不落）。形态键住 `REMINDER_SETTER_FORMS`（英文键，**不许用 A 当键**）。
   落地口径：一屏只留一层话——原型里那些旁白／口径句一律不上屏（用户对上一批打 2 分的理由就是「文本量太大」）。
   注：这里**不写「族」字**，也**不加汇总入口**（同前几件的口径）。
   **只加行、不重排**（并行席位的约定）。 */
export * from './reminder-setter/index.js';
/* 交互与流程（同一批）的第九件：`quick-capture` —— 快速录入条，**只落一档**：
   `oneline` 一行式录入 ＋ 解析预览（哪格认错了点哪格改）。用户在墙上给这一档逐格 4/4/4。
   形态键住 `QUICK_CAPTURE_FORMS`（英文键，**不许用 A 当键**）；触屏上每一格都可点开改，
