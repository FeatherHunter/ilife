...
export const GAP_BAND_SLOTS = [
  ...
  'yticks',
  /** 一枚纵轴刻度（只有最高那一枚带单位；**每枚都绝对定位在它自己那个值的位置上**）。 */
  'ytick',
  /** 刻度列的隐形撑子（与刻度同字逐行一份：各枚刻度绝对定位后列里没有在流内容，列宽由它撑住）。 */
  'yticks-sizer',
...
/** 刻度枚数上限（＝目标 ＋ 2）：**枚数先夹到它再进循环**，迭代次数恒由常量定、不由数据定。
 *  为什么要有这一条：`(hi − lo) / step` 是拿数据算出来的（`niceAxis` 在两头取整时各会多吞一枚），
 *  不夹的话「今天有一笔 1e21 的读数」这类输入会让 `tickValues()` 的 `for` 循环按数据长度跑
 *  （先例 `spread-dist/attrs.ts` 的 `SPREAD_DIST_MAX_TICKS`，同一个坑）。 */
export const GAP_BAND_MAX_TICKS = GAP_BAND_AXIS_TICKS + 2;
