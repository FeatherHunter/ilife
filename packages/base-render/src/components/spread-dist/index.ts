 ...
 *  `SPREAD_DIST_FORMS`（形态闭集：箱线／逐日范围柱／分位尺）／`SPREAD_DIST_CLASS` 与 `spreadDistSlot()`（标记契约）／
 *  `SPREAD_DIST_DAYS_PX`／`SPREAD_DIST_NARROW_DAYS_PX`／`SPREAD_DIST_NARROW_PX`／`SPREAD_DIST_TRACK_PX`
 *  （几何常量：判据拿它对各档几何）。
 *
