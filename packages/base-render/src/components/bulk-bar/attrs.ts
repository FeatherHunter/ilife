...
export const BULK_BAR_SLOTS = [
   ...
   *  横向按按钮的内距外扩 ⇒ 它这块正好是按钮的可用宽度；出流 ⇒ 一个字都不参与按钮的固有宽
   *  ——不这样，「正在 ＋ 按钮字」会比静息态那枚长，把动作排顶成两行（390 档实测四枚动作被挤成 3＋1）。
   *  换字时按钮尺寸纹丝不动（正常那枚 `visibility:hidden` 仍占着它的位置）。 */
  'busy',
  ...
  'sum',
  /** 取消（收起确认面，**看得见、点得到的按钮**；页脚那枚，另带 `is-plain` 视觉档）。 */
  'cancel',
  /** 改这 N 条（派发事件；本件不写库。页脚那枚，另带 `is-primary` 视觉档）。 */
  'submit',
] as const;
