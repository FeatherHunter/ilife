...
/** 一枚动作。 */
function actionOf(value: unknown, at: number, seen: Set<string>, unit: string): BulkBarActionModel {
  ...
  return {
    ...
    disabled: optBool(raw.disabled, field + '.disabled') === true,
...
export function normalizeBulkBar(input: unknown): BulkBarModel {
  ...
  const actions = actionsRaw.map((one, i) => actionOf(one, i, seenActions, countUnit));
  ...
  const model: BulkBarModel = {
    ...
    hint: optText(raw.hint, 'bulk-bar: input.hint'),
