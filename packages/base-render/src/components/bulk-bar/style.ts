...
/** 动作按钮的左右内距（px）。**忙碌那枚字也读它**：它靠这个数把自己撑到按钮的整幅宽度
 *  （负外扩 = 内距），于是「忙碌字比标签长」这件事只在按钮内部消化——两侧都不越出按钮，
 *  也就不会给动作排或页面添出横向溢出。两处必须同源，改一个就得改另一个。 */
const ACT_PAD_X_PX = 14;
...
export function bulkBarCss(input?: { readonly prefix?: string }): string {
  ...
  return [
    ...
    '  margin-top: 10px;',
    '  padding: 10px 12px;',
    '  /* **不透明**：粘住时它盖在条目上，用实底 ＋ 一条**强调色**上边线 ＋ 皮肤自己的投影档，',
    '     不用半透明（那会把底下的读数糊成一层灰）。上边线取 `accent`（不是普通发丝线）：它是这条',
    '     操作条与「选中了哪些条目」同源的记号——原型那条 3px 零模糊外圈在纸上留不下来，改由它承担。 */',
    ...
    '   一个字都不参与按钮的固有宽。为什么：忙碌那枚是「正在 ＋ 按钮字」，天生比静息态长；',
    '   让它参与固有宽，390 档实测四枚动作被挤成 3＋1、危险那枚独占整行（静息态根本没按过它）。 */',
    s('act') + ' {',
    '  position: relative;',
    '  display: inline-grid;',
    '  place-items: center;',
    ...
    '  max-width: 100%;',
    '  padding: 6px ' + String(ACT_PAD_X_PX) + 'px;',
    '  border: 1px solid ' + skinVar('line') + ';',
    ...
    '   它的尺寸就是按钮的尺寸来源——静息态的动作排换行只由它决定。 */',
    s('act') + ' > ' + c('label') + ' {',
    '  position: relative;',
    '  grid-area: 1 / 1;',
    '  overflow-wrap: anywhere;',
    '}',
    '/* 忙碌那枚字：**绝对定位在正常那枚字这块上**，横向按按钮的左右内距外扩 ⇒ 这一块正好是',
    '   按钮的可用宽度（`正在 ＋ 按钮字` 太长就在按钮内部折行，两侧都不越出按钮）。',
    '   出流 ⇒ 它一个字都不参与按钮的固有宽；换字时按钮尺寸纹丝不动（正常那枚 `visibility:hidden`',
    '   仍占着它的位置）。为什么不住成并排的兄弟：绝对定位的兄弟盒子落的是**网格区**，',
    '   实测比标签这一格偏右 7px、偏下 3px（两枚字对不齐）；住进标签里，基准就是标签自己这块。 */',
    s('busy') + ' {',
    '  position: absolute;',
    '  left: -' + String(ACT_PAD_X_PX) + 'px;',
    '  right: -' + String(ACT_PAD_X_PX) + 'px;',
    '  top: 50%;',
