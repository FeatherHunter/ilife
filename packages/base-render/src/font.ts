 ...
 *
 * 谁在用（写得出哪两处在用）：**help 模板**（`src/style.ts` 的 `helpShell` 区，`.ilife-help-shell` 基础）
 *  与**共享页面模板**（`src/blocks.ts` 的 `pageShell` 区，`body` 基础）。
 *  此前这份栈只写在 `style.ts` 里一处、只有 help 模板用；#179 给共享页面模板补文档级字体基础时出现**第二个用法**
