...
export function pageUiCss(input?: PageUiCssInput): string {
  ...
  return [
    ...
    '  height: auto;',
    '}',
    '/* ② 复制区债之一：没有日志可复制的页面，页尾那颗「复制日志」是**禁用态**，',
    '   必须一眼看出点不动（此前只有 disabled 属性、没有任何置灰规则，看着像能点）。',
    '   半透明档 .45 与 HELP `button.copy:disabled` 逐值同。 */',
    root + ' .' + p + 'copy-btn[disabled] {',
    '  opacity: .45;',
    '  cursor: not-allowed;',
    '}',
    root + ' .' + p + 'copy-btn[disabled]:active {',
    '  transform: none;',
    '}',
