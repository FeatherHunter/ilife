...
export function pageUiCss(input?: PageUiCssInput): string {
  ...
  return [
    ...
    '   「1280 档没把宽屏用起来」，以及编排者 2026-09-15 的补记：页壳 960 在 1280 档左右各空 140px）。',
    '   口径＝「宽屏不空荡、正文不散」：页壳从 `blocks.ts` 的 960（那儿是别票写集，本件不改它）',
    ...
    '   pageShell 区注明「不搬 HELP 那一条」），壳是 content-box ⇒ 不写它则 `max-width` 只管内容宽、',
    ...
    '  font-weight: 600;',
    '}',
    root + ' .' + p + 'block-feedback-block-note .' + p + 'block-feedback-block-note-icon::after {',
    '  font-size: 12px;',
    '  line-height: 1;',
    '}',
    root + ' .' + p + 'block-feedback-block-note-icon-ok::after {',
    '  content: "已完成";',
    '}',
    root + ' .' + p + 'block-feedback-block-note-icon-warn::after {',
    '  content: "注意";',
    '}',
    root + ' .' + p + 'block-feedback-block-note-icon-danger::after {',
    '  content: "未通过";',
    '}',
    root + ' .' + p + 'block-feedback-block-note-icon-info::after {',
    '  content: "提示";',
    '}',
  ].join(LF);
}
