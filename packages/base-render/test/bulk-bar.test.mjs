 ...
 *     空态／转义面（每个文本字段都塞一遍注入串）／**全部**非法入参分支（每个都断 `BlocksError`）；
 ...
 *  ③ **加法式**：渲染本件不改动同页别的件的产物；换前缀时 scope 与类名一起换；
 *  ④ **两档几何（真机 headless Chrome ＋ CDP）**：**容器**宽 390 与 1280 下零横向溢出、
 *     条目与操作条**不重叠**、命中盒 ≥44×44、相邻触控目标间距 ≥8px、关键语义零截断、
 *     四套皮肤下标记逐字节相同；**起不来就退确定性几何判据并打印原因**；
 *  ⑤ **行为（真机）**：勾选变化 → 已选数原地改写 ＋ 条按 0 条 `hidden` ＋ 尾段读数换 `—`（勾回原样则复原）
 *     ＋ 派发 `ilife:bulk-change`；点带预演的动作 → 就地展开确认面；「最近用过」填值；
 *     「取消」收起并把焦点还给按钮；「改这 N 条」派发 `ilife:bulk-action`（带 keys 与 value）。
 *
 * 期望值一律从组件自己的常量派生（`BULK_BAR_*`），不抄字面量：改了名字这里跟着红。
 */
...
const confirmOf = (html, key) => {
  const head = bulkBarSlot('confirm') + '" id="ilife-bulk-confirm-ledger-bulk-';
  const start = html.indexOf(head + key);
  assert.ok(start > 0, '找不到动作 ' + key + ' 的确认面');
  const next = html.indexOf(head, start + 1);
...
describe('bulk-bar ① 渲染契约 · 形态 A 骨架', () => {
  ...
  it('操作条：已选数 ＋ 单位 ＋ 尾段读数；条与提示都在；带预演的动作有 `aria-controls`', () => {
    ...
    assert.ok(html.includes(bulkBarSlot('hint') + '" id="ilife-bulk-hint-ledger-bulk">共 5 条'), '提示那句');
    assert.equal(countOf(html, ' aria-controls="ilife-bulk-confirm-ledger-bulk-'), 2, '只有带预演的两枚动作指确认面');
  ...
  it('错态与忙碌态：错那句写在按钮旁边（`aria-describedby` 指它）；忙碌时**两枚字都在**（宽度锁住）', () => {
    ...
    assert.ok(err.includes('id="ilife-bulk-error-ledger-bulk-cat" role="alert">改分类：这批里有 1 条没改成（网络断了）</p>'));
    assert.ok(err.includes(' aria-describedby="ilife-bulk-error-ledger-bulk-cat"'), '按钮指到那句错');
    assert.ok(err.includes('data-ilife-bulk-busy="1"'), '忙碌标记');
    assert.ok(err.includes(' aria-busy="true"'), '忙碌的语义面');
    assert.ok(err.includes('<span class="' + bulkBarSlot('label') + '">删除</span>'), '正常态那枚字也在（宽度锁住）');
  ...
  it('入参违规一律拒（不静默降级）：形态／清单／动作／预演逐条', () => {
    ...
    assert.equal(throwsBlocks(() => renderBulkBar({ ...ok, actions: [{ key: 'a', label: '改', preview: { title: '改', rows: new Array(51).fill({ keep: true, to: 'x' }) } }] })), true,
      '预演条数超上限');
    assert.equal(throwsBlocks(() => renderBulkBar({ ...ok, items: new Array(201).fill({ key: 'k', title: 't' }) })), true, '条目数超上限');
  });
...
describe('bulk-bar ② 样式与零 DOM 纪律', () => {
  ...
  it('零手写色值（兜底链那一处除外）、源码级零手写 `var(--ilife-…)`、**不拿 ink 系当面**', () => {
    ...
    for (const file of ['style.ts', 'style-column.ts', 'style-confirm.ts', 'style-sizes.ts']) {
  ...
  it('触控地板与「条不盖条目」的几何事实都写在样式里（数字只有一处出处）', () => {
    ...
    assert.ok(clean.includes('box-shadow: var(--ilife-shadow,'), '投影走皮肤档（纸面那几套是 none）');
  });
  ...
  it('状态矩阵齐：`:active` 只碰 transform、`disabled` 写得出原因、忙碌两枚字叠在一格', () => {
    assert.ok(clean.includes('transform: scale(.98)'), '真按下');
    assert.ok(clean.includes('transform 80ms'), '动效 ≤80ms');
    ...
    assert.ok(clean.includes('grid-area: 1 / 1'), '两枚字叠在一格 ⇒ 宽度锁住');
  ...
  it('运行时段产出的是 JS 文本：DOM 名只出现在那段文本里', () => {
    const js = buildBulkBarJs();
    assert.ok(typeof js === 'string' && js.length > 1000);
    assert.ok(js.includes('var doc=document;'), '运行时的 DOM 入口只有这一处别名');
...
function assertStaticGeometry(css, html) {
  ...
  assert.ok(html.includes('hidden'), '选中不足时那条必须是 hidden 的（静态面看得出来）');
}
  ...
  it('容器 390 与 1280：零横溢／条不盖条目／命中盒 ≥44／间距 ≥8／零截断／四套皮肤标记逐字节相同', async (t) => {
    ...
    try {
      ...
      const opened = await page.ev('(function(){var p=document.querySelector(' + actSel('cat') + ');'
        + 'var panel=document.querySelector(' + JSON.stringify('#' + 'ilife-bulk-confirm-ledger-bulk-cat') + ');'
      ...
      const closed = await page.ev('(function(){var panel=document.querySelector('
        + JSON.stringify('#' + 'ilife-bulk-confirm-ledger-bulk-cat') + ');'
