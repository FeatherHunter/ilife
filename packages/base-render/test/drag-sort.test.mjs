 ...
 *     **起不来就退确定性几何判据并打印原因**；
 *  ⑤ **行为（真机）**：拿起（点把手 → 三样出来 ＋ 状态句 ＋ `pick` 事件）／
 *     放下（点另一行 → 全序真动 ＋ 位置读数整列重写 ＋ `drop` 事件带新 `keys`）／
 *     取消（点取消键／再点同一行 → 顺序不动 ＋ `cancel` 事件）；四套皮肤下标记逐字节相同。
 *
...
describe('drag-sort ⑤ 行为（真机：拿起／放下／取消）＋ 四套皮肤同构', async () => {
