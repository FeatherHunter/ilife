...
//   ③ `max-width` 住在基座规则里（恰一条 —— `ui-fix-154` 那条守卫的口径）且没推翻 #507／上一票的数值列与行首列；
...
describe('#512 ② 数据表宽度上限（宽档才触发）', () => {
  it('基座规则恰一条，且带 `max-width: 680px`（挂在**表卡**上，不是表/单元格上）', () => {
    const decls = declsOf(CSS, TABLE_BASE);
    assert.equal(declValue(decls, 'max-width'), '680px', '值写死：宽档表宽上限，窄档天然不触发');
