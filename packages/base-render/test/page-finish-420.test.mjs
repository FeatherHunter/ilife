...
//   · D2：原先拿测试内硬编码整页字面量当基线（旧常量名 TODAY＋_SHELL），属自证。基线改从 `dc16ba1^`
...
/** 跳过注释；命中返回跳过后的下标，否则 `-1`（注释一律不参与判定）。 */
...
/** 配对花括号：`from`＝开括号下标，返回匹配的闭括号下标（不成对返回 `-1`）。 */
function matchBrace(text, from) {
  ...
  for (let i = from; i < text.length; i += 1) {
    ...
    else if (ch === '/' && text[i + 1] === '*') { const skip = skipComment(text, i); i = skip < 0 ? i : skip - 1; }
    else if (ch === '{') depth += 1;
    else if (ch === '}') {
      depth -= 1;
      if (depth === 0) return i;
...
/** 取 `@media print` 段的**段内正文**（不含外层 at-rule 头与那对花括号）。 */
function printBody(css) {
  ...
  assert.ok(close > open, '@media print 段花括号不配对');
  return css.slice(open + 1, close);
}

/**
 * 把**段内正文**解析成逐条规则 `{ prelude, body }`（#428 D1）。
 * 只认 `前导符 { … }` 结构：前导符读到首个括号；注释／字符串里的花括号一律不算；
 * 声明块（只有 `;` 没有 `{`）不进列表。段内不许嵌套 at-rule，故不递归。
 */
function rules(text) {
  const out = [];
  let i = 0;
  while (i < text.length) {
    ...
    for (let k = i; k < text.length; k += 1) {
      ...
      else if (c === '/' && text[k + 1] === '*') { const s2 = skipComment(text, k); k = s2 < 0 ? k : s2 - 1; }
      else if (c === '(' || c === '[') nesting += 1;
      else if (c === ')' || c === ']') nesting -= 1;
      else if (nesting === 0 && c === '{') { preludeEnd = k; break; }
      else if (nesting === 0 && c === '}') { preludeEnd = k; break; }
    }
    ...
    if (text[preludeEnd] === '}') {
      i = preludeEnd + 1;
      if (prelude !== '') out.push({ prelude, body: '' });
      continue;
    }
    const close = matchBrace(text, preludeEnd);
    assert.ok(close > preludeEnd, '段内规则花括号不配对：' + JSON.stringify(prelude));
    out.push({ prelude, body: text.slice(preludeEnd + 1, close) });
    i = close + 1;
  }
  return out;
}
...
// 取 `dc16ba1^` 的 `blocks.ts`（原始字节 → 剥类型 → 相对导入改指当刻 `dist/` → 跑真产物）得到，
// 共享的 `packages/base-render/src/blocks.ts` 与 `dist/` 全程一行未改
// （镜像窗口读数见 `docs/base/base-render/t428-判据硬化.md` §2）。
// 本测试**不写死任何整页 HTML**：真值一律从夹具读，且夹具自证两件事——
//   ① 它记的基线提交 == 当刻仓库解的 `dc16ba1^`（本件不记死提交号，照当刻仓库解）；
//   ② 它记的源码 blob 摘要 == 当刻仓库里该提交的 `blocks.ts` 字节摘要（sha256 ＋ git blob sha1）。
// 篡改夹具会被 ①／② 当场抓住，指到别的提交也过不了 ⇒ 基线可追溯、不自证。
// 夹具真值的取法（可复跑）：`.scratch/t428/mirror-window.mjs` 取 `git show dc16ba1^:…blocks.ts`
// 的原始字节，剥类型（`node:module` 的 `stripTypeScriptTypes`）＋把相对导入改指当刻 `dist/`，
// 得到一份**字节同源**的可执行镜像，`gen-baseline.mjs` 跑它取真值；共享源码一行未动。
// 加上这一层：「真值＝基线提交那份代码跑出来的」，而不是本件里抄的一段字面量。
...
describe('#428 可追溯基线（打印版式基线的来源与自证检查）', () => {
  ...
  it('本件不再留任何硬编码整页字面量（夹具是唯一来源）', () => {
    ...
    assert.ok(!source.includes('TODAY' + '_SHELL'), '旧的硬编码基线常量必须已删除');
  });
});
...
describe('#420 blocksCss 打印段（作用域纪律，按 #428 硬化）', () => {
  const printRules = () => rules(printBody(blocksCss())).filter((rule) => !rule.prelude.startsWith('@'));

  it('含 @media print 与具名 @page，且隐藏页内导航与复制区', () => {
    const css = blocksCss();
    const body = printBody(css);
    assert.ok(css.includes('@media print'), '缺 @media print：' + css.slice(0, 40));
    assert.ok(body.includes('@page printable'), '缺具名页规则（@page printable）');
    assert.ok(new RegExp('\\.' + CLASS + '\\s+\\{[^}]*page:\\s*printable').test(body), '打印页名必须由 .ilife-page-printable 绑定');
    assert.ok(new RegExp('\\.' + CLASS + '\\s+\\.ilife-block-toc\\s*\\{[^}]*display:\\s*none').test(body), '打印时必须隐藏页内导航');
    assert.ok(new RegExp('\\.' + CLASS + '\\s+\\.ilife-block-copy-block\\s*\\{[^}]*display:\\s*none').test(body), '打印时必须隐藏复制区');
  });
  ...
  it('打印段的 at-rule 只允许具名页 @page printable（裸 @page 与嵌套 at-rule 皆 0 处）', () => {
    const body = printBody(blocksCss());
    const atRules = rules(body).filter((rule) => rule.prelude.startsWith('@'));
    assert.ok(atRules.length >= 1, '打印段缺 @page（页边距无处承载）');
    const names = [];
    for (const rule of atRules) {
      const name = pageName(rule.prelude);
      names.push(name);
      assert.equal(rule.prelude.trim().split(/\s+/)[0], '@page', '打印段只允许 @page：' + rule.prelude);
      assert.equal(name, 'printable', '必须是具名页 @page printable：' + JSON.stringify(rule.prelude));
      assert.ok(!/[,{]/.test(name), '页名不许带并列或选择器：' + JSON.stringify(name));
    }
    assert.deepEqual(names, ['printable'], '打印段 @page 只许恰一条具名页');
    assert.ok(!/(^|[},\s])body\s*\{/.test(body), '打印段不得出现裸 body 选择器');
    assert.ok(!body.includes('.wrap'), '打印段不得出现裸 .wrap 选择器');
  });
