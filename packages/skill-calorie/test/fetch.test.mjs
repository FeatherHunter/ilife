...
test('CLI：import/history/audit exit 码 parity', () => {
  ...
  const f = tmp('c.jsonl');
  writeFileSync(f, JSON.stringify(GOOD));
  const out = execFileSync(process.execPath, [cli, 'import', f, '--db', dbp, '--on-duplicate', 's'], { encoding: 'utf8' });
  assert.match(out, /新增: 1/);
  assert.throws(() => execFileSync(process.execPath, [cli, 'validate'], { stdio: 'pipe' }));
  const h = execFileSync(process.execPath, [cli, 'history', '--db', dbp, '--days', '7'], { encoding: 'utf8' });
  assert.match(h, /无记录/);
  let auditErr = null;
  try {
    execFileSync(process.execPath, [cli, 'audit', '--db', dbp, '--catalog', join(tmpdir(), 't25-no-cat.json')], { encoding: 'utf8', stdio: 'pipe' });
  } catch (e) {
    auditErr = e;
  }
  assert.equal(auditErr?.status, 2);
  assert.match(String(auditErr?.stdout ?? ''), /"status": "fail"/);
  assert.throws(() => execFileSync(process.execPath, [cli, 'audit', '--db', dbp, '--catalog', join(tmpdir(), 't25-no-cat.json'), '--strict'], { stdio: 'pipe' }));
});
