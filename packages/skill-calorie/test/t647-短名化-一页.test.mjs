 ...
 *
 * **本探针守的边界**（#647 交付时「一页先行」的两条边界；**#648 铺开后已整体翻转**）：
 *   ① 查营养配比页（`calorie.view.nutrition-ratio`）三条分布行名称栏**只出短名**，且范围仍在本页可读
 *      （对比表里 `10%（…）／65%（…）` 这类下限上限逐档在位）；
 *   ② 同区块的另一族调用点（`calorie.view.diet-review` 那 8 词，经 `diet/review.ts`）：
 *      #647 交付时**不传**短名开关 ⇒ 仍是老写法「蛋白（推荐 10–20%）」；**#648 已把这 8 页铺开**
 *      （`{ shortNames: true }`），故 ③④ 两条断言的期望值随之翻转成短名——它们现在的职责是
 *      「铺开别被谁改回去」的守门，逐页铺开面由 `t648-十页铺开.test.mjs` 覆盖。
 *
...
test('#647 ③ 一页边界（由 #648 铺开取代后已翻转）：复盘页现在也走短名', () => {
  const names = rowNames(REVIEW.html);
  assert.deepEqual(names, ['蛋白', '碳水', '脂肪'],
    '复盘页的名称栏不是短名——#648 的铺开坏了（本票交付时这条断言的期望值是老写法三条长标签）');
});

test('#647 ④ 一页边界（由 #648 铺开取代后已翻转）：看今日营养同样走短名', () => {
  ...
  /* 窄窗没记录时该页可能不进配比段——那样就没有分布行；有行就必须是短名。 */
  for (const n of names) {
    assert.equal(n.includes('（推荐 '), false, '看今日营养的名称栏还是老写法：' + n);
  }
});
