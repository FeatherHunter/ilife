...
function runScenario(sc) {
  ...
  const localEnv = { ...homeEnvOf(cfg), ...ctx.env };
  const ids = [];
...
test('#97 · affectedRows＝库真实行数（只读句柄独立复核，非自报）', () => {
  // 硬删：food_log 行数前后差 = affectedRows
  {
    ...
    const r = run('calorie.diet.add', { foodName: '核对2', calories: 100, protein: 1, date: '2026-09-07', time: '10:01:00' }, { ...homeEnvOf(calorieConfigDir(ctx.dir))});
  ...
  {
    const ctx = mkEnv();
    const env = { ...homeEnvOf(calorieConfigDir(ctx.dir))};
...
test('#97 · 重复跳过：affectedRows=0 且写入字段摘要为空（无写入不虚报）', () => {
  ...
  const before = countOf(ctx.dir, 'food_log');
  const r = run('calorie.diet.add', { foodName: '重复项', calories: 200, protein: 35, date: '2026-09-06', time: '12:10:00' }, { ...homeEnvOf(calorieConfigDir(ctx.dir))});
  assert.equal(r.status, 0);
  const rc = JSON.parse(r.stdout).data.receipt;
...
test('#97 · P9：stdout 恒为一行 JSON，M5 只落字段不改 stdout 形态', () => {
  ...
  assert.equal(env.shape, 'receipt');
  assert.equal(env.data.ok, true);
  const r = run('calorie.weight.log', { kg: 70.3, date: '2026-09-06', time: '07:05:00' }, { ...homeEnvOf(calorieConfigDir(mkEnv().dir))});
  assert.equal(r.status, 0);
