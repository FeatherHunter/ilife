 ...
 *   ③ `data.metrics` 与**改前**逐字段相同（取数口径一个字不许变）——「改前」值是实施席在改代码之前
 *      用真 CLI 跑出来的读数（`.scratch/t566/before.json`）；这条是**反向锁**：正文换版式换出花来
 *      也不许动一个数，动了就红；
...
/** 3 件实跑产物；`metrics` ＝**改代码之前**真 CLI 跑出来的读数（`.scratch/t566/before.json`），逐字段冻结。 */
const CASES = [
  {
...
/* ── ③ 取数口径没被动过（反向锁：与改前读数逐字段相同） ── */

test('#566 ③ data.metrics 与改前逐字段相同，且这条件不是恒真', () => {
  for (const r of RUNS) {
    assert.equal(r.exit, 0, '「' + r.c.name + '」应 exit 0');
    assert.deepEqual(r.env?.data?.metrics, r.c.metrics,
      '「' + r.c.name + '」的 data.metrics 与改前不一致（取数口径被动了）');
  }
  assert.deepEqual(CONTROL_RUN.env?.data?.metrics, CONTROL.metrics, '对照件 data.metrics 与改前不一致');
