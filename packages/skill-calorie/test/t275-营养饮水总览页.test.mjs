...
function render(dir, key, params, extraEnv) {
  ...
  const r = spawnSync(process.execPath, [CLI, key, '--params', JSON.stringify(params), '--html', out], {
    ...
    env: { ...process.env, ...homeEnvOf(calorieConfigDir(dir)), ...freezeClock(D), ...(extraEnv ?? {}) },
  });
  return {
...
const WORDS = [
  {
    ...
    blocks: ['热量来源占比（按营养素折算）', '推荐范围对比（克数均为这 7 天合计）',
      '折算合计（千卡）', '距范围', '✓ 在范围内', '✓ 均衡',
      /* #648 · 短名化铺开：图例条名称栏只留短名（蛋白／碳水／脂肪），原来那三串「推荐 10–20%」
         长标签不再进产物（6em 窄槽装不下、会被静默裁，D1）。**推荐范围这一事实仍在页上**——
         由紧邻的「推荐范围对比」表按 下限／上限 两列逐档承担，故这里改钉那两列的读数
         （#647 已在「查营养配比」那一格走完同一步，本票把复盘这 8 页一并对齐）。 */
      '10%（', '20%（', '45%（', '65%（', '35%（'],
  ...
  },
  {
    ...
    /* 小票版（2026-09-24）：达标那一态的判语从读数卡的 `statusText` 搬进主数字头的**印章**，
       字面去掉了与「进度 100%」重复的括号百分比；「今日每杯」从两列表改**明细行**（无表头）⇒
       needle 换形状、`cols` 撤（保护的事没变：达标说得出、每杯逐条在）。 */
    blocks: ['今日进度', '本周 7 天', '今日每杯（共 1 杯）', '目标', '已完成目标'],
  ...
  },
  {
    ...
    /* **接线缺口**（归 #271）：这条词与「看最近 7 天饮食」在路由上参数一字不差，命令这一层分不出
       ...
       **复制日志那一格已接线**（小票版 2026-09-24）：这一页原先把 `command` 留空 ⇒ 日志按钮是死按钮，
       ⑥ 组按缺口断 `disabled`；同批改造把本次命令原文接进复制区（实测载荷里是
       `calorie-cmd-read calorie.view.diet --params '{"window":"7d"}'`），故 `gap` 那一格撤掉——
       ⑥ 组改走「活按钮 ＋ 第 4 段＝命令原文」那条断言。 */
...
const OWN_PAGES = [
  {
    ...
    blocks: ['热量来源占比（按营养素折算）', '推荐范围对比（克数均为这 7 天合计）',
      ...
      /* #647 · 短名化：图例条名称栏只留短名（蛋白／碳水／脂肪），原来这三串「推荐 10–20%」长标签
         不再进产物（6em 窄槽装不下、会被静默裁，D1）；**推荐范围这一事实仍在页上**——由紧邻的
         「推荐范围对比」表按 下限／上限 两列逐档承担，故这里改钉那两列的读数。
         新形状的靶向断言（名称栏三行短名 ＋ 值栏交叉核对）住 `t647-短名化-一页.test.mjs`。 */
      '10%（', '20%（', '45%（', '65%（', '35%（'],
    cols: ['营养素', '实际', '下限', '上限', '距范围', '状态'],
...
test('#275 ⑧ 原样必绿：被变异打的三处断言在未改动时成立', () => {
  ...
  /* 丙（① 逐块）：环图中心标签与图例条三行在位。
     ...
     #648 · 图例条名称栏随铺开改短名（形状见 #647／#648 证据件）：老写法那三行
     「蛋白（推荐 10–20%）」不再上屏，本条改钉短名那一行 **＋ 紧随其后的值栏**——
     `good.text` 是 `visibleText`（标签与属性全剥），故行就是文本段，用行界锚住，
     避免误命中同页 KPI 卡与对比表里的裸「蛋白」/「碳水」/「脂肪」。 */
  assert.equal(good.text.includes('折算合计（千卡）'), true, '原样产物里读不到环图中心标签');
  assert.equal(good.text.includes('蛋白（推荐 '), false, '原样产物里还有老写法的图例条标签');
  for (const [name, pct] of [['蛋白', '13%'], ['碳水', '64%'], ['脂肪', '11%']]) {
    const re = new RegExp('(^|\\n)' + name + '\\n' + pct + '(\\n|$)');
    assert.equal(re.test(good.text), true, '原样产物里读不到图例条那一行（名称＋值）：' + name + ' / ' + pct);
  }
});
