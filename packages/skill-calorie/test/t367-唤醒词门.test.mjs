 ...
 *    `test/cmd-registry-294.test.mjs:301` 对**体重域**跑；全包铺开会当场逮到 17 条**路由未回同键**行（#651 起 18→17：「卡路里HELP」补上本命令路由后出列；读数 `ROUTE-SAMEKEY off=<n>`；**注意与下面登记表的 19 行不是同一批**——17 行＝表内词但路由别键，与登记表交集仅 1）
 *    （现场读数见 `docs/skills/skill-calorie/t367-门证据.md` §九，本门只打 `ROUTE-SAMEKEY off=<n>`
 *    与逐行 `ROUTE-UNBOUND` 读数、**不判红**）——清／登记这 17 行属另一张框架票，已按票面「遗留出口」
 *    在证据里登记转票措辞，不塞进本票写集。
