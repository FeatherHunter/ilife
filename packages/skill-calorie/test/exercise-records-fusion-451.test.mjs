 ...
 * 另：三类工程话**全文**命中 0（`calorie.view.`／票号 `t\d{3}`／工序词「移植」；#921 起票号那一类扫的是
 * 去样式段的正文——页内样式段带的是公共层配方自己的记账注释，不是上屏文本），本页也不写第二份类别色表。
 *
...
function assertFusion(r, what, opts = {}) {
  ...
  // #921：本族接上 `pageUi: true`（页面级移动端配方）后，页内样式段带进了公共层配方定义地的记账注释
  // （`base-render/src/pageUi.ts` 与 `base-render/src/pageUiToast.ts` 里的 `t728`／`t849`）。票号判据的
  // 落点是**上屏文本**，取法照本件 `bodyOf()` 那段注释（拿整份产物判会把公共层算在本页账上）——
  // 实测整份 2 处命中、全在 `<style>` 段内，去样式段后 0 处。牙口未减：样式段之外任何一处票号仍必红。
  assert.ok(!/\bt\d{3}\b/i.test(bodyOf(r.file)), what + ' 产物里出现票号样式的字');
