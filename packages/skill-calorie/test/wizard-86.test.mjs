...
test('#86 身材照 wizard：纯配置＋新 CLI 命令段', () => {
  ...
  try {
    ...
    assert.ok(!bare.html.includes('srcPaths'), '#474：参数名不许进可见文本');
...
test('#86 GIF 框选器：列表＋框选＋裁剪＋命令段（无 cropper.js）', () => {
  ...
  try {
    ...
    assert.ok(!v.html.includes('>存在</td>') && !v.html.includes('>未校验</td>') && !v.html.includes('>缺失</td>'),
      '#474：正常行不该喊存在／未校验／缺失');
    // #474 表单：限制进字段名、数字框带上下限、两个枚举改下拉、坐标 JSON 撤栏改人话。
    assert.ok(!v.html.includes('{"12":[x1,y1,x2,y2]}'), '#474：坐标 JSON 那栏须撤掉');
    assert.ok(v.html.includes('要裁剪哪几张？（留空＝整图，不裁）'), '#474：裁剪栏须改人话');
    ...
    assert.ok(v.html.includes('>无限</option>') && v.html.includes('>淡入淡出</option>'),
      '#474：循环与切换效果须出中文下拉项');
