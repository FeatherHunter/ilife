...
test('#111 非法窗：start 晚于 end 即 exit 2', () => {
  const { dir } = mkPortDb();
  const r = run(BIN, 'calorie.view.exercise-trend', { start: '2026-09-07', end: '2026-09-05' }, { ILIFE_CONFIG_DIR: calorieConfigDir(dir) });
  assert.equal(r.status, 2, '非法窗未拒收（status=' + r.status + '）');
});

test('#111 命名底座可用：力量训练总览落点＋回传一致＋产物为全文档', () => {
  const { dir } = mkPortDb();
  const r = run(BIN, 'calorie.view.exercise-strength', WIN, { ILIFE_CONFIG_DIR: calorieConfigDir(dir) });
