...
// ---------------------------------------------------------------- sweep 子进程
// 单进程跑完 42 个读键：某个键若因缺参走到 fail(2) 会 process.exit，父进程靠「少了一行」发现。
...
// ---------------------------------------------------------------- 验收 ①：读键全部可用 ＋ 接线等价
test('#93 ① 42 读键在只读句柄上逐个可用，且与可写句柄 data/html 全等', () => {
  const tpl = makeTemplate();
  ...
  assert.equal(READ_KEYS.length, 42, '读键应为 42 个（77 组合键 − 35 写键）');
