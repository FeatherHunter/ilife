...
「唤醒词」列是该键的代表词（**由命令声明派生**：`gen-cli.mjs` 写 `REPR` 表，本生成器不另存第二份）；「预检页」列是该写命令**先出的预检确认页命令**（读命令与「读—确认—写」那一类留空，事实出处＝`src/body/wizardPlate.ts` 的 `WIZARD_WRITE_KEYS`）；「流程」列是它服务的工作流程名。三列的事实都住各自能力目录的声明与路由，本表由 `pnpm help:build` 生成。
