...
- 技能发版脚本：`scripts/wizard-publish.sh` —— 发 `skill-calorie@0.2.5`（硬前提 `base-paint@0.3.4` 已在 registry，第 1 stage 自动查）。
- 插件发版脚本：`packages/plugin-calorie/scripts/wizard-publish.sh` —— 发 `dsh-calorie@0.2.6`。它住插件自己的目录（发谁的包，脚本就住谁的家）；硬前提是技能已落 registry（插件精确 pin 技能版本，wizard 第 1 stage 自动拦）。
