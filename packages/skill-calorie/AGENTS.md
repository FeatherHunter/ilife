...
| `src/weight/history.ts` | — | 882 | 超因：「看体重明细」与「看体重曲线」在命令面上是同一个命令，两条子功能共用一件。 |
...
| `src/health.ts` | — | 539 | 已超线，需要根据规则进行重构。超因与拆法待补（本行由 `--sync` 自动补出，请补写超因与拆法）。 |
...
| `src/workout/land.ts` | — | 376 | 已超线，需要根据规则进行重构。超因：（**#943 引入，312→376**）这一件同时住着三组活——①「某天有几段／为什么没得落地」的取数与判据（`landSessionsOf`／`landNoSegmentWhy`／`landPlanGate` ＋ `LandPlan` 形状）；② 单日四步链的编排（`writeLand`：补计划／记心愿／推送／回写的调用、按步点名、四步读数）；③ 两条跨技能桥的真实现（`defaultLandPlan`／`defaultLandWish` 与 `LandBridgeDeps` 注入缝）。本票给它补「无事可做」那一态（`landNothing` ＋ `LAND_NOTHING_STATUS`）与逐段注释，故越线。**本次先不拆**：拆分（结构重排）不在 #943 写集（票面只给 `land.ts`／`landBatch.ts` 两件源码 ＋ 四条用例 ＋ 种子），且本票在同一件上连改三刀（缺失阻断 → 无事可做页 → 两态文案），窗口内搬件会把「同一件上三刀」的读数搅成搬迁的读数。**拆法**＝按「取数判据／编排／桥」切三件姊妹件：`workout/landDay.ts`（`LandPlan`／`landSessionsOf`／`landNoSegmentWhy`／`landPlanGate`／`LAND_NOTHING_STATUS`）与 `workout/landBridge.ts`（两条 `defaultLand*` ＋ `LandBridgeResult`／`LandBridgeDeps`）另立，本件只留 `writeLand` 与 `landNothing` 薄转出；先例照同族 `sportDocs.ts`（版式）＋`sportUi.ts`（形状与样式）那次一分为二；待收口票认领。 |
