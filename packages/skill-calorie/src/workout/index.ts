 ...
 *
 * R4 破环（#613）：表延迟到首次调用建（不用顶层 `new Map`）。训记取数口经本门读计划
 * （`xunji/planSource.ts`），而本门经 `xunjiPush.ts` 反引训记门——门级互引在 ESM
 * 求值期即闭环（`gen` 以 `commands.ts` 先入即爆 `WORKOUT_COMMANDS` TDZ）。
 * 本门只在调用时碰声明绑定，环上各件顶层亦只定义不用，环求值安全；调用时环早已解开。 */
let byKey: Map<string, CommandSpec> | null = null;
function byKeyMap(): Map<string, CommandSpec> {
  if (byKey === null) byKey = new Map<string, CommandSpec>(WORKOUT_COMMANDS.map((c) => [c.key, c]));
  return byKey;
}
