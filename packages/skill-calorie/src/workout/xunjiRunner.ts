 ...
 * `spawnSync` 把异步隔离进子进程（`dist/xunji/cli.js`，模块自己的对外面，老 `xunji_bridge` 同形），
...
/** 一次外调的结局：退出码 ＋ stdout JSON（码非 0 时为 null）＋ 是否走挡板。 */
...
function stubOf(raw: string, step: string): XunjiCall {
  ...
  return { code: o.code as number, data: (o as { data?: unknown }).data ?? null, stubbed: true };
}
...
export function invokeXunji(argv: readonly string[], step: string): XunjiCall {
  ...
  if (code !== 0) return { code, data: null, stubbed: false };
  let data: unknown;
  try {
    data = JSON.parse(String(r.stdout ?? '').trim());
  } catch (e) {
    fail(4, step + '回执解析失败（子进程退 0 但 stdout 不是 JSON）：' + (e instanceof Error ? e.message : String(e)));
  }
  return { code, data: data as unknown, stubbed: false };
}
