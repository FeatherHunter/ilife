...
const NOT_COUNTED_NOTE = '本页的完成度与缺口只算热量；上面这三项目标今天还没纳入，所以本页不给它们的完成率。';
...
export function buildGoalProgressDoc(input: GoalProgressDocInput): string {
  ...
  if (empty) {
    ...
    sections.push('<section id="' + SEC_HISTORY.id + '">' + historyTable(g, input.historyDays, day, input.end)
      // 两档窗口的转场：上面的折线是**本窗**、这张表是**达标账那个窗口**，句子里先交代清楚。
      + renderCaliberLine('上面那张折线图是窗口 ' + f.windowDays + ' 天里的每一天；下面这张表看的是近 '
        + input.historyDays + ' 天，图表两处不是同一个窗口。')
  ...
  return assembleDocPage({
    docTitle: title === '今日目标进度' ? DOC_TITLE : DOC_TITLE + ' · ' + title,
    title,
    eyebrow: EYEBROW,
