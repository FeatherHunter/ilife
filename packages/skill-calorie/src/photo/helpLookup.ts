...
export const PHOTO_HELP_MODULE = 'skill-calorie/photo/photo' as const;
...
function execFor(module: string, fn: string, argsExpr: string): string {
  return 'node --input-type=module -e "import { openDb } from \'skill-calorie\'; ' +
...
const KEY_EXEC: Record<string, { module: string; fn: string; args: string }> = {
  body_photo_add_single: { module: 'skill-calorie/photo/photos', fn: 'addPhotos', args: 'process.env.CALORIE_PHOTOS_DIR, { srcPaths: ["<照片路径>"], tag: "<标签>" }' },
  body_photo_add_note: { module: 'skill-calorie/photo/photos', fn: 'addPhotos', args: 'process.env.CALORIE_PHOTOS_DIR, { srcPaths: ["<照片路径>"], tag: "<标签>", note: "<备注>" }' },
  body_photo_add_batch: { module: 'skill-calorie/photo/photos', fn: 'addPhotos', args: 'process.env.CALORIE_PHOTOS_DIR, { srcPaths: ["<照片1>", "<照片2>"], tag: "<标签>" }' },
  ...
  body_photo_delete: { module: 'skill-calorie/photo/photos', fn: 'deletePhoto', args: 'process.env.CALORIE_PHOTOS_DIR, <ID>' },
