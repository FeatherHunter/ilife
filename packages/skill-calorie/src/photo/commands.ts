 ...
 * 两条读命令（体成分看／围度看那一族）没有代表唤醒词：`wakeWord` 按契约可缺，缺了速查表退回命令名
 * ——这两条与它们的 `case` 一样是纯搬迁，不补新唤醒词。
 */
...
export const PHOTO_COMMANDS = [
  { kind: 'read', key: 'calorie.help.center', shape: 'list', title: '身材照HELP', wakeWord: '记身材照', run: viewPhotoHelpCenter, example: 'calorie-cmd-read calorie.help.center --params \'{"q":"记身材照"}\'' },
  ...
  { kind: 'read', key: 'calorie.photo.gif', shape: 'analysis', title: '生成GIF', wakeWord: '做身材照GIF', run: viewPhotoGif, example: 'calorie-cmd-read calorie.photo.gif --params \'{"tag":"正面"}\'' },
