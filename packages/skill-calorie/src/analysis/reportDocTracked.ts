...
import { fmt, fmtInt, kvTable, lineOf, tableOf } from './reportDocParts.js';
import type { ReportPlate } from './reportPlate.js';

/** 目标追踪族（蛋白／水分）：两页同构，只换单位与目标源。
 *  达标率／达成率两个词是两张页原本的叫法（蛋白＝达标率、水分＝达成率），不合并成一个词。 */
export function buildTrackedBlocks(plate: ReportPlate): string {
  ...
  return renderKpiGrid([
    ...
    + lineOf(plate.points, which.line + '（虚线为目标）', { target: plate.target })
...
export function buildTdeeBlocks(plate: ReportPlate): string {
  const p = plate.base.profile;
  const avgIntake = plate.base.series.reduce<{ sum: number; n: number }>(
    (acc, s) => (s.calories === null ? acc : { sum: acc.sum + s.calories, n: acc.n + 1 }), { sum: 0, n: 0 });
  const intake = avgIntake.n === 0 ? null : Math.round((avgIntake.sum / avgIntake.n) * 10) / 10;
  const tdee = p.tdee;
  const gap = tdee === null || intake === null ? null : Math.round((tdee - intake) * 10) / 10;
  return renderKpiGrid([
    { label: '每日总消耗', value: fmtInt(tdee), unit: '卡', detail: 'TDEE（静态消耗 ＋ 日常活动）' },
    { label: '日均摄入', value: fmtInt(intake), unit: '卡', detail: '窗口 ' + plate.base.days + ' 天里 ' + avgIntake.n + ' 天有记录' },
    {
    ...
    + kvTable('计算口径' + (p.missing.length === 0 ? '' : '（缺：' + p.missing.join('、') + '）'), [
      { k: '公式', v: 'Mifflin-St Jeor：BMR × 活动系数' },
      { k: '基础代谢 BMR', v: fmtInt(p.bmr, ' 卡') },
      { k: '活动系数', v: p.activityFactor === null ? '—' : String(p.activityFactor) + '（' + p.activityLabel + '）' },
      { k: '身高 / 年龄 / 性别', v: fmt(p.heightCm, ' cm') + ' / ' + fmt(p.age, ' 岁') + ' / ' + p.genderLabel },
...
function tdeeCurve(plate: ReportPlate): string {
  ...
  if (p.heightCm === null || p.age === null || p.activityFactor === null || p.gender === null) {
    return renderDataTable({
      ...
      rows: [{ note: '档案四要素不全，画不了「总消耗随体重变化」曲线（缺：' + p.missing.join('、') + '）' }],
      caption: '总消耗随体重变化',
    });
  }
...
export function buildBmrBlocks(plate: ReportPlate): string {
  const p = plate.base.profile;
  ...
  return renderKpiGrid([
    { label: '基础代谢 BMR', value: fmtInt(p.bmr), unit: '卡', detail: 'Mifflin-St Jeor（四要素齐备才算）' },
    { label: '每日总消耗', value: fmtInt(p.tdee), unit: '卡', detail: 'BMR × 活动系数 ' + (p.activityFactor === null ? '—' : String(p.activityFactor)) },
    {
    ...
    + (danger
      ? renderDataTable({
        ...
        caption: '低于基础代谢（老侧口径：≥3 天即告警）',
      })
      : '')
    + lineOf(plate.points, '每日摄入（水平线为基础代谢 ' + fmtInt(th, ' 卡') + '）', { target: th })
    + renderDataTable({
      columns: [
    ...
    + kvTable('计算口径' + (p.missing.length === 0 ? '' : '（缺：' + p.missing.join('、') + '）'), [
      { k: '公式', v: 'Mifflin-St Jeor：10×体重 + 6.25×身高 − 5×年龄 ' + (p.gender === 'male' ? '+ 5' : '− 161') },
      { k: '身高 / 年龄 / 性别', v: fmt(p.heightCm, ' cm') + ' / ' + fmt(p.age, ' 岁') + ' / ' + p.genderLabel },
      { k: '活动量档位', v: p.activityLabel + (p.activityFactor === null ? '' : '（系数 ' + String(p.activityFactor) + '）') },
      { k: '危险信号判据', v: '窗口内摄入低于基础代谢的天数 ≥ 3 即告警（老侧口径）' },
    ]);
}
