...
import { renderCaliberLine, renderDataTable, renderKpiGrid, renderListRows, renderParamForm } from 'base-paint/blocks';
...
export function buildSearchDoc(s: ProductSearch): string {
  const parts: string[] = [
    ...
    renderParamForm({
      ...
      description: '食品库按名称／品牌模糊查找（营养值为每 100g）',
    }),
    renderKpiGrid([{ label: '找到', value: String(s.total), unit: '条', detail: '关键词：' + s.keyword }]),
    renderFoodGrid(s.items, true),
    renderCaliberLine('边输入边搜，不用按回车。'),
    line('food-src', '📂 数据来自本机食品库'),
    dataCopyArea('复制数据', {
      envelope: {
  ...
  return assembleDocPage({
    ...
    title: '🍱 食物热量查询',
    eyebrow: '卡路里 · 饮食',
    subtitle: '搜索：' + s.keyword + ' · 找到 ' + s.total + ' 条',
    content: parts.join(''),
    charts: false,
  });
}
...
export function buildLibraryDoc(lib: ProductLibrary, statsTotal: number): string {
  const parts: string[] = [
    ...
    renderParamForm({
      fields: [{
        name: 'category', label: '搜索', value: lib.category ?? '',
        hint: '输入名称或品牌，如 主食 / 蛋白类 / 水果（留空＝全部）',
      }],
      description: '按名称或品牌查找，不填显示全部',
    }),
    renderKpiGrid([{
      label: '本页',
      value: String(lib.total),
      unit: '条',
      detail: (lib.category === null ? '全部食品' : '分类：' + lib.category) + ' · 库内 ' + statsTotal + ' 条',
    }]),
...
/** 副题照老实物 `.sub`（`查询<query_label> · 匹配 N 条`；全量那支报库内总数）。
 *  #496 · 两处内部说法改人话（审查件第 70、72、74 条）：「查询全量」是内部状态，
 *  条数原本在第 4 行与卡片格下面那行说了两遍 ⇒ 副题只报「全部食品 · 共 N 条」，卡片格下那行删。 */
function librarySubtitle(lib: ProductLibrary, statsTotal: number): string {
  return lib.category === null
    ? '全部食品 · 共 ' + statsTotal + ' 条'
    : '分类：' + lib.category + ' · 共 ' + lib.total + ' 条';
}
...
/** #496 · 列名与文案改人话（审查件第 6、7、8 条）：「冗余」「重复组」「无重复组」都是内部词。
 *  表头 `ID` 是**记录编号**（读者要拿它去下架／改食品，所以留着可核对），列名照回执页写成中文。 */
...
export function buildDedupeDoc(v: DedupeView): string {
  ...
  const parts: string[] = [
    ...
    renderListRows({
      items: clean
        ? [{ main: '无需处理。' }]
        : [
          { main: '处理建议：保留营养数据最完整的一条，其余可用「下架食品」下架（数据保留可溯源）。' },
          { main: '或「改食品」合并数据后下架多余条目。' },
        ],
    }),
    line('dedupe-src', '📊 判定标准：名称和品牌都一样的算重复'),
    dataCopyArea('复制数据', {
      envelope: {
  ...
  return assembleDocPage({
    ...
    title: '📦 食品库去重',
    eyebrow: '卡路里 · 饮食',
    subtitle: '重复组 ' + v.groupCount + ' 组 · 可合并 ' + v.rowCount + ' 条 · 库内 ' + v.totalProducts + ' 条',
    content: parts.join(''),
    charts: false,
  });
}
