 ...
 *
 * #355 起本片两条对比记录（`order 246/247` 对比体脂／对比围度）已按 key 归属搬进
 * `src/body/routes.ts` 转 `exec`（`calorie.view.body-composition-compare`／
 * `calorie.view.body-measure-compare`）；本片留空片（只留文件头与空数组，不删文件）。
 * 记录形状见 `src/triggers/routeSpec.ts`；`order` 是该记录在**原列表内**的 0 基位次（顺序权威，
