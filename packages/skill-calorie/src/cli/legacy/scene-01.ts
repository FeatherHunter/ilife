/** 未搬迁命令的场景分区 · 场景 01（home）：**0 条**（#314 已整片搬空）。
 *
 ...
 * #314 起本片四条命令（`calorie.view.home`／`calorie.view.diet`／`calorie.view.exercise`／
 * `calorie.view.goal-progress`）住能力目录 `src/home/commands.ts`；本文件**只搬空、不删文件**
 * （片数与文件面是等号断言），仍留文件头与 `routing.ts` 字样给出键 → 场景的依据出处。
 *
