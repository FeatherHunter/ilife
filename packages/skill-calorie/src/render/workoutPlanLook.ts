 ...
 * 两级同时用会互相抢锚点（周与日共用一个 `#`）；② 选钮式**首屏无选中态可默认全展开**——
 * 首页「全部周次／全部」两枚默认选中，页面上**所有场次都在 DOM 里可见**，浏览器查找（Ctrl/⌘+F）
 * 与打印都拿得到全文；`:target` 式默认态没有锚点、样式也盖不到页签自身（要给页签上激活态得靠
...
/** 一枚页签（选钮的 id ＋ 显示词 ＋ 是否为「全部」那枚 ＋ 该日有无安排）。 */
interface TabItem {
  readonly id: string;
...
/** 选钮串：视觉上藏起来（类名 `ilw-wkr`／`ilw-dyr`，见页内样式），`all` 那枚默认选中。 */
...
function weekSection(index: number, w: PlanWeek): string {
  ...
  const items: TabItem[] = DAY_ITEMS.map((d) => ({
    id: prefix + '-' + d.id, text: d.text, all: d.id === 'all', off: d.id !== 'all' && !has.has(Number(d.id)),
  }));
...
/** 整块训练安排：周次页签 ＋ 周区块（首屏「全部周次」默认选中 ⇒ 各周全部展开、可搜可打印）。 */
export function planWeeksHtml(weeks: readonly PlanWeek[]): string {
  const items: TabItem[] = [
    { id: 'ilw-wk-all', text: '全部周次', all: true, off: false },
    ...weeks.map((w, i) => ({ id: 'ilw-wk-' + i, text: '第 ' + w.week + ' 周', all: false, off: false })),
  ];
