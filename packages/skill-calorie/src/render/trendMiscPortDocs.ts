...
/** 「整齐数」：把 v 收到 1／2／2.5／5／10 的整数倍上（`k=5` 偏细，给量程用）。 */
function niceRound(v: number, k: 2 | 2.5 | 5): number {
  const base = Math.pow(10, Math.floor(Math.log10(Math.abs(v))));
  const scaled = v / base;
  for (const choice of [1, 2, 2.5, 5, 10]) {
    if (scaled <= choice * k) return choice * base;
  }
  return 10 * base;
}
...
function calorieAxisOf(peak: number | null): {
  ...
  return axisOf(0, niceRound(peak, 5), '卡');
}
