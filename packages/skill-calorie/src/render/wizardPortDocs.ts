...
export function buildPhotoLogWizardDoc(v: PhotoLogWizardView): string {
  const content = [
    renderKpiGrid([
      // #474：照片格明细＝「最多 20 张」是**限制**，不是这一栏的数；值/单位就够，明细不再重复。
      { label: '照片', value: String(v.srcPaths.length), unit: '张', detail: '最多 20 张' },
...
function gifForm(v: GifPlannerView): string {
  return renderParamForm({
    ...
    fields: [
      ...
      // #474：原来是 `{"12":[x1,y1,x2,y2]}（像素，左上原点）`——全页最不可填的一栏（要现拼 JSON）。
      //  票面给的第二个选择：撤掉这一栏、改成一句自然语；不摆 4 个数字输入，是因为本层表单是
      //  零 JS 静态预览（B-09），输入不会回填进上面的指令，摆出来就是**点了没用的死控件**。
      { name: 'crops', label: '要裁剪哪几张？（留空＝整图，不裁）', value: '', hint: '想裁就说一句，如：帮我裁左上角 1/4' },
      { name: 'duration', label: '每帧多久（毫秒）', value: String(v.duration), hint: '50..5000', min: 50, max: 5000, step: 50 },
      // #474：`loop`／`transition` 由「看不见的 placeholder 写 `0/1/3/5`／`cut / fade / dissolve`」
      //  改成下拉（`blocks.ts` 的 options 分支已支持）。
      //  注意：**下拉的选项值就是 `loop`／`transition` 的字面量**（`option value` 即文本）——
      //  这样页面上是「无限／1 次／3 次／5 次」「硬切／淡入淡出／溶解」，
      //  复制给 AI 的指令里也是同一个词（中文），AI 照词就能跑，不再要求用户记英文码。
      { name: 'loop', label: '循环', value: String(v.loop), options: ['无限', '1 次', '3 次', '5 次'], hint: '选一个' },
      ...
      { name: 'transition', label: '切换效果', value: v.transition, options: ['硬切', '淡入淡出', '溶解'], hint: '选一个' },
