...
function deprecateRecord(db: DatabaseSync, id: number): void {
  db.prepare('UPDATE nutrition_products SET is_deprecated = 1, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(id);
}
  ...
  for (const { line, rec } of valid) {
    ...
    try {
      ...
      if (applyAll) {
        ...
        if (!opts.ask) throw new Error('重复需人工确认，CLI 请加 --on-duplicate o|s|d');
