...
#   skill-calorie 0.2.4（饮食／运动／体重／目标／身体细节／计划／训记联动；
...
# ──────────────────────────────────────────────────────────────────────────
# STAGES — 卡路里技能线发版（版本号已定死，只走交互式认证）。
...
banner "发布 skill-calorie $V_SKILL（1 包，你扫码）"
...
if (j.version !== '$V_SKILL') { console.error('  ✗ skill-calorie = ' + j.version + '（期望 $V_SKILL）—— 停下来找编排者'); process.exit(1); }
console.log('  ✓ skill-calorie = ' + j.version);
console.log('  skill-calorie → base-paint = ' + j.dependencies['base-paint']);
"
...
say "registry 现状（官方源）：skill-calorie 线上: $(reg_ver skill-calorie)；HEAD: $(git rev-parse --short HEAD)"

say "仓库自带的发版前置门（check-publish --pre）："
if node tooling/check-publish.mjs --pre --only skill-calorie,base-paint; then
...
step "清单里必须有 dist/cli/cmd_read.js（唯一出口）、SKILL.md、templates/*.html（diet／exercise／goal／help／home／photo-gallery 六件）。"
step "清单里必须**没有**源码之外的秘密：不许出现 node_modules、.env、密钥。"
step "对照 files 白名单：dist、SKILL.md、workflows/*.md、templates/*.html。"
pause "清单没问题？按回车进入发布（下面开始弹浏览器了）"

# ── Stage 4：发 skill-calorie ──────────────────────────────────────────
stage "4/5 · 发布 skill-calorie@$V_SKILL（你扫码）"
say "卡路里技能 0.2.4：饮食／运动／体重／目标／身体细节／计划／训记联动，唯一出口 calorie-cmd-read。"
if already skill-calorie "$V_SKILL"; then
  ...
  pause "看到 + skill-calorie@$V_SKILL 且浏览器已批准？按回车继续"
  cd "$ROOT"
fi
...
say "  skill-calorie : $SB（期望 $V_SKILL）"
say ""
say "skill-calorie 在 registry 上的依赖（都不许出现 workspace:）："
npm view "skill-calorie@$V_SKILL" dependencies --registry="$REG" --prefer-online 2>/dev/null | sed 's/^/    /' || warn "  查不到（registry 可能同步延迟，等 1 分钟重跑本 stage）"
if npm view "skill-calorie@$V_SKILL" dependencies --registry="$REG" --prefer-online 2>/dev/null | grep -qi 'workspace'; then
...
if [[ "$SB" == "$V_SKILL" ]]; then
  ...
  say "版本号对上 —— 技能发布成功。"
  say ""
  say "下一步：跑插件 wizard：packages/plugin-calorie/scripts/wizard-publish.sh（dsh-calorie 精确 pin skill-calorie@$V_SKILL，必须等这一版落 registry 才能发）。"
else
