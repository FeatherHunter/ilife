 ...
 *
 * 为什么需要它：`package.json` 的 exports 一条子路径只能指一个文件，而本件分三份写
 * （契约／状态／视图）。若把 `./directory-browser` 直接指到 `directory-browser-contract.js`，
 * 六家就取不到「开图接线」（它在 state 那份里）。故设这一个门，把两份纯逻辑都转出；
 * 视图那一份走 `./directory-browser-ui`（它要吃 React，单独一条）。
 *
 * 本件不含实现、也不含判断，只做转出——实现见同目录三份。
 */
