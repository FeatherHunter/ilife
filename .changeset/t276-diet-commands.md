...
二阶段收尾：`order: 40`（「批量导入食品」）复进 exec 路由（`src/diet/routes.ts` 加回那一行、`src/cli/legacy/routes/scene-02.ts` 删占位），真库实跑 exit 0 并写进库；`cmd-write-40-persist.test.mjs` 补 `calorie.product.import` 的写后只读回读断言；`m5-receipt-97.test.mjs` 的场景表补一行（45 → 46）。
